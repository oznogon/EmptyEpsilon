...
sp::ecs::Entity Faction::find(const string& name)
{
    ...
    LOG(Warning, "[faction] Attempted to find faction named ", name, ", but no faction name matched.");
    return {};
}

FactionInfo& Faction::getInfo(sp::ecs::Entity entity)
{
    if (auto faction = entity.getComponent<Faction>())
    {
        if (auto info = faction->entity.getComponent<FactionInfo>())
            return *info;
    }
...
FactionRelation Faction::getRelation(sp::ecs::Entity a, sp::ecs::Entity b)
{
    ...
    if (auto fb = b.getComponent<Faction>()) return fia.getRelation(fb->entity);
...
void Faction::didAnOffensiveAction(sp::ecs::Entity entity)
{
    ...
    // Check for each faction. If this faction knows if the target is hostile or
    // friendly, the target's faction now knows if this entity is hostile or
    // friendly.
    ...
    {
        // This faction knows if the target is hostile or friendly, so check if
        // we need to set its scan state to FriendOrFoeIdentified.
