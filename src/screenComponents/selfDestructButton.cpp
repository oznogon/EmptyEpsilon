...
void GuiSelfDestructButton::onUpdate()
{
    ...
    {
        if (keys.engineering_self_destruct_start.isDiscreteStepDown() && activate_button->isVisible())
        {
            activate_button->hide();
            confirm_button->show();
            cancel_button->show();
        }
        if (keys.engineering_self_destruct_confirm.isDiscreteStepDown() && confirm_button->isVisible())
        {
            confirm_button->hide();
            my_player_info->commandActivateSelfDestruct();
        }
        if (keys.engineering_self_destruct_cancel.isDiscreteStepDown() && cancel_button->isVisible())
        {
            activate_button->show();
            confirm_button->hide();
            cancel_button->hide();
