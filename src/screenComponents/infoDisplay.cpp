...
void EnergyInfoDisplay::onUpdate()
{
    ...
    if (reactor)
    {
        ...
        else setValue(toNearbyIntString(reactor->energy));

        if (reactor->energy < 100.0f) setBackColor(glm::u8vec4(255, 0, 0, 255));
        else setBackColor(glm::u8vec4{255, 255, 255, 255});
    }
}
...
void VelocityInfoDisplay::onUpdate()
{
    ...
    if (physics)
    {
        float velocity = glm::length(physics->getVelocity()) / 1000.0f * 60.0f;
        setValue(tr("{value} {unit}/min").format({
            {"value", string(velocity, 1)},
...
void HullInfoDisplay::onUpdate()
{
    ...
    if (hull)
    {
        setValue(string(hull->percentage()) + "%");

        if (hull->current < hull->max * 0.25f)
...
{
    if (shield_index == 0) setIcon("gui/icons/shields-fore");
    else if (shield_index == 1) setIcon("gui/icons/shields-aft");
    else setIcon("gui/icons/shields");
    setTextSize(20.0f);
}
...
void ShieldsInfoDisplay::onUpdate()
{
    ...
    {
        if (shields && static_cast<int>(shields->entries.size()) > shield_index)
        {
