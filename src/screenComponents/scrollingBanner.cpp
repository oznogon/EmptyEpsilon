...
void GuiScrollingBanner::onDraw(sp::RenderTarget& renderer)
{
    ...
    {
        // Use theme font. Fall back to bold_font if theme font is invalid.
        auto font = banner.font;
        if (!font) font = bold_font;

        // Prepare scrolling text.
        auto prepared = font->prepare(gameGlobalInfo->banner_string, 32, rect.size.y * 0.67f, banner.color, rect.size, sp::Alignment::CenterLeft);
