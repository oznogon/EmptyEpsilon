...
void GuiFrequencyCurve::onDraw(sp::RenderTarget& renderer)
{
    ...
    if (!scanned)
    {
        renderer.drawText(rect, tr("scienceFrequencyGraph", "Target not scanned"), sp::Alignment::Center, 30.0f);
        return;
    }

    if (frequency >= 0 && frequency <= BeamWeaponSys::max_frequency && enemy_has_equipment)
    {
        ...
        renderer.drawText(sp::Rect(rect.position.x, rect.position.y, rect.size.x, 40.0f), text, sp::Alignment::Center, 20.0f);
    }
    else
    {
        if (frequency_type == FrequencyType::Beam)
            renderer.drawText(rect, tr("scienceFrequencyGraph", "No beam data available"), sp::Alignment::Center, 30.0f);
        else
            renderer.drawText(rect, tr("scienceFrequencyGraph", "No shield data available"), sp::Alignment::Center, 30.0f);
    }
}
