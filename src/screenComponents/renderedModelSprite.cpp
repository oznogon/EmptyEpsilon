...
: GuiElement(owner, id), entity(entity), zoom_factor(1.0f), height(-1.f), angle(90.f), needs_rerender(true), last_render_size(0, 0), is_rotating(true)
{
}
...
void GuiRenderedModelSprite::onDraw(sp::RenderTarget& renderer)
{
    ...
    // Re-render if size changed, rotation is enabled, or flagged for rerender
    if (needs_rerender || is_rotating || pixel_size != last_render_size)
    {
        renderer.finish(); // Flush any pending 2D rendering
        bool render_success = renderToTexture(pixel_size);
        last_render_size = pixel_size;
        needs_rerender = is_rotating; // Keep re-rendering if rotating
