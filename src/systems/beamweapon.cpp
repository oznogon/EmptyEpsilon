...
void BeamWeaponSystem::update(float delta)
{
    ...
    for (auto [entity, beamsys, transform, reactor, docking_port, warp] : sp::ecs::Query<BeamWeaponSys, sp::Transform, sp::ecs::optional<Reactor>, sp::ecs::optional<DockingPort>, sp::ecs::optional<WarpDrive>>())
    {
        ...
        for (auto& mount : beamsys.mounts)
        {
            ...
            if (mount.range > 0.0f && Faction::getRelation(entity, target_entity) == FactionRelation::Enemy && delta > 0.0f && (!warp || warp->current == 0.0f) && (!docking_port || docking_port->state == DockingPort::State::NotDocking))
            {
                if (auto target_transform = target_entity.getComponent<sp::Transform>())
                {
                    ...
                    if (distance < mount.range * 1.3f)
                    {
                        ...
                        // If the target is in the beam's arc and range, the
                        // beam has cooled down, and the beam can consume enough
                        // energy to fire ...
                        if (distance < mount.range && mount.cooldown <= 0.0f
                            && fabsf(angle_diff) < mount.arc * 0.5f
                            && (!reactor || reactor->useEnergy(mount.energy_per_beam_fire)))
                        {
                            ...
                            // When we fire a beam, and we hit an enemy, check
                            // if we are not scanned yet, if we are not, and we
                            // hit something that we know is an enemy or
                            // friendly, we now know if this ship is an enemy or
                            // friend.
                            Faction::didAnOffensiveAction(entity);

                            // Reset beam's cycle time.
                            mount.cooldown = mount.cycle_time;

                            // Get the angle of the beam strike to draw its
                            // effect.
                            ...
                            {
                                const float r_half = r * 0.5f;
                                auto local_hit_location = hit_location - target_transform->getPosition();
                                be.target_offset = glm::vec3(
                                    local_hit_location.x + random(-r_half, r_half),
                                    local_hit_location.y + random(-r_half, r_half),
                                    random(-r * 0.25f, r * 0.25f)
                                );

                                auto shield = target_entity.getComponent<Shields>();
                                if (shield && shield->active)
                                    be.target_offset = glm::normalize(be.target_offset) * r;
                                else
                                    be.target_offset = glm::normalize(be.target_offset) * random(0.0f, r_half);
            ...
            // If the beam is turreted and can move, but doesn't have a target,
    ...
    // Progress the BeamEffect drawing.
...
void BeamWeaponSystem::render3D(sp::ecs::Entity e, sp::Transform& transform, BeamEffect& be)
{
    ...
    glUniform4f(beamShader.get().uniform(ShaderRegistry::Uniforms::Color), be.lifetime, be.lifetime, be.lifetime, 1.0f);
    ...
    // Draw the beam.
    {
        ...
        quad[1].vertex = v1;
        quad[1].texcoords = {0.0f, 1.0f};
        quad[2].vertex = v2;
        quad[2].texcoords = {1.0f, 1.0f};
        quad[3].vertex = v3;
        quad[3].texcoords = {1.0f, 0.0f};
...
void drawArc(sp::RenderTarget& renderer, glm::vec2 arc_center, float angle0, float arc_angle, float arc_radius, glm::u8vec4 color)
{
    ...
    if (beam_arc < 360.f)
    {
        ...
        auto halfway_angle = vec2FromAngle(angle0 + beam_arc * 0.5f) * beam_range;
...
void BeamWeaponSystem::renderOnRadar(sp::RenderTarget& renderer, sp::ecs::Entity entity, glm::vec2 screen_position, float scale, float rotation, BeamWeaponSys& beamsystem)
{
    ...
    for (auto& mount : beamsystem.mounts)
    {
        ...
        drawArc(renderer, arc_center, rotation + (beam_direction - beam_arc * 0.5f), beam_arc, beam_range * scale, color);
        ...
        drawArc(renderer, arc_center, rotation + (turret_direction - turret_arc * 0.5f), turret_arc, beam_range * scale, color);
    }
}
...
void BeamWeaponSystem::renderOnRadar(sp::RenderTarget& renderer, sp::ecs::Entity entity, glm::vec2 screen_position, float scale, float rotation, BeamEffect& beam_effect)
{
    ...
    renderer.drawLine(source_position, target_position, 2.0f, beam_color);
