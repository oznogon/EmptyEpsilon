...
void TractorBeamSystem::update(float delta)
{
    ...
    for(auto [entity, tractorsys, target, transform, reactor, docking_port] : sp::ecs::Query<TractorBeamSys, Target, sp::Transform, sp::ecs::optional<Reactor>, sp::ecs::optional<DockingPort>>()) {
        ...
        for(auto& mount : tractorsys.mounts) {
            if (mount.cooldown > 0.0f)
                mount.cooldown -= delta * tractorsys.getSystemEffectiveness();
                    ...
                    if (distance < mount.range * 1.3f)
                    {
                        ...
                        if (mount.turret_arc > 0)
                        {
                            ...
                            if (mount.turret_rotation_rate > 0)
                            {
                                ...
                                if (fabsf(turret_angle_diff) < mount.turret_arc / 2.0f)
                                {
                                    ...
                                    {
                                        mount.direction += (angle_diff / fabsf(angle_diff)) * std::min(mount.turret_rotation_rate * tractorsys.getSystemEffectiveness(), fabsf(angle_diff));
                                    }
                                // If the target is outside of the turret's arc ...
                                    ...
                                    {
                                        mount.direction += (reset_angle_diff / fabsf(reset_angle_diff)) * std::min(mount.turret_rotation_rate * tractorsys.getSystemEffectiveness(), fabsf(reset_angle_diff));
                                    }
                                }
                            }
                        }
                        ...
                        if (distance < mount.range && mount.cooldown <= 0.0f && fabsf(angle_diff) < mount.arc / 2.0f && (!reactor || reactor->useEnergy(mount.energy_per_beam_fire)))
                        {
                            ...
                            if (entity.hasComponent<Coolant>())
                                tractorsys.addHeat(mount.heat_per_beam_fire);
                            ...
                            info.frequency = tractorsys.frequency;
                            info.system_target = tractorsys.system_target;
