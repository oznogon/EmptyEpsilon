...
: ShipAI(owner)
{
    ...
    timeout = 0.0f;
    aggression = random(0.0f, 0.25f);
}

bool FighterAI::canSwitchAI()
{
    return !(owner.hasComponent<Target>() && (has_missiles || has_beams) && attack_state == State::Dive);
}

void FighterAI::run(float delta)
{
    if (timeout > 0.0f) timeout -= delta;
    ShipAI::run(delta);
}

void FighterAI::runOrders()
{
    if (aggression > 0.5f) aggression -= random(0.0f, 0.25f);
    ShipAI::runOrders();
}
...
void FighterAI::runAttack(sp::ecs::Entity target)
{
    ...
    case State::Dive:
    {
        auto target_size = target_physics ? std::max(target_physics->getSize().x, target_physics->getSize().y) : 0.0f;
        if (distance < 2500.0f + target_size && has_missiles)
        {
        ...
        flyTowards(tt->getPosition(), 500.0f);

        if (distance < 500.0f + target_size)
        {
            ...
            if (a_diff < 0.0f) evade_direction = target_dir - random(25.0f, 40.0f);
            else evade_direction = target_dir + random(25.0f, 40.0f);
        }
    ...
    case State::Evade:
        if (distance > 2500.0f || timeout <= 0.0f) attack_state = State::Dive;
        else
        {
            if (auto thrusters = owner.getComponent<ManeuveringThrusters>()) thrusters->target = evade_direction;
            if (auto impulse = owner.getComponent<ImpulseEngine>()) impulse->request = 1.0f;
        }
        break;
    case State::Recharge:
