...
float EvasionAI::evasionDangerScore(sp::ecs::Entity ship, float scan_radius)
{
    ...
    if (enemy_missile_strength <= 0.0f && (enemy_beam_dps <= 0.0f || enemy_max_beam_range <= 0.0f || enemy_max_utility_range <= 0.0f))
    {
    ...
    if (enemy_max_utility_range > 0.0f && distance < 4.0f * enemy_max_utility_range)
    {
        // danger falls off the further we are away from beam range
        danger += (4.0f * enemy_max_utility_range - std::max(distance, enemy_max_utility_range)) / (3 * enemy_max_utility_range);
    }
