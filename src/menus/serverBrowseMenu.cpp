...
ServerBrowserMenu::ServerBrowserMenu(std::optional<GameClient::DisconnectReason> last_attempt /* = {} */)
{
    ...
    (new GuiTextTooltip(back_button, "BACK_TIP", tr("tooltips", "Return to the main menu.")))->setWidth();
    ...
    connect_button
        ...
        ->setSize(250.0f, GuiElement::GuiSizeRow);
    (new GuiTextTooltip(connect_button, "CONNECT_TIP", tr("tooltips", "Connect to the EmptyEpsilon server at this address.")))->setWidth();
    ...
    manual_ip
        ...
        ->setSize(250.0f, GuiElement::GuiSizeRow);
    (new GuiTextTooltip(manual_ip, "MANUAL_IP_TIP", tr("tooltips", "Enter the server's IP address or domain. If it uses a custom port, add a colon (:) and then the port number at the end.")))->setWidth();
