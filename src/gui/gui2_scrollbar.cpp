...
void GuiScrollbar::onDraw(sp::RenderTarget& renderer)
{
    ...
    range = static_cast<float>(max_value - min_value);
    arrow_size = rect.size.x / 2.0F;
    move_height = rect.size.y - (arrow_size * 2.0F);
    // Clamp the size of the draggable bar to no less than 2px.
    bar_size = std::clamp(move_height * value_size / range, 2.0F, move_height);
