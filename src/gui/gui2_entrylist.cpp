...
int GuiEntryList::indexByValue(string value) const
{
    for (unsigned int n = 0; n < entries.size(); n++)
        if (entries[n].value == value) return n;
...
void GuiEntryList::removeEntry(int index)
{
    ...
    if (selection_index == index) setSelectionIndex(-1);
    if (selection_index > index) setSelectionIndex(selection_index - 1);
...
{
    if (selection_index < 0 || selection_index >= static_cast<int>(entries.size())) return "";
    return entries[selection_index].value;
}

void GuiEntryList::sortEntries()
{
    std::sort(entries.begin(), entries.end(),
        [](const GuiEntry& a, const GuiEntry& b) { return a.name < b.name; });
    entriesChanged();
}

GuiEntryList* GuiEntryList::setSortByName(bool enable)
{
    sort_by_name = enable;
    if (enable) sortEntries();
    return this;
}

bool GuiEntryList::getSortByName() const
{
    return sort_by_name;
}
