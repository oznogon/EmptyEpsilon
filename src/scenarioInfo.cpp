...
ScenarioInfo::ScenarioInfo(string filename)
{
    ...
    locale = i18n::Catalogue::create("locale/" + filename.replace(".lua", "." + PreferencesManager::get("language", "en_US") + ".po"));
    string key;
    string value;
    ...
    {
        LOG(Warning, "[scenarioinfo] No category for scenario: ", filename);
...
void ScenarioInfo::addKeyValue(string key, string value)
{
    ...
    else if (key.lower() == "setting" && additional != "")
    {
        ...
        LOG(Warning, "[scenarioinfo] Unknown scenario meta data: ", key, ": ", value);
}
...
bool ScenarioInfo::addSettingOption(string key, string option, string description)
{
    ...
    for (auto& setting : settings)
    {
        if (setting.key == key)
        {
            ...
            if (tag == "default") setting.default_option = option;
...
const std::vector<ScenarioInfo>& ScenarioInfo::getScenarios()
{
    if (cached_full_list.empty())
    {
        ...
        LOG(Debug, "[scenarioinfo] Scenario time: ", static_cast<int>(
...
std::vector<ScenarioInfo> ScenarioInfo::getScenarios(const string& category)
{
    ...
    for (const auto& info : getScenarios())
        if (info.hasCategory(category)) result.push_back(info);
