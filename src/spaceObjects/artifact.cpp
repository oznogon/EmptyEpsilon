...
REGISTER_SCRIPT_SUBCLASS(Artifact, SpaceObject)
{
    ...
    /// Example: setModel("artifact6")
    REGISTER_SCRIPT_CLASS_FUNCTION(Artifact, setModel);
    /// Defines whether to destroy this artifact with a visual explosion.
    ...
    /// Sets a function to call every tick when a SpaceObject is colliding with the artifact.
    /// This passes the artifact and colliding SpaceObject to the called function.
    ...
    /// Sets a function to call every tick when a PlayerSpaceship is colliding with the artifact.
    /// This passes the artifact and colliding PlayerSpaceship to the called function.
    ...
    /// Sets a function to call once when a PlayerSpaceship collides with the artifact and allowPickup is enabled.
    /// This passes the artifact and colliding PlayerSpaceship to the called function.
    ...
    REGISTER_SCRIPT_CLASS_FUNCTION(Artifact, onPickup);
    /// Defines whether the artifact rotates, and if so at what rotational velocity.
    ...
    /// Optional; artifacts by default use "blip.png".
