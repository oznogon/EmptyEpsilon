  ...
  selected_entity(sp::ecs::Entity())
{
    ...
    for (int n = 0; n < ShipSystem::COUNT; n++)
    {
        ...
        info.heat_icon
            ->setColor(theme->getStyle("overlay.overheating")->get(getState()).color)
            ->setPosition(0.0f, 0.0f, sp::Alignment::Center)
    ...
    for (int n = 0; n < ShipSystem::COUNT; n++)
    {
        ...
        info.damage_icon
            ->setColor(theme->getStyle("overlay.damaged")->get(getState()).color)
            ->setPosition(0.0f, 0.0f, sp::Alignment::Center)
            ->setSize(GuiElement::GuiSizeMatchHeight, GuiElement::GuiSizeMax);
    ...
    (new GuiButton(supply_controls_center_row, "", "<",
        [this]()
        {
            ...
            auto bay = my_spaceship.getComponent<DockingBay>();
            if (!bay) return;
            auto carrier_launcher = my_spaceship.getComponent<ScanProbeLauncher>();
            if (!carrier_launcher) return;
            if (selected_berth_index < 0 || selected_berth_index >= static_cast<int>(bay->berths.size())) return;
...
void DockingBayScreen::updateSelectedEntityDisplay()
{
    ...
    selected_entity = selected_berth.docked_entity;
    ...
    auto docked_pickup = selected_entity.getComponent<PickupCallback>();
    energy_transfer_direction->setEnable(carrier_reactor && (docked_reactor || docked_pickup));

    if (docked_reactor || docked_pickup)
    {
        entity_energy
            ->setColor(glm::u8vec4(255, 255, 255, 255));
        ...
        if (docked_reactor)
        {
            ...
            entity_energy->setValue(toNearbyIntString(docked_pickup->give_energy));
    }
    else
    {
        entity_energy
    ...
    {
        entity_probes
            ->setValue(static_cast<string>(scan_probes->stock) + "/" + static_cast<string>(scan_probes->max))
            ->setColor(glm::u8vec4(255, 255, 255, 255));
        berth_scan_probes
            ->setValue(static_cast<string>(scan_probes->stock) + "/" + static_cast<string>(scan_probes->max))
            ->setColor(glm::u8vec4(255, 255, 255, 255));
    }
    else if (auto scan_probes = selected_entity.getComponent<PickupCallback>())
    {
        entity_probes
            ->setValue(scan_probes->give_probe)
            ->setColor(glm::u8vec4(255, 255, 255, 255));
        berth_scan_probes
            ->setValue(scan_probes->give_probe)
            ->setColor(glm::u8vec4(255, 255, 255, 255));
    ...
    {
        entity_probes
            ->setValue(tr("dockingbay", "No launcher"))
            ->setColor(glm::u8vec4(128, 128, 128, 255));
        berth_scan_probes
            ->setValue(tr("dockingbay", "No launcher"))
            ->setColor(glm::u8vec4(128, 128, 128, 255));
