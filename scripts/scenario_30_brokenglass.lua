...
function init()
    ...
    Admin_station.nebula_bonuses.surprise_attack.onFullScan = function(self)
        ...
        Player:addToShipLog(
            _(
                "nebulaBonuses-shipLog",
                "We have activated a drone defence system!"
            ),
            "Red"
        )
    end
...
function CommsDefenceStation(comms_source, comms_target)
    ...
    elseif Defence_station.tier2_mission_state == "arrived" then
        setCommsMessage(
            _(
                "defenceStn-comms",
                [[Your orders are to scan those beacons and find the defence grid controller]]
            )
        )
        return
    elseif
...
If you're not busy, please drop by to learn more about our projects.]]
        ...
        then
            setCommsMessage(
                _(
                    "adminStn-comms",
                    [[Always a pleasure to have you as our honoured guests.]]
                )
            )
            ...
            then
                ...
                if not Defence_station.cover_blown then
                    ...
                    Defence_station:sendCommsMessage(
                        ...
                        _(
                            "defenceStn-incCall",
                            [[Your cover has been blown and the proximity defence system remains active. We suggest leaving and trying again to arrive with a future convoy.]]
                        )
                    )
                    Player:addToShipLog(
...
function TransferToDrone()
    ...
    Player:addToShipLog(
        _(
            "drone-shipLog",
            "This ship will not activate the drone station proximity defences as long as you arrive with a convoy."
        ),
        "Yellow"
    )
end
            ...
            if stn:isValid() then
                ...
                if distance(Player, stn) < 2000 and stn.prox_spawned == nil then
                    ...
                    Player:addToShipLog(
                        _(
                            "drone-shipLog",
                            "We got too close to a drone station and it launched defences."
                        ),
                        "Red"
                    )
