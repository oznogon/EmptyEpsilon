...
-- Enemies[Large]: Larger battle. This increases the fleet sizes to 100 ships per side.
-- Enemies[Medium|Default]: Medium battle. 60 ships per side.
-- Enemies[Huge]: Huge battle. This increases the fleet sizes to 500 ships per side.

--- Scenario
-- @script scenario_99_battlefield

function init()
    neutral_station = SpaceStation()
    ...
    neutral_station.comms_data =
        { supplydrop = "neutral", weapons = { Mine = "friend" } }
    friendly_station = SpaceStation()
        :setTemplate("Large Station")
        :setPosition(-10000, -25000)
        :setRotation(random(0, 360))
    ...
    -- Scale fleet sizes based on the scenario settings.
    if getScenarioSetting("Enemies") == "Large" then
        battle_scale = 3.3
        location_scale = 1.5
    elseif getScenarioSetting("Enemies") == "Huge" then
        battle_scale = 16.6
        location_scale = 3
    else
        battle_scale = 1
    ...
    for n = 1, 20 * battle_scale do
        CpuShip()
            ...
            :setPosition(
                random(-10000 * location_scale, 10000 * location_scale),
                random(0, 3000)
            )
            :setRotation(90)
            :setFaction(faction)
    ...
    for n = 1, 20 * battle_scale do
        CpuShip()
            :setTemplate("MT52 Hornet")
            :setPosition(
                random(-13000 * location_scale, 13000 * location_scale),
                random(5000, 8000)
            )
            :setRotation(-90)
            :setFaction(faction)
    ...
    for n = 1, 1 * battle_scale do
        CpuShip()
            ...
            :setPosition(
                random(-3000 * location_scale, 3000 * location_scale),
                7000
            )
            :setRotation(-90)
            :setFaction(faction)
