...
function init()
    ...
    -- == Core area
    Coreplanet = Planet()
        :setPosition(29855, -4285)
        :setPlanetRadius(3000)
        :setPlanetAtmosphereColor(0.20, 0.20, 1.00)
        :setDistanceFromMovementPlane(-2000.00)
        ...
        :setPlanetCloudRadius(3150.00)
    Planet()
        :setPosition(30179, 9539)
        :setOrbit(Coreplanet, 120)
        :setPlanetRadius(1000)
        :setDistanceFromMovementPlane(-2000.00)
        :setPlanetSurfaceTexture("planets/moon-1.png")
        :setPlanetCloudRadius(1050.00)
        :setAxialRotationTime(20.00)
        :setPlanetAtmosphereColor(0.20, 0.20, 1.00)
        :setPlanetAtmosphereTexture("planets/atmosphere.png")

    -- Mission stations
    Admin_station = SpaceStation()
        :setTemplate("Small Station")
        :setFaction("Human Navy")
        :onTakingDamage(CommsBeingAttacked)
        :setCallSign("Admin Stn")
        :setPosition(35170, -8650)
        :setRepairDocked(true)
        :setRestocksScanProbes(true)
        :setSharesEnergyWithDocked(true)
    Defence_station = SpaceStation()
        :setTemplate("Medium Station")
        :setFaction("Human Navy")
        :onTakingDamage(CommsBeingAttacked)
        :setCallSign("Defence Stn")
        :setPosition(-9366, 7900)
        :setRepairDocked(true)
        :setRestocksScanProbes(true)
        :setSharesEnergyWithDocked(true)
    Wormhole_station = SpaceStation()
        :setTemplate("Medium Station")
        :setFaction("Independent")
        :onTakingDamage(CommsBeingAttacked)
        :setCallSign("Wormhole Stn")
        :setPosition(77152, 32816)
        :setRepairDocked(true)
        :setRestocksScanProbes(true)
        :setSharesEnergyWithDocked(true)
    ...
    Patrol_stations = {
        SpaceStation()
            :setTemplate("Medium Station")
            :setFaction("Human Navy")
            :setCallSign("NW Checkpoint")
            :onTakingDamage(CommsBeingAttacked)
            :setPosition(-14904, -31057)
            :setRepairDocked(true)
            :setRestocksScanProbes(true)
            :setSharesEnergyWithDocked(true),
        SpaceStation()
            :setTemplate("Medium Station")
            :setFaction("Human Navy")
            :setCallSign("SW Checkpoint")
            :onTakingDamage(CommsBeingAttacked)
            :setPosition(-29095, 37628)
            :setRepairDocked(true)
            :setRestocksScanProbes(true)
            :setSharesEnergyWithDocked(true),
        SpaceStation()
            :setTemplate("Medium Station")
            :setFaction("Human Navy")
            :setCallSign("SE Checkpoint")
            :onTakingDamage(CommsBeingAttacked)
            :setPosition(73035, 26331)
            :setRepairDocked(true)
            :setRestocksScanProbes(true)
            :setSharesEnergyWithDocked(true),
        SpaceStation()
            :setTemplate("Medium Station")
            :setFaction("Human Navy")
            :setCallSign("NE Checkpoint")
            :onTakingDamage(CommsBeingAttacked)
            :setPosition(75919, -30678)
            :setRepairDocked(true)
            :setRestocksScanProbes(true)
            :setSharesEnergyWithDocked(true),
    }
    ...
    Lookout_stations = {
        ...
        SpaceStation()
            ...
            :setFaction("Human Navy")
            :setCallSign("DS818")
            :setPosition(-35690, 21033)
            :setRepairDocked(true)
            :setRestocksScanProbes(true)
            :setSharesEnergyWithDocked(true),
    }
    reference_station = SpaceStation()
        :setTemplate("Medium Station")
        :setFaction("Ghosts")
        :setCallSign("DS820")
        :setPosition(-38789, 14192)
    SpaceStation()
        :setTemplate("Medium Station")
        :setFaction("Ghosts")
        :setCallSign("DS821")
        :setPosition(-47557, 15983)
    ...
    local northNebulaTLx, northNebulaTLy =
        sectorToXY(reference_nebula:getSectorName())
    Admin_station.lost_ship = CpuShip()
        :setFaction("Human Navy")
        :setTemplate("Equipment Freighter 2")
        :setCallSign("VS1")
        :setPosition(
            northNebulaTLx + irandom(0, 80000),
            northNebulaTLy + irandom(0, 60000)
        )
        :orderRoaming()
        :setCommsFunction(function()
            setCommsMessage(
                _(
                    "lost-comms",
                    "We're pretty busy scanning for treasure... I mean, conducting research in this nebula. Can we call you back?"
                )
            )
        end)
        ...
        bonus
            ...
            :setDescriptions(
                _(
                    ...
                    "Just a hollow spherical apparition. Kinda looks like it's hiding something."
                ),
                _(
                    "scienceDescription-artifact",
                    "It seems to have changed shape."
                )
            )
            :setScanningParameters(1, 1)
            :setRadarSignatureInfo(0.25, 0, 0)
            :setPosition(
                northNebulaTLx + irandom(0, 80000),
                northNebulaTLy + irandom(0, 60000)
            )
    ...
    Admin_station.nebula_bonuses.nuke.onFullScan = function(self)
        self:setModel("ammo_box"):allowPickup(true):onPickUp(function()
            self.wasFound = true
            Player:setWeaponStorageMax(
                "Nuke",
                Player:getWeaponStorageMax("Nuke") + 1
            )
            Player:setWeaponStorage("Nuke", Player:getWeaponStorage("Nuke") + 1)
            Player:addToShipLog(
                _(
                    "nebulaBonuses-shipLog",
                    "A Nuke and additional nuke storage have been added to our arsenal"
                ),
                "Green"
            )
        end)
    ...
    Admin_station.nebula_bonuses.repair.onFullScan = function(self)
        self:setModel("artifact1"):allowPickup(true):onPickUp(function()
            ...
            Player:addToShipLog(
                _(
                    "nebulaBonuses-shipLog",
                    "A stranded repair crew has been rescued"
                ),
                "Green"
            )
        end)
    ...
    Admin_station.nebula_bonuses.rep.onFullScan = function(self)
        self:setModel("artifact3"):allowPickup(true):onPickUp(function()
            ...
            Player:addToShipLog(
                _(
                    "nebulaBonuses-shipLog",
                    "This scientific data is sure to raise our reputation with the eggheads."
                ),
                "Green"
            )
        end)
    ...
    Admin_station.nebula_bonuses.surprise_attack.onFullScan = function(self)
        ...
        self:sendCommsMessage(
            Player,
            _("nebulaBonuses-incCall", "DRONE SELF DEFENCE SYSTEM ACTIVATED")
        )
        Player:addToShipLog(
            _("nebulaBonuses-shipLog", "DRONE SELF DEFENCE SYSTEM ACTIVATED"),
            "Red"
        )
        self:setCallSign("")
        local x, y = self:getPosition()
        local defenders = SpawnEnemies(
            x + irandom(-5000, 5000),
            y + irandom(-5000, 5000),
            random(0.6, 1),
            "Ghosts"
        ...
        Player:addToShipLog(
            _(
                "nebulaBonuses-shipLog",
                "We have activated a drone defense system!"
            ),
            "Red"
        )
    end
    ...
    Drone_stations = {
        SpaceStation()
            ...
            :setFaction("Ghosts")
            :setCallSign("DS6223")
            :setPosition(-80404, 26565),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6224")
            :setPosition(-84704, 30527),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6225")
            :setPosition(-81989, 28198),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6222")
            :setPosition(-80788, 21186),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6226")
            :setPosition(-81508, 32713),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6234")
            :setPosition(-83718, 24644),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1425")
            :setPosition(-104181, 26524),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1435")
            :setPosition(-106339, 23746),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6241")
            :setPosition(-104996, 21138),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1453")
            :setPosition(-101798, 22233),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1433")
            :setPosition(-98728, 27731),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1426")
            :setPosition(-99716, 24125),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6271")
            :setPosition(-98611, 20912),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6231")
            :setPosition(-87191, 27553),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6232")
            :setPosition(-88233, 24260),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6230")
            :setPosition(-93180, 27670),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6229")
            :setPosition(-90154, 21282),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6254")
            :setPosition(-113187, 33418),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6257")
            :setPosition(-114746, 30023),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6263")
            :setPosition(-118876, 34875),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6249")
            :setPosition(-116859, 37036),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6243")
            :setPosition(-113257, 21426),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6244")
            :setPosition(-117291, 21330),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6247")
            :setPosition(-115322, 26325),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1420")
            :setPosition(-109366, 27152),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1418")
            :setPosition(-107908, 34572),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1436")
            :setPosition(-106339, 30558),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1419")
            :setPosition(-110691, 30558),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1454")
            :setPosition(-109745, 22233),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6210")
            :setPosition(-84102, 34490),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6227")
            :setPosition(-88817, 30377),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1447")
            :setPosition(-94418, 22044),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1427")
            :setPosition(-95334, 24236),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1449")
            :setPosition(-94229, 33207),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1445")
            :setPosition(-89499, 34153),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6269")
            :setPosition(-96064, 35728),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6262")
            :setPosition(-98980, 34974),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1443")
            :setPosition(-103193, 34904),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1422")
            :setPosition(-102933, 30937),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1428")
            :setPosition(-95411, 29922),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS1437")
            :setPosition(-98909, 31499),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6264")
            :setPosition(-118415, 29798),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6246")
            :setPosition(-118973, 24308),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6266")
            :setPosition(-118721, 38835),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6250")
            :setPosition(-113497, 38189),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6251")
            :setPosition(-109558, 37660),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6204")
            :setPosition(-97263, 38093),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6252")
            :setPosition(-107349, 38717),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6211")
            :setPosition(-81220, 38285),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6206")
            :setPosition(-85591, 38429),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6214")
            :setPosition(-91067, 37228),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6205")
            :setPosition(-92844, 39053),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Ghosts")
            :setCallSign("DS6268")
            :setPosition(-102543, 39055),
    }
    ...
    for i = 1, 3 + Difficulty do -- 4, 6, or 8 artifacts
        local a = Artifact():setPosition(
            droneNEx - irandom(2000, 38000),
            droneNEy + irandom(2000, 18000)
        ):setModel("artifact" .. i)
    ...
    Parts_station = SpaceStation()
        :setTemplate("Small Station")
        :setFaction("Exuari")
        :setCallSign("X472")
        :setPosition(361326, 527812)
    ...
    Kw_stations = {
        SpaceStation()
            ...
            :setFaction("Kraylor")
            :setCallSign("KZ2346")
            :setPosition(376303, 543016),
        SpaceStation()
            :setTemplate("Medium Station")
            :setFaction("Kraylor")
            :setCallSign("KZ2682")
            :setPosition(400931, 541600),
        SpaceStation()
            :setTemplate("Medium Station")
            :setFaction("Kraylor")
            :setCallSign("KZ2683")
            :setPosition(360825, 561407),
    }
    ...
    Exuari_junk_stations = {
        SpaceStation()
            ...
            :setFaction("Exuari")
            :setCallSign("X6775")
            :setPosition(315754, 449577),
        SpaceStation()
            :setTemplate("Medium Station")
            :setFaction("Exuari")
            :setCallSign("X6774")
            :setPosition(333160, 452975),
        SpaceStation()
            :setTemplate("Medium Station")
            :setFaction("Exuari")
            :setCallSign("XS6773")
            :setPosition(326495, 458049),
    }
    SpaceStation()
        :setTemplate("Medium Station")
        :setFaction("Human Navy")
        :setCallSign("OP722")
        :setPosition(328913, 490292)
    SpaceStation()
        :setTemplate("Medium Station")
        :setFaction("Independent")
        :setCallSign("DS456")
        :setPosition(327511, 477797)
    ...
    -- Northern colony
    Colony_area_station = SpaceStation()
        :setTemplate("Medium Station")
        :setFaction("Human Navy")
        :setCallSign("DS806")
        :setPosition(40015, -60153)
    Planet()
        :setPosition(41835, -78305)
        :setPlanetRadius(5000)
        :setPlanetCloudRadius(5200.00)
        :setPlanetAtmosphereColor(0.8, 0.20, 0.40)
        :setPlanetSurfaceTexture("planets/gas-2.png")
    Colony_stations = {
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E437")
            :setPosition(34457, -71084)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E146")
            :setPosition(31997, -76192)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E544")
            :setPosition(23031, -75857)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E236")
            :setPosition(25371, -73885)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E245")
            :setPosition(29204, -74511)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E335")
            :setPosition(25371, -78805)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E338")
            :setPosition(27077, -70894)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E465")
            :setPosition(37335, -71048)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E939")
            :setPosition(35632, -65750)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E869")
            :setPosition(29010, -63480)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E166")
            :setPosition(23668, -68398)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E770")
            :setPosition(30334, -67832)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("E671")
            :setPosition(29388, -79374)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z649")
            :setPosition(31984, -82788)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z128")
            :setPosition(26390, -83457)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z342")
            :setPosition(22418, -88122)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z929")
            :setPosition(21252, -84484)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z243")
            :setPosition(27527, -87933)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z750")
            :setPosition(31280, -87888)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z848")
            :setPosition(33846, -85391)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z417")
            :setPosition(26361, -95457)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z359")
            :setPosition(29766, -97160)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z532")
            :setPosition(32257, -91906)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z118")
            :setPosition(32037, -93754)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z441")
            :setPosition(23248, -92193)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("Z260")
            :setPosition(35632, -98484)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H419")
            :setPosition(43760, -87132)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H823")
            :setPosition(51901, -87244)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H663")
            :setPosition(52483, -91098)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H524")
            :setPosition(52658, -82703)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H158")
            :setPosition(48343, -89446)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H251")
            :setPosition(43488, -97649)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H320")
            :setPosition(42444, -94700)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H252")
            :setPosition(43958, -91294)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H425")
            :setPosition(56016, -90406)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H922")
            :setPosition(57203, -94511)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H762")
            :setPosition(54743, -97917)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H121")
            :setPosition(49445, -94511)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("H364")
            :setPosition(47845, -98361)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X627")
            :setPosition(51248, -69291)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X957")
            :setPosition(43438, -68261)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X726")
            :setPosition(55485, -68625)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X547")
            :setPosition(49960, -63472)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X856")
            :setPosition(46654, -65612)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X254")
            :setPosition(53771, -63981)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X155")
            :setPosition(43248, -64098)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X267")
            :setPosition(53986, -71427)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X134")
            :setPosition(56067, -75779)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X468")
            :setPosition(49256, -78617)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X333")
            :setPosition(50794, -73146)
            :setCommsFunction(CommsColonyStation),
        SpaceStation()
            :setTemplate("Small Station")
            :setFaction("Independent")
            :setCallSign("X361")
            :setPosition(45996, -70975)
            :setCommsFunction(CommsColonyStation),
    }
    ...
    Defence_station:sendCommsMessage(
        Player,
        _(
            "defenceStn-incCall",
            [[Greetings, Captain.
...
function InitPlayer()
    ...
    Player = PlayerSpaceship()
        ...
        :setCallSign("Propitious 1")
        :setPosition(8785, 800)
...
function InitDroneStations()
    ...
    Drone_control_station.x, Drone_control_station.y =
        Drone_control_station:getPosition()
    Drone_control_station:setCanBeDestroyed(true)
    Drone_control_station:setShieldsMax(
        Drone_control_station:getShieldMax(0) - 50
    )
...
function InitPartsStation()
    ...
    Parts_station:onTakingDamage(function()
        Player:addToShipLog(
            _("wormhole-shipLog", "Firing on invulnerable target"),
            "Yellow"
        )
        if Parts_station.warned == nil then
            Parts_station.warned = true
            Wormhole_station:sendCommsMessage(
                Player,
                _(
                    "wormhole-incCall",
                    [[Captain, Stop!
...
function InitTraffic()
    ...
    Traffic.srcdest =
        { --'J2','D0','B10','J8','zz1','zz6','zz8','D10','H11','K5'
            { x = -60000, y = 80000 }, --J2
            { x = -100000, y = -40000 }, --D0
...
function SpawnEnemies(xOrigin, yOrigin, danger, enemyFaction)
    ...
    while enemyStrength > 0 do
        ...
        if deployConfig < 50 then
            if enemyPosition > #fleetPosDelta1x then
                enemyPosition = 1
            end
            ship:setPosition(
            ...
            if enemyPosition > #fleetPosDelta2x then
                enemyPosition = 1
            end
            ship:setPosition(
...
function SpawnPatrolEnemies()
    ...
    Defence_station.patrol_enemies =
        SpawnEnemies(x - 5000, y - 5000, random(1.6, 2), "Ghosts")
...
The SW Checkpoint is under attack. Don't mess around, get down there and help them!]]
    ...
    Player:addToShipLog(
        _("defenceStnCheckpoint-shipLog", "Defend the SW checkpoint"),
        "Red"
    )
end
...
function SpawnConvoyEnemies()
    ...
    for i, enemy in ipairs(Defence_station.convoy_enemies) do
        ...
        if
            ...
            or Defence_station.drones_think_were_friendly == true
            or Defence_station.tier2_mission_state == "pre-start"
            or Defence_station.tier2_mission_state == "joinconvoy"
        then
...
function ConvoyGoAggro(__, instigator)
    ...
    for idx, e in ipairs(Defence_station.convoy_enemies) do
        if e.goneAggro ~= nil then
            return
        end -- Bail if already activated
        e.goneAggro = true
        if e:isValid() then
...
function DroneStationGoAggro(self, instigator)
    ...
    if self.spawns_left > 0 then
        ...
        local defenders =
            SpawnEnemies(x + spx, y + spy, random(0.8, 1.2), "Ghosts")
        for idx, ship in ipairs(defenders) do
            table.insert(Defence_station.ghost_defenders, ship)
        end
    end
    ...
    if not self:getCanBeDestroyed() then
        ...
        Player:addToShipLog(
            _("warning-shipLog", "This station appears immune to our attacks"),
            "Yellow"
        )
    end
...
function SpawnKWEnemies(start_aggro)
    if Difficulty >= 1 then
        ...
        local reference_visual_asteroid =
            VisualAsteroid():setPosition(369999, 589207):setSize(118)
        local x, y = sectorToXY(reference_visual_asteroid:getSectorName())
        local enemies = SpawnEnemies(x, y, random(0.8, 1.2), "Kraylor")
    ...
    if Difficulty >= 3 then
        ...
        local reference_visual_asteroid =
            VisualAsteroid():setPosition(429747, 550234):setSize(112)
        local x, y = sectorToXY(reference_visual_asteroid:getSectorName())
        local enemies = SpawnEnemies(x, y, random(0.8, 1.2), "Kraylor")
        for idx, e in ipairs(enemies) do
            table.insert(Kw_enemies, e)
    ...
    if Difficulty == 5 then
        ...
        local reference_visual_asteroid =
            VisualAsteroid():setPosition(410346, 589894):setSize(113)
        local x, y = sectorToXY(reference_visual_asteroid:getSectorName())
        local enemies = SpawnEnemies(x, y, random(0.8, 1.2), "Kraylor")
        for idx, e in ipairs(enemies) do
            table.insert(Kw_enemies, e)
...
function KWGoAggro(self, instigator)
    ...
    Player:addToShipLog(
        string.format(
            _(
                "KraylorWarning-shipLog",
                "We have lost %s reputation for breaking the treaty"
            ),
            repToLose
        ),
        "Red"
    )
    Kw_mainStation:sendCommsMessage(
        Player,
        _(
            "KraylorWarning-IncCall",
...
function SpawnMockDroneShip()
    if MockDroneShip == nil then
        ...
        MockDroneShip = CpuShip()
            :setFaction("Ghosts")
            :setTemplate("Hathcock")
            :setCallSign("DD007")
            :setPosition(sx + 1000, sy + 1000)
            :orderIdle()
        MockDroneShip:setCommsFunction(function()
            setCommsMessage(
                _("drone-comms", "Do not communicate on this channel.")
            )
        end)
...
function SpawnRepairEnemies()
    ...
    Wormhole_station.repair_enemies =
        SpawnEnemies(x - 5000, y - 5000, random(1.8, 2.2), "Kraylor")
...
Captain, a bomb has just gone off on our station and we are under attack from the Kraylor. We need your help immediately!]]
    ...
    Player:addToShipLog(
        _("wormhole-shipLog", "Defend the Wormhole Station"),
        "Red"
    )
end
...
function SpawnHarrasment()
    ...
    Defence_station.harass_enemies =
        SpawnEnemies(x + spx, y + spy, random(0.25, 0.5), "Exuari")
...
function CommsWormholeStation(comms_source, comms_target)
    ...
    if
        Wormhole_station.tier2_mission_state ~= nil
        and Wormhole_station.tier2_mission_state ~= "wait for attack"
    then
...
We're studying the energy radiating from the wormhole, and the similarities to black holes are astonishing. Please drop by if you'd like to help out.]]
        ...
        elseif
            ...
            or Wormhole_station.mission_state == "return parts"
        then
            setCommsMessage(
                _("wormhole-comms", "Please bring us the spare parts!")
            )
        elseif Wormhole_station.mission_state == "done" then
            setCommsMessage(
                _("wormhole-comms", "Thanks so much for your help!")
            )
        end
...
Our research is going well, but we could use some spare stabilizers for our Hawking Scanner. Would you mind salvaging some for us from the other side of the wormhole?]]
            ...
            addCommsReply(
                _("wormhole-comms", "Okay, we can help with that."),
                StartMissionSpareParts
            )
            addCommsReply(_("wormhole-comms", "Do you pay tips?"), function()
                setCommsMessage(
                    _("wormhole-comms", [[Maybe later, then. Goodbye.]])
                )
...
These spare parts will really help with the program budget.]]
                ...
                addCommsReply(
                    _(
                        "wormhole-comms",
                        "What do we get for being your errand boy?"
                    ),
                    function()
                        setCommsMessage(
                            _(
                                "wormhole-comms",
                                [[You get half of what would have with a smarter attitude! So rude.]]
                            )
                        )
                        Player:addReputationPoints(5)
                ...
                addCommsReply(
                    _("wormhole-comms", "We're so glad we could help"),
                    function()
                        setCommsMessage(
                            _("wormhole-comms", [[We are too. Thanks a bunch.]])
                        )
                ...
                setCommsMessage(
                    _(
                        "wormhole-comms",
                        [[We still need those spare parts from X472 on the other side of the wormhole!]]
                    )
                )
            end
        elseif Wormhole_station.mission_state == "done" then
    ...
    if not comms_source:isDocked(comms_target) then -- UNDOCKED
        if Wormhole_station.tier2_mission_state == "damaged" then
            setCommsMessage(
                _(
                    "wormhole-comms",
                    [[Come pick up our Hawking Scanner for repairs.]]
                )
            )
        elseif
            Wormhole_station.tier2_mission_state == "rma"
            or Wormhole_station.tier2_mission_state == "fixed"
        then
            setCommsMessage(
                _(
                    "wormhole-comms",
                    [[Please find someone to repair our Hawking Scanner.]]
                )
            )
        end
    else -- DOCKED
        if
            ...
            or Wormhole_station.tier2_mission_state == "rma"
        then
            Wormhole_station.tier2_mission_state = "rma"
...
We need you to find someone to repair this Hawking Scanner. Please bring it back once it's working again.]]
            ...
            addCommsReply(
                _("wormhole-comms", "Any idea where we should take it?"),
                function()
                    setCommsMessage(
                        _(
                            "wormhole-comms",
                            [[One of the nebula researchers should be able to point you in the right direction.
...
If you're not sure where to find them, ask for directions at the Admin Station]]
            ...
            addCommsReply(
                _(
                    "wormhole-comms",
                    "Have you considered shoving it where the sun don't shine?"
                ),
                function()
                    setCommsMessage(
                        _("wormhole-comms", [[What is it with you, anyways?]])
                    )
                    Player:takeReputationPoints(2)
...
Glory to you and your kin.]]
            ...
            addCommsReply(
                _("wormhole-comms", "You're very welcome."),
                function()
                    setCommsMessage(
                        _(
                            "wormhole-comms",
                            "Goodbye captain. May peace be with you."
                        )
                    )
                    FinishMissionRepair()
                end
            ...
            addCommsReply(_("wormhole-comms", "Kiss my feet!"), function()
                ...
                if CHEEVOS["nowormhole"] then
                    setCommsMessage(
                        _(
                            "wormhole-comms",
                            [[I was about to call this in to Admin and have them declare you victorious, but I've had enough of you.
...
YOU MAY BE A HERO, BUT YOU'RE ALSO A JERK!]]
                    ...
                    setCommsMessage(
                        _("wormhole-comms", [[A real peach, you are.]])
                    )
                end
                FinishMissionRepair()
            end)
...
function CommsColonyStation(comms_source, comms_target)
    ...
    if
        ...
        and Wormhole_station.tier2_mission_state == "rma"
    then
        addCommsReply(
            _("colony-comms", "We're looking for Dr. Hendrix"),
            function()
...
What can I do for you?]]
                        ...
                        addCommsReply(
                            _(
                                "colony-comms",
                                "Can you fix this Hawking Scanner?"
                            ),
                            function()
                                setCommsMessage(
                                    _(
...
I've put in a requisition for one in your name. If you can find one in a Navy storehouse, bring it to the wormhole station and they should be good to go.]]
                                ...
                                Player:addToShipLog(
                                    _("colony-shipLog", "Find a Graviton Lens"),
                                    "Green"
                                )
        ...
        addCommsReply(
            _(
                "colony-comms",
                "Some filler text and whatever. What are we even doing here?"
            ),
            function()
...
function CommsPatrolStation(comms_source, comms_target)
    setCommsMessage(
        _(
            "checkpointsStn-comms",
            "Not much here bud, just doing checkpoint kinda things, ya know?"
        )
    )
end
...
function CommsDefenceStation(comms_source, comms_target)
    ...
    elseif Defence_station.mission_state == "patrolling" then
        setCommsMessage(
            string.format(
                _(
                    "defenceStn-comms",
                    "You are on 'patrol'.\nSee that you make it to %s next."
                ),
                Defence_station.next_station
            )
        )
        return
    elseif
        Defence_station.mission_state == "drone convoy"
        and Defence_station.tier2_mission_state == nil
    then
        setCommsMessage(
            _(
                "defenceStn-comms",
                [[Your orders are to follow the convoy to see where it goes]]
            )
        )
        return
    elseif Defence_station.tier2_mission_state == "joinconvoy" then
        setCommsMessage(
            _(
                "defenceStn-comms",
                [[Your orders are to join a convoy and infiltrate the drone nests.]]
            )
        )
        return
    elseif Defence_station.tier2_mission_state == "arrived" then
        setCommsMessage(
            _(
                "defenceStn-comms",
                [[Your orders are to scan those beacons and find the defense grid controller]]
            )
        )
        return
    elseif
        Defence_station.tier2_mission_state == "done"
        and Defence_station.tier2_final_comms == nil
    then
        ...
        if DroneShip ~= nil then
            extra_string = _(
                "defenceStn-comms",
                "We are keeping the 007, and you can continue to fly it, or switch back to the Propitious 1 at any time. We'll keep it docked here at the station for you."
            )
        end
        setCommsMessage(
            string.format(
...
We need you to patrol the area. It's not exciting work, but it's got to be done. No rush, just make sure you visit each checkpoint in order, and we'll let you know if anything more pressing comes up.]]
            ...
            addCommsReply(
                _("defenceStn-comms", "Okay. We're ready to start the patrol"),
                StartMissionPatrol
            )
        end
...
We've tracked that convoy to a huge nest of drone stations in the west. The stations all form part of one proximity defense grid, and we have identified some beacons that appear to be related to its operation. We're hoping that if we can get a closer look at them, we might be able to figure out how to knock out the central control.

There is a drone ship docked here which you can use to infiltrate the nests. If you arrive with a convoy, you should automatically be granted access and we think the proximity defenses will remain disabled while you stay in the area. Of course, if you start firing, then all bets are probably off.
...
Hop in that drone ship, find the beacons, and get close enough to scan them - EASY!]]
            ...
            addCommsReply(
                _(
                    "defenceStn-comms",
                    "We're ready to infiltrate the drone nest"
                ),
                StartMissionDroneNest
            )
            addCommsReply(
                _(
                    "defenceStn-comms",
                    "(Object) Isn't that a treaty violation!?"
                ),
                function()
                    setCommsMessage(
                        _(
                            "defenceStn-comms",
                            [[(Objecting)
Sir, decoding the communications of alien defense systems is a violation of the Deepspace Militarized Citizenry Accord (DMCA).
[...]
...
However you want to do it, you need to find that central control. Get on it!]]
                    ...
                    addCommsReply(
                        _("defenceStn-comms", "Okay, we'll figure it out."),
                        StartMissionDroneNest
                    )
                end
            )
        ...
        if Defence_station.tier2_mission_state == "done" then
            setCommsMessage(
                _(
                    "defenceStn-comms",
                    [[Great job on that drone nest, Captain.
...
Remember, your primary orders are to support the research facilities in the area. See to it that they all get the assistance they need.]]
            ...
            if Player:getCallSign() == "DD007" then
                addCommsReply(
                    _("defenceStn-comms", "We'd rather crew the Propitious 1"),
                    function()
                        Player:transferPlayersToShip(PP1)
                        Player = PP1
                        setCommsMessage(
                            _("defenceStn-comms", [[Very well. Good Hunting.]])
                        )
                    end
                )
            else
                if DroneShip ~= nil then
                    addCommsReply(
                        _("defenceStn-comms", "We'd rather crew the DD007"),
                        function()
    ...
    if Wormhole_station.tier2_mission_state == "done" then
        ...
        setCommsMessage(
            string.format(
                ...
                CheevoString("treasure"),
                CheevoString("junk"),
                CheevoString("relic"),
                CheevoString("distancing"),
                CheevoString("ourship"),
                CheevoString("DMCA"),
                CheevoString("noghosts"),
                CheevoString("eyeondr"),
                CheevoString("nowormhole"),
                CheevoString("heros"),
                CheevoString("grandslam")
            )
        )
        addCommsReply(
            _("adminStnEndTips-comms", "Hints for each CHEEVO"),
            function()
                setCommsMessage(
                    _(
                        "adminStnEndTips-comms",
                        "Select the achievement for a hint"
                    )
                )
                addCommsReply(
                    _("adminStnEndTips-comms", "Experienced treasure hunter"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "The locals are known to hunt treasure in the nebula"
                            )
                        )
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "Clean up the space junk"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "So many long abandoned stations"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "Historical relic preservation"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "Destroying that old station might spook the Kraylor"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "Social distancing"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "Don't get too proximate"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "Our ship is all we need"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "We'll stick with the ship we know, thanks"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "No DMCA violations"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "Hacking alien defense systems is illegal"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "I ain't afraid of no ghosts"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "No one is afraid if there are NO GHOSTS"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "Keep an eye on that doctor"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "Do we really need to ask for directions?"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "Ban wormhole research"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "I just can't stand wormholes, or their researchers!"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "Heros for the ages"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "Their reputation is unmatched"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
                ...
                addCommsReply(
                    _("adminStnEndTips-comms", "Grand Slam"),
                    function()
                        setCommsMessage(
                            _(
                                "adminStnEndTips-comms",
                                "You can do it, but can you do it ALL?"
                            )
                        )
                        addCommsReply(_("<- Back"), CommsAdminStation)
                    end
        ...
        addCommsReply(
            _("adminStnEnd-comms", "Click here to lance the champagne!"),
            function()
    ...
    if not comms_source:isDocked(comms_target) then
        if Admin_station.mission_state == nil then
            setCommsMessage(
                _(
                    "adminStn-comms",
                    [[We are the station that co-ordinates research activities in the nearby nebulae.
...
If you're not busy, please drop by to learn more about our projects.]]
        ...
        elseif Admin_station.mission_state == "lost" then
            setCommsMessage(
                _("adminStn-comms", "Please bring our researchers home!")
            )
        elseif
            Admin_station.mission_state == "found"
            or Admin_station.mission_state == "done"
        then
            setCommsMessage(
                _(
                    "adminStn-comms",
                    "Thanks so much for finding our researchers! Dock with us if you'd like to chat."
                )
            )
        end
    else
        ...
        if Admin_station.mission_state == nil then
            setCommsMessage(
                _(
                    "adminStn-comms",
                    [[We are so glad to have you aboard. What can we help you with?]]
                )
            )
            addCommsReply(
                _(
                    "adminStn-comms",
                    "How long have you been operating in this system?"
                ),
                StartMissionLost
            )
            addCommsReply(
                _(
                    "adminStn-comms",
                    "Where is the colony where your researchers live?"
                ),
                StartMissionLost
            )
            addCommsReply(
                _("adminStn-comms", "Tell us about the nearby nebulae."),
                StartMissionLost
            )
            addCommsReply(
                _("adminStn-comms", "Are there any Kraylor in the area?"),
                StartMissionLost
            )
            addCommsReply(
                _("adminStn-comms", "Are there any Exuari in the area?"),
                StartMissionLost
            )
        elseif Admin_station.mission_state == "lost" then
            setCommsMessage(
        ...
        then
            setCommsMessage(
                _(
                    "adminStn-comms",
                    [[Always a pleasure to have you as our honored guests.]]
                )
            )
            if Wormhole_station.tier2_mission_state == "rma" then
                addCommsReply(
                    _(
                        "adminStn-comms",
                        "Any idea where we can get a Hawking Scanner fixed?"
                    ),
                    function()
                        setCommsMessage(
                            _(
                                "adminStn-comms",
                                [[Dr. Hendrix, who you rescued earlier, is the leading expert on those scanners.]]
                            )
                        )
                    end
                )
            ...
            addCommsReply(
                ...
                function()
                    setCommsMessage(
                        _(
                            "adminStn-comms",
                            [[We have been operating here for about 50 years]]
                        )
                    )
                    addCommsReply(_("<- Back"), CommsAdminStation)
                end
            ...
            addCommsReply(
                ...
                function()
                    setCommsMessage(
                        _(
                            "adminStn-comms",
                            [[You'll find it to the east of the nebulae]]
                        )
                    )
                    addCommsReply(_("<- Back"), CommsAdminStation)
                end
            ...
            addCommsReply(
                ...
                function()
                    setCommsMessage(
                        _(
                            "adminStn-comms",
                            [[Well I suppose you already know about the nebulae. They are to the north.
            ...
            addCommsReply(
                ...
                function()
                    setCommsMessage(
                        _(
                            "adminStn-comms",
                            [[We see some from time to time, but with the non-aggression treaty, they haven't been too much trouble.]]
                        )
                    )
                    addCommsReply(_("<- Back"), CommsAdminStation)
                end
            ...
            addCommsReply(
                ...
                function()
                    setCommsMessage(
                        _(
                            "adminStn-comms",
                            [[Old space junk, mostly. We haven't seen any real Exauri activity in decades.]]
                        )
                    )
                    addCommsReply(_("<- Back"), CommsAdminStation)
                end
...
function CommsFoundShip(comms_source, comms_target)
    if Admin_station.mission_state == "almost found" then
        setCommsMessage(
            _(
                "found-comms",
                [[Come in? Dr. Hendrix here... Come in?
...
We need to install a new hyperplasma full bridge rectifier, but our plasma conduit driver has gone missing. You must have one in your engine room? If you let us borrow it, we will upgrade your sensors with our new experimental modifications.]]
        ...
        addCommsReply(
            _("found-comms", "Sure, you can borrow our conduit driver!"),
            function()
                setCommsMessage(
                    _(
                        "found-comms",
                        [[Great. Your sensors are upgraded. We'll get this rectifier installed and be on our way.
...
function CommsInvestigator(comms_source, comms_target)
    setCommsMessage(
        _("investigate-comms", "What do you have to say for yourself?")
    )
    addCommsReply(
        _("investigate-comms", "We're just running a salvage mission."),
        function()
            setCommsMessage(
                _(
                    "investigate-comms",
                    "Carry on, then, we're keeping our eye on you."
                )
            )
        end
    )
    addCommsReply(
        _("investigate-comms", "Back off, this is a special military operation"),
        function()
            setCommsMessage(
                _(
                    "investigate-comms",
                    "You want to back up that escalation with some weapons fire?"
                )
            )
        end
    )
...
function CommsDroneStation(comms_source, comms_target)
    if not comms_source:isDocked(comms_target) then
        setCommsMessage(
            _(
                "drone-comms",
                [[We oversee the B-3 liquidation of the Ghosts assets.
...
Alright, enjoy... focusing your gravitons, I guess!]]
                ...
                setCommsMessage(
                    _("drone-comms", "For focusing the gravitational field.")
                )
                addCommsReply(_("<- Back"), CommsDroneStation)
            end
        end)
        addCommsReply(_("drone-comms", "Holographic Projector"), function()
            setCommsMessage(
                _("drone-comms", "Shows very convincing drone stations")
            )
            addCommsReply(_("<- Back"), CommsDroneStation)
        end)
    end
...
function CommsBeingAttacked(self, instigator)
    if
        self.lastAttackedComms == nil
        or ((getScenarioTime() - self.lastAttackedComms) > 300)
    then
        self:sendCommsMessage(
            Player,
            string.format(
                _(
                    "attack-incCall",
                    "HELP!\nWe are taking damage from %s and might need assistance."
                ),
                instigator:getCallSign()
            )
        )
        self.lastAttackedComms = getScenarioTime()
    end
    if
        self.lastAttackedLog == nil
        or ((getScenarioTime() - self.lastAttackedLog) > 30)
    then
        Player:addToShipLog(
            string.format(
                _("attack-shipLog", "%s is taking damage!"),
                self:getCallSign()
            ),
            "RED"
        )
...
It seems one of our researchers, Dr. Hendrix, and her crew have lost their way in a nearby nebula and need our help. They were running experiments in the nebula when their engines and transponder went offline. Please follow our scout vessel, SR7, to the nebula and search for them.]]
    ...
    local reference_visual_asteroid =
        VisualAsteroid():setPosition(29793, -29765):setSize(116)
    Admin_station.assist_ship = CpuShip()
        :setCallSign("SR7")
        :setPosition(Admin_station.x + 200, Admin_station.y)
        :orderFlyTowardsBlind(
            sectorToXY(reference_visual_asteroid:getSectorName())
        )
        :setFaction("Human Navy")
        ...
        :setCommsFunction(function()
            if
                ...
                or Admin_station.assist_ship.state == "waiting"
            then
                setCommsMessage(
                    _("adminStn-comms", "Meet me at the edge of the nebula")
                )
            end
...
function UpdateMissionLost()
    if
        Admin_station.mission_state == nil
        or Admin_station.mission_state == "done"
    then
        return
    end
    ...
    if
        Admin_station.assist_ship.state == "waiting"
        and distance(Admin_station.assist_ship, Player) <= 20000
    then
        Admin_station.assist_ship:sendCommsMessage(
            Player,
            _(
                "adminStn-incCall",
                [[Ready to head in?
...
I'll follow you. Let's find Dr. Hendrix and bring her crew home.]]
        ...
        Player:addToShipLog(
            _("adminStn-shipLog", "Fly into the Nebula and find Dr. Hendrix"),
            "Green"
        )
    ...
    if Admin_station.mission_state == "lost" then
        if
            distance(
                Player,
                Admin_station.lost_location_x,
                Admin_station.lost_location_y
            ) <= 20000
        then -- Must be 20U from lost ship to find it
            Admin_station.found_ship = CpuShip()
                :setFaction("Human Navy")
                :onTakingDamage(CommsBeingAttacked)
                :setTemplate("Equipment Freighter 2")
                :setWarpDrive(true)
    ...
    if
        Admin_station.mission_state == "found"
        and Admin_station.found_ship:isDocked(Admin_station.hendrix_station)
    then
        Admin_station.mission_state = "done"
...
function FinishMissionLost()
    ...
    Player:addToShipLog(
        _("finishMissionNebulae-shipLog", "Sensors have been upgraded."),
        "Green"
    )
    Player:addReputationPoints(5)

    -- Send to a random colony station
    Admin_station.hendrix_station =
        Colony_stations[irandom(1, #Colony_stations)]
    ...
    if
        Admin_station.assist_ship ~= nil and Admin_station.assist_ship:isValid()
    then
...
Start at the checkpoint in the North West. You'll need to dock at each station before proceeding to the next.]]
    ...
    Defence_station.next_station =
        Patrol_stations[Defence_station.patrol_index]:getCallSign()
...
function UpdateMissionPatrol()
    ...
    if
        Defence_station.mission_state == nil
        or Defence_station.mission_state == "done"
    then
        return
    end
    ...
    if Defence_station.mission_state == "patrolling" then
        ...
        if Player:isDocked(Patrol_stations[Defence_station.patrol_index]) then
            ...
            if Defence_station.patrol_index > #Patrol_stations then
                Defence_station.patrol_index = Defence_station.patrol_index
                    - #Patrol_stations
            ...
            if
                Defence_station.patrol_second_round ~= nil
                and Defence_station.patrol_index == 1
            then
                Defence_station.mission_state = "patrol attack"
                SpawnPatrolEnemies()
            else
                Defence_station:sendCommsMessage(
                    Player,
                    string.format(
                        _("defenceStn-incCall", "Please proceed to %s"),
                        Defence_station.next_station
                    )
                )
                Player:addToShipLog(
                    string.format(
                        _("defenceStn-shipLog", "Please proceed to %s"),
                        Defence_station.next_station
                    ),
                    "Green"
                )
...
INVESTIGATE BUT DO NOT ENGAGE! We have it on good word that attacking this convoy is extremely risky. Keep your distance, but we'd like to know where they are going.]]
            ...
            Player:addToShipLog(
                _(
                    "defenceStn-shipLog",
                    "Investigate the convoy, but DO NOT ENGAGE"
                ),
                "Red"
            )
...
function CheckConvoyArrived()
    for idx, enemy in ipairs(Defence_station.convoy_enemies) do
        ...
        if enemy:isValid() then
            if
                distance(enemy, enemy.dx, enemy.dy) < 1000
                and not enemy.goneAggro
            then
                enemy:destroy()
            end
        else
            if enemy.arrived == nil then
                ...
                Defence_station.convites_arrived = Defence_station.convites_arrived
                    + 1
            end
        end
    end
    ...
    if
        Defence_station.convites_arrived
            > (#Defence_station.convoy_enemies / 2)
        and Defence_station.tier2_mission_state == nil
    then
        ...
        Player:addToShipLog(
            _(
                "defenceStn-shipLog",
                "Dock with the Defence Station to start another mission"
            ),
            "Green"
        )
        SpawnMockDroneShip()
    ...
    if Defence_station.convites_arrived == #Defence_station.convoy_enemies then
        ...
        if
            ...
            or Defence_station.tier2_mission_state == "arrived"
        then
            SpawnConvoyEnemies()
        end
    end
...
function StartMissionSpareParts()
    ...
    elseif Difficulty == 1 then
        lastpart = _(
            "wormhole-comms",
            "and we've given you some extra missiles to help get the job done."
        )
        Player:setWeaponStorage(
            "Homing",
            6 - missiles + Player:getWeaponStorage("Homing")
        )
    elseif Difficulty == 3 then
        lastpart = _(
            "wormhole-comms",
            "and go find some extra missiles to help get the job done."
        )
    end
...
We are sending a team to investigate what you are up to. If you fire on us, there will be trouble!]]
            ...
            Player:addToShipLog(
                _(
                    "KraylorWarning-shipLog",
                    "The Kraylor are investigating our activities and have warned us not to escalate"
                ),
                "Yellow"
            )
        ...
        SupplyDrop()
            :setFaction("Human Navy")
            :setPosition(Parts_station:getPosition())
            :setEnergy(200)
            :onPickUp(function()
                Wormhole_station:sendCommsMessage(
                    Player,
...
You'd better hurry back, we've seen some Kraylor activity in the area. Hopefully your salvage mission didn't attract the wrong sort of attention.]]
                ...
                Player:addToShipLog(
                    _(
                        "wormhole-shipLog",
                        "Return the spare parts to the Wormhole Station"
                    ),
                    "Green"
                )
    ...
    Parts_station:onTakingDamage(function()
        ...
        Player:addToShipLog(
            _("wormhole-shipLog", "Jump Defence Activated"),
            "Yellow"
        )
    end)
end
...
function UpdateMissionSpareParts()
    if
        Wormhole_station.mission_state == nil
        or Wormhole_station.mission_state == "done"
    then
        return
    end
    ...
    if Wormhole_station.mission_state == "return parts" then
        ...
        if Investigator.warned == nil and Investigator:isValid() then
            ...
            if
                distance(Player, Wormhole_station) < 5000
                and Player.hasSpareParts == true
            then
                Investigator.warned = true
                Investigator:sendCommsMessage(
                    Player,
                    _(
                        "KraylorWarning-incCall",
                        [[Watch your back, Hugh Mon!
...
<The Kraylor Vessel jumps back through the wormhole>]]
                ...
                Player:addToShipLog(
                    _(
                        "KraylorWarning-shipLog",
                        "The Kraylor seem hostile and have warned us to keep our distance"
                    ),
                    "Yellow"
                )
...
function UpdateMissionDroneNest()
    if
        Defence_station.tier2_mission_state == nil
        or Defence_station.tier2_mission_state == "done"
    then
        return
    end
    ...
    if Defence_station.tier2_mission_state == "joinconvoy" then
        if
            (getScenarioTime() - Defence_station.convoy_timer >= 60)
            and #Defence_station.convoy_enemies == 0
        then
            SpawnConvoyEnemies()
        end
        ...
        local reference_visual_asteroid =
            VisualAsteroid():setPosition(-108533, 31942):setSize(114)
        ...
        if
            ...
            and (py - gy > -2500)
            and (py - gy < 22500)
        then -- Inside nest area
            if
                with_convoy
                and Player:getFaction() == "Ghosts"
                and not Defence_station.cover_blown
            then
                ...
                Defence_station:sendCommsMessage(
                    ...
                    _(
                        "defenceStn-incCall",
                        [[Arrived, scan the security beacons]]
                    )
                )
            else
                if not Defence_station.cover_blown then
                    ...
                    Defence_station:sendCommsMessage(
                        ...
                        _(
                            "defenceStn-incCall",
                            [[Your cover has been blown and the proximity defense system remains active. We suggest leaving and trying again to arrive with a future convoy.]]
                        )
                    )
                    Player:addToShipLog(
                        _("defenceStn-shipLog", "Our cover has been blown"),
                        "Red"
                    )
                end
            end
            ...
            if Defence_station.cover_blown then
                Player:addToShipLog(
                    _("defenceStn-shipLog", "We should be good to try again"),
                    "Green"
                )
...
function FinishMissionDroneNest()
    ...
    Player:setFaction("Human Navy")
    Liquidation_station = SpaceStation()
        :setTemplate("Small Station")
        :setFaction("Human Navy")
        :onTakingDamage(CommsBeingAttacked)
        :setCallSign("DB-3")
        :setPosition(Drone_control_station.x, Drone_control_station.y)
        :setCommsFunction(CommsDroneStation)
    Liquidation_station:sendCommsMessage(
        Player,
        _(
            "drone-incCall",
            [[Great work, Captain!
...
We have taken control of this station and will be liquidating spare parts from the wreckage.]]
    ...
    Player:addToShipLog(
        _("drone-shipLog", "DB-3 has a collection of spare parts"),
        "Green"
    )
    ...
    if
        Admin_station.mission_state == "done"
        and Wormhole_station.mission_state == "done"
        and Wormhole_station.tier2_attack_countdown > 120
    then
...
function StartMissionRepair()
    ...
    if
        Admin_station.mission_state == "done"
        and Wormhole_station.mission_state == "done"
        and Defence_station.tier2_mission_state == "done"
    then
        Wormhole_station.tier2_attack_countdown = 120
    else
...
function UpdateMissionRepair(delta)
    ...
    if Wormhole_station.tier2_mission_state == "wait for attack" then
        Wormhole_station.tier2_attack_countdown = Wormhole_station.tier2_attack_countdown
            - delta
...
Please dock with us to pick up the scanner.]]
            ...
            Player:addToShipLog(
                _(
                    "wormhole-shipLog",
                    "Get the Hawking Scanner from the Wormhole Station"
                ),
                "Green"
            )
        end
    ...
    if
        ...
        and Player:getDockingState() == 0
        and Wormhole_station.victory_comms == nil
    then
...
function TransferToDrone()
    ...
    Player:addToShipLog(
        _(
            "drone-shipLog",
            "This ship will not activate the drone station proximity defenses as long as you arrive with a convoy."
        ),
        "Yellow"
    )
end
...
function TransferToPP1()
    Player:addToShipLog(
        _("drone-shipLog", "It's nice to be back aboard the Propitious 1"),
        "Yellow"
    )
    Player:transferPlayersToShip(PP1)
end
...
function UpdateDroneStations()
    ...
    if Defence_station.drones_think_were_friendly ~= true then
        for idx, stn in ipairs(Drone_stations) do
            if stn:isValid() then
                if distance(Player, stn) > 80000 then
                    return
                end -- Exit early (don't iterate all stations) if player is far
                if distance(Player, stn) < 2000 and stn.prox_spawned == nil then
                    ...
                    local defenders =
                        SpawnEnemies(x_d, y_d, random(0.1, 0.3), "Ghosts")
                    for idx2, d in ipairs(defenders) do
                        d:orderAttack(Player)
                        table.insert(Defence_station.ghost_defenders, d)
                    end
                    stn:sendCommsMessage(
                        Player,
                        _(
                            "drone-incCall",
                            [[PROXIMITY DEFENCE SYSTEM ACTIVATED]]
                        )
                    )
                    Player:addToShipLog(
                        _(
                            "drone-shipLog",
                            "We got too close to a drone station and it launched defenses."
                        ),
                        "Red"
                    )
    ...
    if Defence_station.tier2_mission_state ~= nil then
        if Defence_station.all_beacons_scanned ~= true then
            local scanned = 0
            local desc = _(
                "scienceDescription-beacons",
                "The Ghosts drone security beacons. If we scan them all, we should be able to find the control station. We'll need to get pretty close."
            )
                    ...
                    if a.close_latch ~= true then
                        ...
                        a
                            :setDescriptions(
                                ...
                                _(
                                    "scienceDescription-beacons",
                                    "The beamforming configuration from this beacon will help the boffins triangulate the location of the central control."
                                )
                            )
                            :setScanningParameters(1, 1)
                    end
            ...
            if scanned >= #Drone_artifacts then
                ...
                Player:addToShipLog(
                    string.format(
                        _(
                            "defenceStn-shipLog",
                            "The drone control station is %s"
                        ),
                        Drone_control_station:getCallSign()
                    ),
                    "Green"
...
function UpdateHarassment()
    ...
    if
        ...
        and getScenarioTime() - Defence_station.harass_timer > 300
    then
        Defence_station.harass_timer = nil
        SpawnHarrasment()
    end
...
function UpdateTraffic()
    ...
    if
        getScenarioTime() - Traffic.timer < 30
        or (
                #Traffic.new_ships
                + #Traffic.docked_ships
                + #Traffic.leaving_ships
    ...
    local new_ship = CpuShip()
        :setFaction(faction)
        :setTemplate(type)
        :setPosition(src.x, src.y)
        :orderDock(station)
    if faction == "TSN" then
        new_ship:setCommsFunction(function()
            setCommsMessage(
                _("newShip-comms", "Too busy to talk today, Captain.")
            )
        end)
    end
...
function HendrixHints(stn)
    ...
    if Difficulty ~= 1 and irandom(1, 13 - Difficulty) == 1 then -- Hints every time on easy, but unhelpful 1/10 on med and 1/8 on hard
        stn.hendrix_hint = _(
            "hendrixHints-comms",
            "Sorry, I don't know where Dr. Hendrix lives"
        )
        return
    end
    ...
    if char == 1 then
        stn.hendrix_hint = string.format(
            _(
                "hendrixHints-comms",
                "Dr. Hendrix, of course! I'm pretty sure she was on a %s-class station, but darned if I can remember which one."
            ),
            hint
        )
    else
        stn.hendrix_hint = string.format(
            _(
                "hendrixHints-comms",
                "Gosh, Dr. Hendrix. Okay. Sorry, but honestly all I remember is that her address definitely has a '%s' in it."
            ),
            hint
        )
    end
...
function CheckDefeatConditions()
    for idx, stn in ipairs(Patrol_stations) do
        if not stn:isValid() then
            Player:addToShipLog(
                _(
                    "defeat-shipLog",
                    "DEFEAT - A Patrol Station has been destroyed"
                ),
                "Red"
            )
            victory("Exuari")
    ...
    for idx, stn in ipairs({ Admin_station, Defence_station, Wormhole_station }) do
        if not stn:isValid() then
            Player:addToShipLog(
                _(
                    "defeat-shipLog",
                    "DEFEAT - A Core station (Admin, Defence, Wormhole) has been destroyed"
                ),
                "Red"
            )
            victory("Exuari")
    ...
    if Defence_station.tier2_mission_state == "done" then
        if not Liquidation_station:isValid() then
            Player:addToShipLog(
                _("defeat-shipLog", "DEFEAT - Station B-3 has been destroyed"),
                "Red"
            )
            victory("Exuari")
        end
...
function CheckCheevoTreasure()
    ...
    for name, bonus in pairs(Admin_station.nebula_bonuses) do
        if
            bonus.wasFound ~= true
            and (name == "nuke" or name == "repair" or name == "rep")
        then
