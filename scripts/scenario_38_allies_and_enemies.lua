...
function init()
    ...
    print(
        string.format(
            "    ----    Scenario: Allies and Enemies    ----    Version %s    ----    Tested with EE version %s    ----",
            scenario_version,
            ee_version
        )
    )
...
function constructEnvironment()
    ...
    repeat
        ...
        buildStationsPlus()
        if
            #kraylorStationList < 5
            or #humanStationList < 5
            or #exuariStationList < 5
            or #arlenianStationList < 5
        then
            resetStationsPlus()
    ...
    until (
            #kraylorStationList >= 5
            and #humanStationList >= 5
            and #exuariStationList >= 5
            and #arlenianStationList >= 5
        ) or universeCreateRetryCount > repeatExitBoundary
    if universeCreateRetryCount > 0 then
        if diagnostic then
            print("universe create retry count: " .. universeCreateRetryCount)
        end
    end
...
function setGossipSnippets()
    ...
    table.insert(
        ...
        _(
            "gossip-comms",
            "My mining friends tell me Krak or Kruk is about to strike it rich"
        )
    ) --2
    table.insert(
        gossipSnippets,
        _(
            "gossip-comms",
            "Did you know you can usually hire replacement repair crew cheaper at friendly stations?"
        )
    ) --3
    table.insert(
        gossipSnippets,
        _(
            "gossip-comms",
            "Under their uniforms, the Kraylors have an extra appendage. I wonder what they use it for"
        )
    ) --4
    table.insert(
        gossipSnippets,
        _(
            "gossip-comms",
            "The Kraylors may be human navy enemies, but they make some mighty fine BBQ Mynock"
        )
    ) --5
    table.insert(
        gossipSnippets,
        _(
            "gossip-comms",
            "The Kraylors and the Ktlitans may be nearing a cease fire from what I hear. That'd be bad news for us"
        )
    ) --6
    table.insert(
        gossipSnippets,
        _(
            "gossip-comms",
            "Docking bay 7 has interesting mind altering substances for sale, but they're monitored between 1900 and 2300"
        )
    ) --7
    table.insert(
        gossipSnippets,
    ...
    table.insert(
        ...
        _(
            "gossip-comms",
            "I think the shuttle pilot has a tame miniature Ktlitan caged in his quarters. Sometimes I hear it at night"
        )
    ) --9
    table.insert(
        gossipSnippets,
        _(
            "gossip-comms",
            "Did you hear the screaming chase in the corridors on level 4 last night? Three Kraylors were captured and put in the brig"
        )
    ) --10
    table.insert(
        gossipSnippets,
        _(
            "gossip-comms",
            "Rumor has it that the two Lichten brothers are on the verge of a new discovery. And it's not another wine flavor either"
        )
    ) --11
end
...
function setTerritoryZones()
    ...
    borderStartX, borderStartY =
        vectorFromAngle(borderStartAngle, random(3500, 4900))
    ...
    local hlx, hly = vectorFromAngle(
        borderStartAngle + 270 - ba,
        random(firstLowerLength, firstUpperLength)
    )
    table.insert(humanLeftX, sharedZoneBorderLeftX + hlx)
    table.insert(humanLeftY, sharedZoneBorderLeftY + hly)
    ba = bendAngle(0)
    hlx, hly = vectorFromAngle(
        borderStartAngle + 90 + ba,
        random(firstLowerLength, firstUpperLength)
    )
    ...
    for i = 1, 20 do
        ...
        hlx, hly = vectorFromAngle(
            borderStartAngle + 270 - ba,
            random(lowerLength, upperLength)
        )
    ...
    humanZone = Zone():setPoints(
        ...
        humanLeftX[1],
        humanLeftY[1],
        sharedZoneBorderLeftX,
        sharedZoneBorderLeftY,
        sharedZoneBorderRightX,
        sharedZoneBorderRightY,
        humanRightX[1],
        humanRightY[1],
        humanRightX[2],
        humanRightY[2],
    ...
    humanZone
        :setColor(0, 54, 0)
        :setLabel(_("ZoneLabelDescription-faction", "Human"))
    arlenianZone
        :setColor(0, 0, 54)
        :setLabel(_("ZoneLabelDescription-faction", "Arlenian"))
    kraylorZone
        :setColor(54, 0, 0)
        :setLabel(_("ZoneLabelDescription-faction", "Kraylor"))
    exuariZone
        :setColor(54, 0, 0)
        :setLabel(_("ZoneLabelDescription-faction", "Exuari"))
end
...
function buildStationsPlus()
    ...
    repeat
        ...
        psx = (gRegion[sri][1] - (gbHigh / 2)) * gSize
            + random(-gSize / 2 * 0.95, gSize / 2 * 0.95) --place station x coordinate
        psy = (gRegion[sri][2] - (gbHigh / 2)) * gSize
            + random(-gSize / 2 * 0.95, gSize / 2 * 0.95) --place station y coordinate
        ...
        gx = adjList[rn][1]
        gy = adjList[rn][2]
    until not neutralStationsRemain
        or not humanStationsRemain
        or not kraylorStationsRemain
        or not exuariStationsRemain
    local oobCount = 0
    local humanOob = 0
    local kraylorOob = 0
    local exuariOob = 0
    ...
    if oobCount > 0 then
        if diagnostic then
            print(
                string.format(
                    "OOB: %i, Human: %i, Kraylor: %i, Exuari: %i, Arlenian: %i, Neutral: %i",
                    oobCount,
                    humanOob,
                    kraylorOob,
                    exuariOob,
    ...
    if diagnostic then
        print(
            string.format(
                "Human stations: %i, Kraylor stations: %i, Exuari stations: %i, Arlenian stations: %i, Neutral stations: %i",
                #humanStationList,
                #kraylorStationList,
                #exuariStationList,
                #arlenianStationList,
    ...
    if not diagnostic then
        ...
        local nebula_list =
            placeRandomListAroundPoint(Nebula, nebula_count, 1, 150000, 0, 0)
...
function placeAlcaleica()
    --Alcaleica
    stationAlcaleica = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationAlcaleica
        :setPosition(psx, psy)
        :setCallSign("Alcaleica")
        :setDescription(_("scienceDescription-station", "Optical Components"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationAlcaleica] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "optic", 5, 66 },
            }
        else
            goods[stationAlcaleica] =
                { { "food", math.random(5, 10), 1 }, { "optic", 5, 66 } }
    ...
    stationAlcaleica.generalInformation = _(
        "stationGeneralInfo-comms",
        "We make and supply optic components for various station and ship systems"
    )
    stationAlcaleica.stationHistory = _(
        "stationStory-comms",
        "This station continues the businesses from Earth based on the merging of several companies including Leica from Switzerland, the lens manufacturer and the Japanese advanced low carbon electronic and optic company"
    )
    return stationAlcaleica
end
function placeAnderson()
    --Anderson
    stationAnderson = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationAnderson
        :setPosition(psx, psy)
        :setCallSign("Anderson")
        :setDescription(
            _("scienceDescription-station", "Battery and software engineering")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationAnderson] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "battery", 5, 65 },
                { "software", 5, 115 },
        ...
        goods[stationAnderson] =
            { { "battery", 5, 65 }, { "software", 5, 115 } }
    end
    tradeLuxury[stationAnderson] = true
    stationAnderson.publicRelations = true
    stationAnderson.generalInformation = _(
        "stationGeneralInfo-comms",
        "We provide high quality high capacity batteries and specialized software for all shipboard systems"
    )
    stationAnderson.stationHistory = _(
        "stationStory-comms",
        "The station is named after a fictional software engineer in a late 20th century movie depicting humanity unknowingly conquered by aliens and kept docile by software generated illusion"
    )
    return stationAnderson
end
function placeArcher()
    --Archer
    stationArcher = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationArcher:setPosition(psx, psy):setCallSign("Archer"):setDescription(
        _("scienceDescription-station", "Shield and Armor Research")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationArcher] = {
            ...
            goods[stationArcher] =
                { { "food", math.random(5, 10), 1 }, { "shield", 5, 90 } }
    ...
    stationArcher.generalInformation = _(
        "stationGeneralInfo-comms",
        "The finest shield and armor manufacturer in the quadrant"
    )
    stationArcher.stationHistory = _(
        "stationStory-comms",
        "We named this station for the pioneering spirit of the 22nd century Starfleet explorer, Captain Jonathan Archer"
    )
    return stationArcher
end
function placeArchimedes()
    --Archimedes
    stationArchimedes = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationArchimedes] =
                { { "food", math.random(5, 10), 1 }, { "beam", 5, 80 } }
    ...
    stationArchimedes.generalInformation = _(
        "stationGeneralInfo-comms",
        "We fabricate general and specialized components for ship beam systems"
    )
    stationArchimedes.stationHistory = _(
        "stationStory-comms",
        "This station was named after Archimedes who, according to legend, used a series of adjustable focal length mirrors to focus sunlight on a Roman naval fleet invading Syracuse, setting fire to it"
    )
    return stationArchimedes
end
function placeArmstrong()
    --Armstrong
    stationArmstrong = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationArmstrong] =
                { { "food", math.random(5, 10), 1 }, { "repulsor", 5, 62 } }
        end
    else
    ...
    stationArmstrong.generalInformation = _(
        "stationGeneralInfo-comms",
    ...
    stationArmstrong.stationHistory = _(
        "stationStory-comms",
        "The station is named after the late 19th century astronaut as well as the fictionlized stations that followed. The station initially constructed entire space worthy vessels. In time, it transitioned into specializeing in propulsion systems."
    )
    return stationArmstrong
end
function placeAsimov()
    --Asimov
    stationAsimov = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationAsimov
        :setCallSign("Asimov")
        :setDescription(
            _("scienceDescription-station", "Training and Coordination")
        )
        :setPosition(psx, psy)
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationAsimov] =
                { { "food", math.random(5, 10), 1 }, { "tractor", 5, 48 } }
        end
    else
        goods[stationAsimov] = { { "tractor", 5, 48 } }
    end
    stationAsimov.publicRelations = true
    stationAsimov.generalInformation = _(
        "stationGeneralInfo-comms",
    ...
    stationAsimov.stationHistory = _(
        "stationStory-comms",
...
function placeBarclay()
    ...
    stationBarclay
        ...
        :setCallSign("Barclay")
        :setDescription(
            _("scienceDescription-station", "Communication components")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationBarclay] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "communication", 5, 58 },
            }
    ...
    stationBarclay.generalInformation = _(
        "stationGeneralInfo-comms",
        "We provide a range of communication equipment and software for use aboard ships"
    )
    stationBarclay.stationHistory = _(
        "stationStory-comms",
        "The station is named after Reginald Barclay who established the first transgalactic com link through the creative application of a quantum singularity. Station personnel often refer to the station as the Broccoli station"
    )
    return stationBarclay
end
function placeBethesda()
    --Bethesda
    stationBethesda = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationBethesda
        :setPosition(psx, psy)
        :setCallSign("Bethesda")
        :setDescription(_("scienceDescription-station", "Medical research"))
    goods[stationBethesda] = {
        { "food", math.random(5, 10), 1 },
        { "medicine", 5, 5 },
        { "autodoc", 5, 36 },
    }
    stationBethesda.publicRelations = true
    stationBethesda.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and treat exotic medical conditions"
    )
    stationBethesda.stationHistory = _(
        "stationStory-comms",
...
function placeBroeck()
    ...
    stationBroeck
        ...
        :setCallSign("Broeck")
        :setDescription(
            _("scienceDescription-station", "Warp drive components")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationBroeck] = {
            ...
            if random(1, 100) < 62 then
                tradeLuxury[stationBroeck] = true
            end
        else
            goods[stationBroeck] =
                { { "food", math.random(5, 10), 1 }, { "warp", 5, 130 } }
            if random(1, 100) < 53 then
                tradeMedicine[stationBroeck] = true
            end
            if random(1, 100) < 62 then
                tradeLuxury[stationBroeck] = true
            end
    ...
    stationBroeck.generalInformation = _(
        "stationGeneralInfo-comms",
        "We provide warp drive engines and components"
    )
    stationBroeck.stationHistory = _(
        "stationStory-comms",
        "This station is named after Chris Van Den Broeck who did some initial research into the possibility of warp drive in the late 20th century on Earth"
    )
    return stationBroeck
end
function placeCalifornia()
    --California
    stationCalifornia = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
...
function placeCalvin()
    ...
    stationCalvin
        :setPosition(psx, psy)
        :setCallSign("Calvin")
        :setDescription(_("scienceDescription-station", "Robotic research"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationCalvin] = {
                { "food", math.random(5, 10), 1 },
            ...
            goods[stationCalvin] =
                { { "food", math.random(5, 10), 1 }, { "robotic", 5, 87 } }
        end
    else
        goods[stationCalvin] = { { "robotic", 5, 87 } }
        if random(1, 100) < 8 then
            tradeFood[stationCalvin] = true
        end
    end
    tradeLuxury[stationCalvin] = true
    stationCalvin.publicRelations = true
    stationCalvin.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and provide robotic systems and components"
    )
    stationCalvin.stationHistory = _(
        "stationStory-comms",
        "This station is named after Dr. Susan Calvin who pioneered robotic behavioral research and programming"
    )
    return stationCalvin
end
function placeCavor()
    --Cavor
    stationCavor = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationCavor:setPosition(psx, psy):setCallSign("Cavor"):setDescription(
        _("scienceDescription-station", "Advanced Material components")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationCavor] = {
            ...
            goods[stationCavor] =
                { { "food", math.random(5, 10), 1 }, { "filament", 5, 42 } }
    ...
    stationCavor.generalInformation = _(
        "stationGeneralInfo-comms",
        "We fabricate several different kinds of materials critical to various space industries like ship building, station construction and mineral extraction"
    )
    stationCavor.stationHistory = _(
        "stationStory-comms",
        "We named our station after Dr. Cavor, the physicist that invented a barrier material for gravity waves - Cavorite"
    )
    return stationCavor
end
function placeChatuchak()
    --Chatuchak
    stationChatuchak = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationChatuchak
        :setPosition(psx, psy)
        :setCallSign("Chatuchak")
        :setDescription(_("scienceDescription-station", "Trading station"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationChatuchak] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "luxury", 5, 60 },
            }
...
function placeCoulomb()
    --Coulomb
    stationCoulomb = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationCoulomb:setPosition(psx, psy):setCallSign("Coulomb"):setDescription(
        _("scienceDescription-station", "Shielded circuitry fabrication")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationCoulomb] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "circuit", 5, 50 },
            }
            ...
            goods[stationCoulomb] =
                { { "food", math.random(5, 10), 1 }, { "circuit", 5, 50 } }
            if random(1, 100) < 27 then
                tradeMedicine[stationCoulomb] = true
            end
        end
    else
        ...
        if random(1, 100) < 16 then
            tradeFood[stationCoulomb] = true
        end
    end
    if random(1, 100) < 82 then
        tradeLuxury[stationCoulomb] = true
    end
    stationCoulomb.publicRelations = true
    stationCoulomb.generalInformation = _(
        "stationGeneralInfo-comms",
    ...
    stationCoulomb.stationHistory = _(
        "stationStory-comms",
...
function placeCyrus()
    --Cyrus
    stationCyrus = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationCyrus:setPosition(psx, psy):setCallSign("Cyrus"):setDescription(
        _("scienceDescription-station", "Impulse engine components")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationCyrus] = {
            ...
            goods[stationCyrus] =
                { { "food", math.random(5, 10), 1 }, { "impulse", 5, 124 } }
            if random(1, 100) < 34 then
                tradeMedicine[stationCyrus] = true
            end
        end
    else
        ...
        if random(1, 100) < 13 then
            tradeFood[stationCyrus] = true
        end
    end
    if random(1, 100) < 78 then
        tradeLuxury[stationCyrus] = true
    end
    stationCyrus.publicRelations = true
    stationCyrus.generalInformation = _(
        "stationGeneralInfo-comms",
        "We supply high quality impulse engines and parts for use aboard ships"
    )
    stationCyrus.stationHistory = _(
        "stationStory-comms",
        "This station was named after the fictional engineer, Cyrus Smith created by 19th century author Jules Verne"
    )
    return stationCyrus
end
function placeDeckard()
    --Deckard
    stationDeckard = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationDeckard
        :setPosition(psx, psy)
        :setCallSign("Deckard")
        :setDescription(_("scienceDescription-station", "Android components"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationDeckard] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "android", 5, 73 },
            }
            ...
            goods[stationDeckard] =
                { { "food", math.random(5, 10), 1 }, { "android", 5, 73 } }
        end
    else
    ...
    stationDeckard.generalInformation = _(
        "stationGeneralInfo-comms",
        "Supplier of android components, programming and service"
    )
    stationDeckard.stationHistory = _(
        "stationStory-comms",
        "Named for Richard Deckard who inspired many of the sophisticated safety security algorithms now required for all androids"
    )
    return stationDeckard
end
function placeDeer()
    --Deer
    stationDeer = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationDeer:setPosition(psx, psy):setCallSign("Deer"):setDescription(
        _("scienceDescription-station", "Repulsor and Tractor Beam Components")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationDeer] = {
...
function placeErickson()
    ...
    stationErickson
        :setPosition(psx, psy)
        :setCallSign("Erickson")
        :setDescription(
            _("scienceDescription-station", "Transporter components")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationErickson] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "transporter", 5, 63 },
            }
            ...
            goods[stationErickson] =
                { { "food", math.random(5, 10), 1 }, { "transporter", 5, 63 } }
    ...
    stationErickson.generalInformation = _(
        "stationGeneralInfo-comms",
        "We provide transporters used aboard ships as well as the components for repair and maintenance"
    )
    stationErickson.stationHistory = _(
        "stationStory-comms",
        "The station is named after the early 22nd century inventor of the transporter, Dr. Emory Erickson. This station is proud to have received the endorsement of Admiral Leonard McCoy"
    )
    return stationErickson
end
function placeEvondos()
    --Evondos
    stationEvondos = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationEvondos
        :setPosition(psx, psy)
        :setCallSign("Evondos")
        :setDescription(_("scienceDescription-station", "Autodoc components"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationEvondos] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "autodoc", 5, 56 },
            }
            ...
            goods[stationEvondos] =
                { { "food", math.random(5, 10), 1 }, { "autodoc", 5, 56 } }
    ...
    if random(1, 100) < 41 then
        tradeLuxury[stationEvondos] = true
    end
    stationEvondos.publicRelations = true
    stationEvondos.generalInformation = _(
        "stationGeneralInfo-comms",
        "We provide components for automated medical machinery"
    )
    stationEvondos.stationHistory = _(
        "stationStory-comms",
...
function placeFeynman()
    ...
    stationFeynman
        ...
        :setCallSign("Feynman")
        :setDescription(
            _("scienceDescription-station", "Nanotechnology research")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationFeynman] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "nanites", 5, 79 },
                { "software", 5, 115 },
...
function placeGrasberg()
    ...
    stationGrasberg
        :setPosition(psx, psy)
        :setCallSign("Grasberg")
        :setDescription(_("scienceDescription-station", "Mining"))
    stationGrasberg.publicRelations = true
    stationGrasberg.generalInformation = _(
        "stationGeneralInfo-comms",
        "We mine nearby asteroids for precious minerals and process them for sale"
    )
    stationGrasberg.stationHistory = _(
        "stationStory-comms",
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            elseif grasbergGoods < 60 then
                ...
                goods[stationGrasberg] =
                    { { "luxury", 5, 70 }, { "food", math.random(5, 10), 1 } }
            end
        end
    else
        if grasbergGoods < 20 then
            goods[stationGrasberg] =
                { { "luxury", 5, 70 }, { "gold", 5, 25 }, { "cobalt", 4, 50 } }
        elseif grasbergGoods < 40 then
            goods[stationGrasberg] = { { "luxury", 5, 70 }, { "gold", 5, 25 } }
        elseif grasbergGoods < 60 then
            goods[stationGrasberg] =
                { { "luxury", 5, 70 }, { "cobalt", 4, 50 } }
        else
...
function placeHayden()
    --Hayden
    stationHayden = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationHayden:setPosition(psx, psy):setCallSign("Hayden"):setDescription(
        _("scienceDescription-station", "Observatory and stellar mapping")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationHayden] = {
            ...
            goods[stationHayden] =
                { { "food", math.random(5, 10), 1 }, { "nanites", 5, 65 } }
        end
    else
        goods[stationHayden] = { { "nanites", 5, 65 } }
    end
    stationHayden.publicRelations = true
    stationHayden.generalInformation = _(
        "stationGeneralInfo-comms",
...
function placeHeyes()
    ...
    stationHeyes
        :setPosition(psx, psy)
        :setCallSign("Heyes")
        :setDescription(_("scienceDescription-station", "Sensor components"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationHeyes] = {
                { "food", math.random(5, 10), 1 },
            ...
            goods[stationHeyes] =
                { { "food", math.random(5, 10), 1 }, { "sensor", 5, 72 } }
        end
    else
    ...
    stationHeyes.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and manufacture sensor components and systems"
    )
    stationHeyes.stationHistory = _(
        "stationStory-comms",
        "The station is named after Tony Heyes the inventor of some of the earliest electromagnetic sensors in the mid 20th century on Earth in the United Kingdom to assist blind human mobility"
    )
    return stationHeyes
end
function placeHossam()
    --Hossam
    stationHossam = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationHossam
        :setPosition(psx, psy)
        :setCallSign("Hossam")
        :setDescription(_("scienceDescription-station", "Nanite supplier"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationHossam] = {
                { "food", math.random(5, 10), 1 },
            ...
            goods[stationHossam] =
                { { "food", math.random(5, 10), 1 }, { "nanites", 5, 48 } }
            if random(1, 100) < 44 then
                tradeMedicine[stationHossam] = true
            end
        end
    else
        ...
        if random(1, 100) < 24 then
            tradeFood[stationHossam] = true
        end
    end
    if random(1, 100) < 63 then
        tradeLuxury[stationHossam] = true
    end
    stationHossam.publicRelations = true
    stationHossam.generalInformation = _(
        "stationGeneralInfo-comms",
        "We provide nanites for various organic and non-organic systems"
    )
    stationHossam.stationHistory = _(
        "stationStory-comms",
        "This station is named after the nanotechnologist Hossam Haick from the early 21st century on Earth in Israel"
    )
...
function placeImpala()
    ...
    stationImpala.generalInformation = _(
        "stationGeneralInfo-comms",
...
function placeKomov()
    ...
    stationKomov
        ...
        :setCallSign("Komov")
        :setDescription(
            _("scienceDescription-station", "Xenopsychology training")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationKomov] = {
            ...
            goods[stationKomov] =
                { { "food", math.random(5, 10), 1 }, { "filament", 5, 46 } }
            if random(1, 100) < 44 then
                tradeMedicine[stationKomov] = true
            end
        end
    else
        ...
        if random(1, 100) < 24 then
            tradeFood[stationKomov] = true
        end
    end
    stationKomov.publicRelations = true
    stationKomov.generalInformation = _(
        "stationGeneralInfo-comms",
        "We provide classes and simulation to help train diverse species in how to relate to each other"
    )
    stationKomov.stationHistory = _(
        "stationStory-comms",
        "A continuation of the research initially conducted by Dr. Gennady Komov in the early 22nd century on Venus, supported by the application of these principles"
    )
    return stationKomov
end
function placeKrak()
    --Krak
    stationKrak = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    if random(1, 100) < 50 then
        tradeFood[stationKrak] = true
    end
    if random(1, 100) < 50 then
        tradeLuxury[stationKrak] = true
    end
...
function placeKruk()
    ...
    if random(1, 100) < 50 then
        tradeFood[stationKruk] = true
    end
    if random(1, 100) < 50 then
        tradeMedicine[stationKruk] = true
    end
    return stationKruk
end
function placeLipkin()
    --Lipkin
    stationLipkin = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationLipkin] =
                { { "food", math.random(5, 10), 1 }, { "autodoc", 5, 76 } }
        end
    else
    ...
    stationLipkin.generalInformation = _(
        "stationGeneralInfo-comms",
        "We build and repair and provide components and upgrades for automated facilities designed for ships where a doctor cannot be a crew member (commonly called autodocs)"
    )
    stationLipkin.stationHistory = _(
        "stationStory-comms",
...
function placeMadison()
    --Madison
    stationMadison = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationMadison:setPosition(psx, psy):setCallSign("Madison"):setDescription(
        _("scienceDescription-station", "Zero gravity sports and entertainment")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationMadison] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "luxury", 5, 70 },
            }
            ...
            goods[stationMadison] =
                { { "food", math.random(5, 10), 1 }, { "luxury", 5, 70 } }
    ...
    stationMadison.generalInformation = _(
        "stationGeneralInfo-comms",
        "Come take in a game or two or perhaps see a show"
    )
    stationMadison.stationHistory = _(
        "stationStory-comms",
        "Named after Madison Square Gardens from 21st century Earth, this station was designed to serve similar purposes in space - a venue for sports and entertainment"
    )
    return stationMadison
end
function placeMaiman()
    --Maiman
    stationMaiman = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationMaiman
        ...
        :setCallSign("Maiman")
        :setDescription(
            _("scienceDescription-station", "Energy beam components")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationMaiman] = {
            ...
            goods[stationMaiman] =
                { { "food", math.random(5, 10), 1 }, { "beam", 5, 70 } }
    ...
    stationMaiman.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and manufacture energy beam components and systems"
    )
    stationMaiman.stationHistory = _(
        "stationStory-comms",
        "The station is named after Theodore Maiman who researched and built the first laser in the mid 20th centuryon Earth"
    )
    return stationMaiman
end
function placeMarconi()
    --Marconi
    stationMarconi = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    stationMarconi.generalInformation =
        _("stationGeneralInfo-comms", "We manufacture energy beam components")
    stationMarconi.stationHistory = _(
        "stationStory-comms",
        "Station named after Guglielmo Marconi an Italian inventor from early 20th century Earth who, along with Nicolo Tesla, claimed to have invented a death ray or particle beam weapon"
    )
    return stationMarconi
end
function placeMayo()
    --Mayo
    stationMayo = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationMayo
        ...
        :setDescription(_("scienceDescription-station", "Medical Research"))
    goods[stationMayo] =
        { { "food", 5, 1 }, { "medicine", 5, 5 }, { "autodoc", 5, 128 } }
    stationMayo.publicRelations = true
    stationMayo.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research exotic diseases and other human medical conditions"
    )
    stationMayo.stationHistory = _(
        "stationStory-comms",
...
function placeMiller()
    ...
    stationMiller
        :setPosition(psx, psy)
        :setCallSign("Miller")
        :setDescription(_("scienceDescription-station", "Exobiology research"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationMiller] = {
                { "food", math.random(5, 10), 1 },
            ...
            goods[stationMiller] =
                { { "food", math.random(5, 10), 1 }, { "optic", 10, 60 } }
        end
    else
        goods[stationMiller] = { { "optic", 10, 60 } }
    end
    stationMiller.publicRelations = true
    stationMiller.generalInformation = _(
        "stationGeneralInfo-comms",
        "We study recently discovered life forms not native to Earth"
    )
    stationMiller.stationHistory = _(
        "stationStory-comms",
        "This station was named after one the early exobiologists from mid 20th century Earth, Dr. Stanley Miller"
    )
    return stationMiller
end
function placeMuddville()
    --Muddville
    stationMudd = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationMudd] =
                { { "food", math.random(5, 10), 1 }, { "luxury", 10, 60 } }
        end
    else
        goods[stationMudd] = { { "luxury", 10, 60 } }
    end
    stationMudd.publicRelations = true
    stationMudd.generalInformation = _(
        "stationGeneralInfo-comms",
        "Come to Muddvile for all your trade and commerce needs and desires"
    )
    stationMudd.stationHistory = _(
        "stationStory-comms",
        "Upon retirement, Harry Mudd started this commercial venture using his leftover inventory and extensive connections obtained while he traveled the stars as a salesman"
    )
    return stationMudd
end
function placeNexus6()
    --Nexus-6
    stationNexus6 = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationNexus6] =
                { { "food", math.random(5, 10), 1 }, { "android", 5, 93 } }
    ...
    stationNexus6.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and manufacture android components and systems. Our design our androids to maximize their likeness to humans"
    )
    stationNexus6.stationHistory = _(
        "stationStory-comms",
        "The station is named after the ground breaking model of android produced by the Tyrell corporation"
    )
    return stationNexus6
end
function placeOBrien()
    --O'Brien
    stationOBrien = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationOBrien] =
                { { "food", math.random(5, 10), 1 }, { "transporter", 5, 76 } }
            if random(1, 100) < 34 then
    ...
    if random(1, 100) < 43 then
        tradeLuxury[stationOBrien] = true
    end
    stationOBrien.publicRelations = true
    stationOBrien.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and fabricate high quality transporters and transporter components for use aboard ships"
    )
    stationOBrien.stationHistory = _(
        "stationStory-comms",
        "Miles O'Brien started this business after his experience as a transporter chief"
    )
    return stationOBrien
end
function placeOlympus()
    --Olympus
    stationOlympus = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
...
function placeOutpost15()
    ...
    stationOutpost15
        :setPosition(psx, psy)
        :setCallSign("Outpost-15")
        :setDescription(_("scienceDescription-station", "Mining and trade"))
    tradeFood[stationOutpost15] = true
    outpost15Goods = random(1, 100)
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            if outpost15Goods < 20 then
                goods[stationOutpost15] = {
                    { "luxury", 5, 70 },
                    { "gold", 5, 25 },
                    { "cobalt", 4, 50 },
                    { "food", math.random(5, 10), 1 },
...
function placeOutpost21()
    ...
    stationOutpost21
        :setPosition(psx, psy)
        :setCallSign("Outpost-21")
        :setDescription(_("scienceDescription-station", "Mining and gambling"))
    placeRandomAroundPoint(Asteroid, 15, 1, 15000, psx, psy)
    outpost21Goods = random(1, 100)
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            if outpost21Goods < 20 then
                goods[stationOutpost21] = {
                    { "luxury", 5, 70 },
                    { "gold", 5, 25 },
                    { "cobalt", 4, 50 },
                    { "food", math.random(5, 10), 1 },
            ...
            if random(1, 100) < 50 then
                tradeMedicine[stationOutpost21] = true
            end
        end
    else
        if outpost21Goods < 20 then
...
function placeOwen()
    --Owen
    stationOwen = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationOwen:setPosition(psx, psy):setCallSign("Owen"):setDescription(
        _("scienceDescription-station", "Load lifters and components")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationOwen] = {
            ...
            goods[stationOwen] =
                { { "food", math.random(5, 10), 1 }, { "lifter", 5, 61 } }
        end
    else
...
function placePanduit()
    ...
    stationPanduit
        :setPosition(psx, psy)
        :setCallSign("Panduit")
        :setDescription(_("scienceDescription-station", "Optic components"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationPanduit] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "optic", 5, 79 },
            }
            ...
            goods[stationPanduit] =
                { { "food", math.random(5, 10), 1 }, { "optic", 5, 79 } }
            if random(1, 100) < 33 then
                tradeMedicine[stationPanduit] = true
            end
        end
    else
...
function placeRipley()
    --Ripley
    stationRipley = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationRipley:setPosition(psx, psy):setCallSign("Ripley"):setDescription(
        _("scienceDescription-station", "Load lifters and components")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationRipley] =
                { { "food", math.random(5, 10), 1 }, { "lifter", 5, 82 } }
    ...
    if random(1, 100) < 47 then
        tradeLuxury[stationRipley] = true
    end
    stationRipley.publicRelations = true
    stationRipley.generalInformation =
        _("stationGeneralInfo-comms", "We provide load lifters and components")
    stationRipley.stationHistory = _(
        "stationStory-comms",
        "The station is named after Ellen Ripley who made creative and effective use of one of our load lifters when defending her ship"
    )
    return stationRipley
end
function placeRutherford()
    --Rutherford
    stationRutherford = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationRutherford
        :setPosition(psx, psy)
        :setCallSign("Rutherford")
        :setDescription(
            _("scienceDescription-station", "Shield components and research")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationRutherford] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "shield", 5, 90 },
            }
    ...
    tradeMedicine[stationRutherford] = true
    if random(1, 100) < 43 then
        tradeLuxury[stationRutherford] = true
    end
    stationRutherford.publicRelations = true
    stationRutherford.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and fabricate components for ship shield systems"
    )
    stationRutherford.stationHistory = _(
        "stationStory-comms",
        "This station was named after the national research institution Rutherford Appleton Laboratory in the United Kingdom which conducted some preliminary research into the feasability of generating an energy shield in the late 20th century"
    )
    return stationRutherford
end
function placeScience7()
    --Science 7
    stationScience7 = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationScience7
        :setPosition(psx, psy)
        :setCallSign("Science-7")
        :setDescription(_("scienceDescription-station", "Observatory"))
    goods[stationScience7] = { { "food", 2, 1 } }
    return stationScience7
end
function placeShawyer()
    --Shawyer
    stationShawyer = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationShawyer:setPosition(psx, psy):setCallSign("Shawyer"):setDescription(
        _("scienceDescription-station", "Impulse engine components")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationShawyer] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "impulse", 5, 100 },
            }
            ...
            goods[stationShawyer] =
                { { "food", math.random(5, 10), 1 }, { "impulse", 5, 100 } }
    ...
    stationShawyer.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and manufacture impulse engine components and systems"
    )
    stationShawyer.stationHistory = _(
        "stationStory-comms",
...
function placeShree()
    --Shree
    stationShree = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationShree:setPosition(psx, psy):setCallSign("Shree"):setDescription(
        _("scienceDescription-station", "Repulsor and tractor beam components")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
...
function placeSoong()
    ...
    stationSoong.generalInformation = _(
        "stationGeneralInfo-comms",
        "We create androids and android components"
    )
    stationSoong.stationHistory = _(
        "stationStory-comms",
        "The station is named after Dr. Noonian Soong, the famous android researcher and builder"
    )
    return stationSoong
end
function placeTiberius()
    --Tiberius
    stationTiberius = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationTiberius
        :setPosition(psx, psy)
        :setCallSign("Tiberius")
        :setDescription(
            _("scienceDescription-station", "Logistics coordination")
        )
    goods[stationTiberius] = { { "food", 5, 1 } }
    stationTiberius.publicRelations = true
    stationTiberius.generalInformation = _(
        "stationGeneralInfo-comms",
        "We support the stations and ships in the area with planning and communication services"
    )
    stationTiberius.stationHistory = _(
        "stationStory-comms",
        "We recognize the influence of Starfleet Captain James Tiberius Kirk in the 23rd century in our station name"
    )
    return stationTiberius
end
function placeTokra()
    --Tokra
    stationTokra = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationTokra:setPosition(psx, psy):setCallSign("Tokra"):setDescription(
        _("scienceDescription-station", "Advanced material components")
    )
    whatTrade = random(1, 100)
    if stationFaction == "Human Navy" then
    ...
    stationTokra.generalInformation = _(
        "stationGeneralInfo-comms",
        "We create multiple types of advanced material components. Our most popular products are our filaments"
    )
    stationTokra.stationHistory = _(
        "stationStory-comms",
        "We learned several of our critical industrial processes from the Tokra race, so we honor our fortune by naming the station after them"
    )
    return stationTokra
end
function placeToohie()
    --Toohie
    stationToohie = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationToohie:setPosition(psx, psy):setCallSign("Toohie"):setDescription(
        _(
            "scienceDescription-station",
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            if random(1, 100) < 25 then
                tradeMedicine[stationToohie] = true
            end
        end
    else
    ...
    stationToohie.generalInformation = _(
        "stationGeneralInfo-comms",
        "We research and make general and specialized components for ship shield and ship armor systems"
    )
    stationToohie.stationHistory = _(
        "stationStory-comms",
        "This station was named after one of the earliest researchers in shield technology, Alexander Toohie back when it was considered impractical to construct shields due to the physics involved."
    )
    return stationToohie
end
function placeUtopiaPlanitia()
    --Utopia Planitia
    stationUtopiaPlanitia = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationUtopiaPlanitia
        :setPosition(psx, psy)
        :setCallSign("Utopia Planitia")
        :setDescription(
            _(
                "scienceDescription-station",
    ...
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            ...
            goods[stationUtopiaPlanitia] =
                { { "food", math.random(5, 10), 1 }, { "warp", 5, 167 } }
        end
    else
        goods[stationUtopiaPlanitia] = { { "warp", 5, 167 } }
    end
    stationUtopiaPlanitia.publicRelations = true
    stationUtopiaPlanitia.generalInformation = _(
        "stationGeneralInfo-comms",
        "We work on all aspects of naval ship building and maintenance. Many of the naval models are researched, designed and built right here on this station. Our design goals seek to make the space faring experience as simple as possible given the tremendous capabilities of the modern naval vessel"
    )
    return stationUtopiaPlanitia
end
function placeVactel()
    --Vactel
    stationVactel = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationVactel:setPosition(psx, psy):setCallSign("Vactel"):setDescription(
        _("scienceDescription-station", "Shielded Circuitry Fabrication")
    )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
...
function placeVeloquan()
    ...
    stationVeloquan
        :setPosition(psx, psy)
        :setCallSign("Veloquan")
        :setDescription(_("scienceDescription-station", "Sensor components"))
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationVeloquan] = {
                { "food", math.random(5, 10), 1 },
                { "medicine", 5, 5 },
                { "sensor", 5, 68 },
            }
            ...
            goods[stationVeloquan] =
                { { "food", math.random(5, 10), 1 }, { "sensor", 5, 68 } }
    ...
    stationVeloquan.generalInformation = _(
        "stationGeneralInfo-comms",
    ...
    stationVeloquan.stationHistory = _(
        "stationStory-comms",
...
function placeZefram()
    ...
    stationZefram
        ...
        :setCallSign("Zefram")
        :setDescription(
            _("scienceDescription-station", "Warp engine components")
        )
    if stationFaction == "Human Navy" then
        if random(1, 5) <= 1 then
            goods[stationZefram] = {
            ...
            goods[stationZefram] =
                { { "food", math.random(5, 10), 1 }, { "warp", 5, 140 } }
            if random(1, 100) < 27 then
                tradeMedicine[stationZefram] = true
            end
        end
    else
    ...
    stationZefram.generalInformation = _(
        "stationGeneralInfo-comms",
        "We specialize in the esoteric components necessary to make warp drives function properly"
    )
    stationZefram.stationHistory = _(
        "stationStory-comms",
        "Zefram Cochrane constructed the first warp drive in human history. We named our station after him because of the specialized warp systems work we do"
    )
...
function placeJabba()
    ...
    stationJabba
        ...
        :setCallSign("Jabba")
        :setDescription(
            _("scienceDescription-station", "Commerce and gambling")
        )
    ...
    stationJabba.generalInformation = _(
        "stationGeneralInfo-comms",
...
function placeLando()
    ...
    stationLando
        :setPosition(psx, psy)
        :setCallSign("Lando")
        :setDescription(_("scienceDescription-station", "Casino and Gambling"))
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
...
function placeMaverick()
    ...
    stationMaverick
        ...
        :setDescription(
            _("scienceDescription-station", "Gambling and resupply")
        )
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
    ...
    stationMaverick.generalInformation =
        _("stationGeneralInfo-comms", "Relax and meet some interesting players")
    return stationMaverick
end
function placeNefatha()
    --Nefatha
    stationNefatha = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationNefatha
        ...
        :setCallSign("Nefatha")
        :setDescription(
            _("scienceDescription-station", "Commerce and recreation")
        )
...
function placeOutpost7()
    ...
    stationOutpost7
        :setPosition(psx, psy)
        :setCallSign("Outpost-7")
        :setDescription(_("scienceDescription-station", "Resupply"))
    goods[stationOutpost7] = { { "luxury", 5, 80 } }
    return stationOutpost7
end
function placeOutpost8()
    --Outpost 8
    stationOutpost8 = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
...
function placePrada()
    ...
    stationPrada
        :setPosition(psx, psy)
        :setCallSign("Prada")
        :setDescription(_("scienceDescription-station", "Textiles and fashion"))
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
...
function placeResearch11()
    --Research-11
    stationResearch11 = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationResearch11
        :setPosition(psx, psy)
        :setCallSign("Research-11")
        :setDescription(
            _("scienceDescription-station", "Stress Psychology Research")
        )
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
...
function placeResearch19()
    --Research-19
    stationResearch19 = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationResearch19
        :setPosition(psx, psy)
        :setCallSign("Research-19")
        :setDescription(_("scienceDescription-station", "Low gravity research"))
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
    ...
    elseif stationGoodChoice == 2 then
        ...
        goods[stationResearch19] =
            { { "communication", 5, math.random(55, 89) } }
    end
    return stationResearch19
end
function placeRubis()
    --Rubis
    stationRubis = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    ...
    stationRubis.generalInformation = _(
        "stationGeneralInfo-comms",
...
function placeScience2()
    ...
    stationScience2
        :setPosition(psx, psy)
        :setCallSign("Science-2")
        :setDescription(
            _("scienceDescription-station", "Research Lab and Observatory")
        )
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
...
function placeScience4()
    ...
    stationScience4
        :setPosition(psx, psy)
        :setCallSign("Science-4")
        :setDescription(_("scienceDescription-station", "Biotech research"))
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
...
function placeSkandar()
    --Skandar
    stationSkandar = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationSkandar:setPosition(psx, psy):setCallSign("Skandar"):setDescription(
        _("scienceDescription-station", "Routine maintenance and entertainment")
    )
    goods[stationSkandar] = { { "luxury", 5, 87 } }
    stationSkandar.publicRelations = true
    stationSkandar.generalInformation = _(
        "stationGeneralInfo-comms",
        "Stop by for repairs. Take in one of our juggling shows featuring the four-armed Skandars"
    )
    stationSkandar.stationHistory = _(
        "stationStory-comms",
        "The nomadic Skandars have set up at this station to practice their entertainment and maintenance skills as well as build a community where Skandars can relax"
    )
    return stationSkandar
end
function placeSpot()
    --Spot
    stationSpot = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
...
function placeStarnet()
    --Starnet
    stationStarnet = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationStarnet:setPosition(psx, psy):setCallSign("Starnet"):setDescription(
        _("scienceDescription-station", "Automated weapons systems")
    )
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
    ...
    stationStarnet.generalInformation = _(
        "stationGeneralInfo-comms",
...
function placeTandon()
    ...
    stationTandon
        ...
        :setCallSign("Tandon")
        :setDescription(
            _("scienceDescription-station", "Biotechnology research")
        )
    stationGoodChoice = math.random(1, 3)
    if stationGoodChoice == 1 then
...
function placeVaiken()
    --Vaiken
    stationVaiken = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationVaiken:setPosition(psx, psy):setCallSign("Vaiken"):setDescription(
        _(
            "scienceDescription-station",
            "Ship building and maintenance facility"
        )
...
function placeAramanth()
    --Aramanth
    stationAramanth = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCallSign("Aramanth")
        :setPosition(psx, psy)
...
function placeMagMesra()
    --Magenta Mesra
    stationMagMesra = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCallSign("Magenta Mesra")
        :setPosition(psx, psy)
    return stationMagMesra
end
function placeMosEisley()
    --Mos Eisley
    stationMosEisley = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCallSign("Mos Eisley")
        :setPosition(psx, psy)
    return stationMosEisley
end
function placeQuestaVerde()
    --Questa Verde
    stationQuestaVerde = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCallSign("Questa Verde")
        :setPosition(psx, psy)
    return stationQuestaVerde
end
function placeRlyeh()
    --R'lyeh
    stationRlyeh = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCallSign("R'lyeh")
        :setPosition(psx, psy)
...
function placeScarletCit()
    --Scarlet Citadel
    stationScarletCitadel = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCommsScript("")
        :setCommsFunction(commsStation)
...
function placeStahlstadt()
    --Stahlstadt
    stationStahlstadt = SpaceStation()
        :setTemplate(szt())
        :setFaction(stationFaction)
        :setCallSign("Stahlstadt")
        :setPosition(psx, psy)
...
function setKraylorDefensiveFleet()
    ...
    kraylorFleet1, kraylorFleet1Power =
        spawnEnemyFleet(f1bx, f1by, random(90, 130))
    ...
    kraylorFleet2, kraylorFleet2Power =
        spawnEnemyFleet(f1bx, f1by, kraylorFleet2Power)
    ...
    repeat
        candidate = kraylorStationList[math.random(1, #kraylorStationList)]
        if
            candidate ~= kraylorFleet1base
            and candidate ~= kraylorFleet2base
        then
            kraylorFleet3base = candidate
        end
    until kraylorFleet3base ~= nil
    local f1bx, f1by = kraylorFleet3base:getPosition()
    kraylorFleet3, kraylorFleet3Power =
        spawnEnemyFleet(f1bx, f1by, kraylorFleet3Power)
    ...
    repeat
        ...
        if
            candidate ~= kraylorFleet1base
            and candidate ~= kraylorFleet2base
            and candidate ~= kraylorFleet3base
        then
            kraylorFleet4base = candidate
        end
    until kraylorFleet4base ~= nil
    local f1bx, f1by = kraylorFleet4base:getPosition()
    kraylorFleet4, kraylorFleet4Power =
        spawnEnemyFleet(f1bx, f1by, kraylorResource / 2)
    ...
    repeat
        ...
        if
            ...
            and candidate ~= kraylorFleet2base
            and candidate ~= kraylorFleet3base
            and candidate ~= kraylorFleet4base
        then
            kraylorFleet5base = candidate
        end
    until kraylorFleet5base ~= nil
    local f1bx, f1by = kraylorFleet5base:getPosition()
    kraylorFleet5, kraylorFleet5Power =
        spawnEnemyFleet(f1bx, f1by, kraylorResource)
...
function setExuariDefensiveFleet()
    ...
    exuariFleet1, exuariFleet1Power =
        spawnEnemyFleet(f1bx, f1by, random(90, 130), 1, "Exuari")
    ...
    exuariFleet2, exuariFleet2Power =
        spawnEnemyFleet(f1bx, f1by, exuariFleet2Power, 1, "Exuari")
    ...
    exuariFleet3, exuariFleet3Power =
        spawnEnemyFleet(f1bx, f1by, exuariFleet3Power, 1, "Exuari")
    ...
    repeat
        candidate = exuariStationList[math.random(1, #exuariStationList)]
        if
            candidate ~= exuariFleet1base
            and candidate ~= exuariFleet2base
            and candidate ~= exuariFleet3base
        then
            exuariFleet4base = candidate
        end
    until exuariFleet4base ~= nil
    local f1bx, f1by = exuariFleet4base:getPosition()
    exuariFleet4, exuariFleet4Power =
        spawnEnemyFleet(f1bx, f1by, exuariResource / 2, 1, "Exuari")
    ...
    repeat
        ...
        if
            ...
            and candidate ~= exuariFleet2base
            and candidate ~= exuariFleet3base
            and candidate ~= exuariFleet4base
        then
            exuariFleet5base = candidate
        end
    until exuariFleet5base ~= nil
    local f1bx, f1by = exuariFleet5base:getPosition()
    exuariFleet5, exuariFleet5Power =
        spawnEnemyFleet(f1bx, f1by, exuariResource, 1, "Exuari")
...
function setArlenianDefensiveFleet()
    ...
    arlenianFleet1base =
        arlenianStationList[math.random(1, #arlenianStationList)]
    local f1bx, f1by = arlenianFleet1base:getPosition()
    arlenianFleet1, arlenianFleet1Power =
        spawnEnemyFleet(f1bx, f1by, random(90, 130), 1, "Arlenians")
    ...
    arlenianFleet2, arlenianFleet2Power =
        spawnEnemyFleet(f1bx, f1by, arlenianFleet2Power, 1, "Arlenians")
    ...
    repeat
        candidate = arlenianStationList[math.random(1, #arlenianStationList)]
        if
            candidate ~= arlenianFleet1base
            and candidate ~= arlenianFleet2base
        then
            arlenianFleet3base = candidate
        end
    until arlenianFleet3base ~= nil
    local f1bx, f1by = arlenianFleet3base:getPosition()
    arlenianFleet3, arlenianFleet3Power =
        spawnEnemyFleet(f1bx, f1by, arlenianFleet3Power, 1, "Arlenians")
    ...
    repeat
        ...
        if
            candidate ~= arlenianFleet1base
            and candidate ~= arlenianFleet2base
            and candidate ~= arlenianFleet3base
        then
            arlenianFleet4base = candidate
        end
    until arlenianFleet4base ~= nil
    local f1bx, f1by = arlenianFleet4base:getPosition()
    arlenianFleet4, arlenianFleet4Power =
        spawnEnemyFleet(f1bx, f1by, arlenianResource / 2, 1, "Arlenians")
    ...
    repeat
        ...
        if
            ...
            and candidate ~= arlenianFleet2base
            and candidate ~= arlenianFleet3base
            and candidate ~= arlenianFleet4base
        then
            arlenianFleet5base = candidate
        end
    until arlenianFleet5base ~= nil
    local f1bx, f1by = arlenianFleet5base:getPosition()
    arlenianFleet5, arlenianFleet5Power =
        spawnEnemyFleet(f1bx, f1by, arlenianResource, 1, "Arlenians")
...
function setHumanDefensiveFleet()
    ...
    humanFleet1, humanFleet1Power =
        spawnEnemyFleet(f1bx, f1by, random(30, 70), 1, "Human Navy")
    ...
    humanFleet2, humanFleet2Power =
        spawnEnemyFleet(f1bx, f1by, humanFleet2Power, 1, "Human Navy")
    ...
    humanFleet3, humanFleet3Power =
        spawnEnemyFleet(f1bx, f1by, humanFleet3Power, 1, "Human Navy")
    ...
    repeat
        candidate = humanStationList[math.random(1, #humanStationList)]
        if
            candidate ~= humanFleet1base
            and candidate ~= humanFleet2base
            and candidate ~= humanFleet3base
        then
            humanFleet4base = candidate
        end
    until humanFleet4base ~= nil
    local f1bx, f1by = humanFleet4base:getPosition()
    humanFleet4, humanFleet4Power =
        spawnEnemyFleet(f1bx, f1by, humanResource / 2, 1, "Human Navy")
    ...
    repeat
        ...
        if
            ...
            and candidate ~= humanFleet2base
            and candidate ~= humanFleet3base
            and candidate ~= humanFleet4base
        then
            humanFleet5base = candidate
        end
    until humanFleet5base ~= nil
    local f1bx, f1by = humanFleet5base:getPosition()
    humanFleet5, humanFleet5Power =
        spawnEnemyFleet(f1bx, f1by, humanResource, 1, "Human Navy")
...
function spawnEnemyFleet(xOrigin, yOrigin, power, danger, enemyFaction)
    ...
    while enemyStrength > 0 do
        ...
        elseif enemyFaction == "Independent" then
            rawNeutralShipStrength = rawNeutralShipStrength
                + stsl[shipTemplateType]
        ...
        if deployConfig < 50 then
            ship:setPosition(
                xOrigin + fleetPosDelta1x[enemyPosition] * sp,
                yOrigin + fleetPosDelta1y[enemyPosition] * sp
            )
        else
            ship:setPosition(
                xOrigin + fleetPosDelta2x[enemyPosition] * sp,
                yOrigin + fleetPosDelta2y[enemyPosition] * sp
            )
        end
...
function handleDockedState()
    ...
    for _, missile_type in ipairs(missile_types) do
        missilePresence = missilePresence
            + player:getWeaponStorageMax(missile_type)
    end
    if missilePresence > 0 then
        ...
        if
            comms_target.nukeAvail
            or comms_target.empAvail
            or comms_target.homeAvail
            or comms_target.mineAvail
            or comms_target.hvliAvail
        then
            addCommsReply(
    ...
    if comms_target == professorStation then
        if professorSearch and not professorProvidedPlans then
            addCommsReply(
                _("doomsday-comms", "Tell me about Gregory Unruh"),
                function()
                    setCommsMessage(
                        _(
                            "doomsday-comms",
                            "He's a retired scientist here. He's on C deck if you'd like to visit"
                        )
                    )
                    addCommsReply(
                        _("doomsday-comms", "Visit Gregory Unruh on C deck"),
                        function()
                            setCommsMessage(
                                _(
                                    "doomsday-comms",
                                    "[Gregory Unruh] What can I do for you?"
                                )
                            )
                            addCommsReply(
                                _(
                                    "doomsday-comms",
                                    "We need your plans for a doomsday device defense mechanism"
                                ),
                                function()
                                    setCommsMessage(
                                        _(
                                            "doomsday-comms",
                                            "Ah, I see someone is taking my research seriously. Here is my incomplete prototype along with the plans. You'll notice that certain components are still required."
                                        )
                                    )
    ...
    if comms_target == doctorStation then
        if doctorSearch and not doctorAssisting then
            addCommsReply(
                _("doctor-comms", "Tell me about Morrigan Thultris"),
                function()
                    setCommsMessage(
                        _("doctor-comms", "She does medical research here")
                    )
                    addCommsReply(
                        _("doctor-comms", "May I speak with her?"),
                        function()
                            setCommsMessage(
                                _(
                                    "doctor-comms",
                                    "She won't come to the communications nexus, but you may visit her in her quarters"
                                )
                            )
                            addCommsReply(
                                _("doctor-comms", "Visit Dr. Thultris"),
                                function()
                                    setCommsMessage(
                                        _(
                                            "doctor-comms",
                                            "[Dr. Thultris behind closed quarters door]\nWho is it?"
                                        )
                                    )
                                    addCommsReply(
                                        string.format(
                                            _(
                                                "doctor-comms",
                                                "I am an officer from the Human Navy ship, %s"
                                            ),
                                            player:getCallSign()
                                        ),
                                        function()
                                            ...
                                            addCommsReply(
                                                ...
                                                function()
                                                    if
                                                        random(1, 4)
                                                            + doctorRejectionCount
                                                        >= 3
                                                    then
                                                        setCommsMessage(
                                                            _(
                                                                "doctor-comms",
                                                                "[Dr. Thultris opens her door]\nWith what?"
                                                            )
                                                        )
                                                        addCommsReply(
                                                            _(
                                                            ...
                                                            function()
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "As you pull her down the corridor back to the ship, explaining why you've come for her, she protests initially then eventually runs back with you, convinced of the urgency of the situation"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                                    ...
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "She slams the door in your face as you reach for her"
                                                                        )
                                                                    )
                                                                    doctorRejectionCount =
                                                                        0
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "A dying patient needs your expertise"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] Then let's get going to help the patient\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                                    ...
                                                                    doctorRejectionCount = doctorRejectionCount
                                                                        + 1
                                                                end
                                                            end
                                                        )
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "An Arlenian Admiral shows symptoms like those you published"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                            ...
                                            addCommsReply(
                                                _(
                                                    "doctor-comms",
                                                    "Medical team suggested your research for a patient"
                                                ),
                                                function()
                                                    if
                                                        random(1, 4)
                                                    ...
                                                    then
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "The symptoms closely correlate to your research"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] I think I can help\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                                    ...
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "Let me know what happens with the patient\n[Dr. Thultris closes her door]"
                                                                        )
                                                                    )
                                                                    doctorRejectionCount = doctorRejectionCount
                                                                        + 1
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "Your research may save an Arlenian Admiral's life"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] I would love to apply my research\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                                    ...
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "Diplomacy and military matters bore me\n[Dr. Thultris closes her door]"
                                                                        )
                                                                    )
                                                                    doctorRejectionCount = doctorRejectionCount
                                                                        + 1
                                            ...
                                            addCommsReply(
                                                _(
                                                    "doctor-comms",
                                                    "Arlenian with an unusual disease is near death"
                                                ),
                                                function()
                                                    if
                                                        random(1, 4)
                                                    ...
                                                    then
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "The Arlenian medical team referred us to you"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] Nice to be recognized outside of human space\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "Your research accurately describes the symptoms"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] I would like to see the patient\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                    ...
                                    addCommsReply(
                                        _(
                                            "doctor-comms",
                                            "I am an admirer of your research and have questions"
                                        ),
                                        function()
                                            setCommsMessage(
                                                _(
                                            ...
                                            addCommsReply(
                                                _(
                                                    "doctor-comms",
                                                    "The depth of your most recent publication"
                                                ),
                                                function()
                                                    if
                                                        random(1, 4)
                                                    ...
                                                    then
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "I know of a patient that might benefit"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] Those are rare. May I see the patient?\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                                    ...
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "My research is unlikely to apply\n[Dr. Thultris closes her door]"
                                                                        )
                                                                    )
                                                                    doctorRejectionCount = doctorRejectionCount
                                                                        + 1
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "Your research could help a sick Arlenian Admiral"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] I would like to see my research in the Arlenian context\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                                    ...
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "I don't think it applies to Arlenians\n[Dr. Thultris closes her door]"
                                                                        )
                                                                    )
                                                                    doctorRejectionCount = doctorRejectionCount
                                                                        + 1
                                            ...
                                            addCommsReply(
                                                _(
                                                    "doctor-comms",
                                                    "The breadth of knowledge you demonstrate"
                                                ),
                                                function()
                                                    if
                                                        random(1, 4)
                                                    ...
                                                    then
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "We need your help with a dying Arlenian Admiral"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] Why didn't you say so? Of course I'll help\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "An Arlenian medical team thinks you can help a dying patient"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] That's reasonable. Let's go\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                                    ...
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "I'm not concerned with Arlenian patients\n[Dr. Thultris closes her door]"
                                                                        )
                                                                    )
                                                                    doctorRejectionCount = doctorRejectionCount
                                                                        + 1
                                            ...
                                            addCommsReply(
                                                _(
                                                    "doctor-comms",
                                                    "Your superb publication presentation"
                                                ),
                                                function()
                                                    if
                                                        random(1, 4)
                                                    ...
                                                    then
                                                        ...
                                                        addCommsReply(
                                                            _(
                                                                "doctor-comms",
                                                                "Of course! I've got a case study for you to examine"
                                                            ),
                                                            function()
                                                                if
                                                                    random(
                                                                ...
                                                                then
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "[Dr. Thultris] An application for my research?! Goody Goody!\n\nAs you both return, you explain the situation to her"
                                                                        )
                                                                    )
                                                                    player.doctorAboard =
                                                                        true
                                                                    ...
                                                                    setCommsMessage(
                                                                        _(
                                                                            "doctor-comms",
                                                                            "Sounds interesting, but not right now, thanks\n[Dr. Thultris closes her door]"
                                                                        )
                                                                    )
                                                                    doctorRejectionCount = doctorRejectionCount
                                                                        + 1
                ...
                if comms_target.characterDescription ~= nil then
                    ...
                    if comms_target.characterDeadEnd == nil then
                        ...
                        if deadEndChoice == 1 then
                            comms_target.characterDeadEnd = string.format(
            ...
            addCommsReply(
                ...
                function()
                    if not player:takeReputationPoints(hireCost) then
                        ...
                        player:setRepairCrewCount(
                            player:getRepairCrewCount() + 1
                        )
                        setCommsMessage(
    ...
    if comms_target:getFaction() == "Arlenians" then
        if scarceResources then
            addCommsReply(_("path-comms", "Where is Bespin?"), function()
                if
                    stationCloudCity ~= nil
                    and stationCloudCity:isValid()
                    and stationCloudCity:areEnemiesInRange(30000)
                then
                    setCommsMessage(
                        string.format(
                            _(
                                "path-comms",
                                "Bespin is in %s and enemies are within 30U of Cloud City"
                            ),
                            planetBespin:getSectorName()
                        )
                    )
                else
                    setCommsMessage(
                        string.format(
                            _("path-comms", "Bespin is in %s"),
                            planetBespin:getSectorName()
                        )
                    )
                end
    ...
    if goods[comms_target] ~= nil then
        addCommsReply(
            ...
            function()
                ...
                addCommsReply(
                    _(
                        "explainGoodsComponent-comms",
                        "Explain the component goods"
                    ),
                    explainComponents
                )
                addCommsReply(
                    _("explainGoodsMineral-comms", "Explain the mineral goods"),
                    explainMinerals
                )
                addCommsReply(
                    _("explainGoodsTrade-comms", "Explain the trade goods"),
                    explainTradeGoods
                )
                addCommsReply(
                    _("explainGoodsVapor-comms", "Explain vapor goods"),
                    function()
                        setCommsMessage(
                            _(
                                "explainGoodsVapor-comms",
                                "On rare occasions, the system cannot determine an appropriate good. In that case one of these 'vapor' or fake goods will be listed:\n   gold pressed latinum, unobtanium, eludium, impossibrium\n\nIf you see one of these, that means that the good is not available. The need or desire for that good will go unmet or the mission will have to be satisfied in some other fashion."
                            )
                        )
                        addCommsReply(_("Back"), commsStation)
                    end
            ...
            repeat
                ...
                addCommsReply(
                    string.format(
                        ...
                        goods[comms_target][gi][1],
                        goods[comms_target][gi][3]
                    ),
                    function()
                        oMsg = string.format(
                ...
                ) --end buy goods from station for player reputation comms reply branch
                gi = gi + 1
            until gi > #goods[comms_target]
            ...
            if tradeFood[comms_target] ~= nil then
                ...
                if foodQuantity > 0 then
                    ...
                    repeat
                        ...
                        ) --end trade food on player ship for goods on station comms reply branch
                        gi = gi + 1
                    until gi > #goods[comms_target]
            ...
            if tradeLuxury[comms_target] ~= nil then
                ...
                if luxuryQuantity > 0 then
                    ...
                    repeat
                        ...
                        ) --end trade luxury on player ship for goods on station comms reply branch
                        gi = gi + 1
                    until gi > #goods[comms_target]
            ...
            if tradeMedicine[comms_target] ~= nil then
                ...
                if medicineQuantity > 0 then
                    ...
                    repeat
                        ...
                        ) --end trade medicine on player ship for goods on station comms reply branch
                        gi = gi + 1
                    until gi > #goods[comms_target]
...
function explainComponents()
    local component_goods_explanation = {
        ["impulse"] = _(
            "explainGoodsComponent-comms",
            "Technicians use impulse components to fix or enhance impulse engines or related impulse systems."
        ),
        ["warp"] = _(
            "explainGoodsComponent-comms",
            "Engineers use warp components to fix or enhance warp engines, warp cores and/or other related warp technology based systems."
        ),
        ["shield"] = _(
            "explainGoodsComponent-comms",
            "Technicians use shield components to enhance or fix shield capacitors, shield charging systems and other shield or energy field generation systems."
        ),
        ["tractor"] = _(
            "explainGoodsComponent-comms",
            "Engineers and technicians use tractor components to repair or enhance systems designed to pull objects towards the ship."
        ),
        ["repulsor"] = _(
            "explainGoodsComponent-comms",
            "Technicians use repulsor components to repair or enhance systems designed to move objects away from the ship."
        ),
        ["beam"] = _(
            "explainGoodsComponent-comms",
            "Engineers and weapon systems repair specialists use beam components to fix, improve and/or tune beam weapons or related beam systems."
        ),
        ["optic"] = _(
            "explainGoodsComponent-comms",
            "Repair crew use optic components in a variety of ship systems. Optic components serve as the information pathways and linkages between many ship systems"
        ),
        ["robotic"] = _(
            "explainGoodsComponent-comms",
            "Engineers often use robotic components to facilitate missile loading. Other typical systems needing robotic components include medical bays, food preparation, automated repair systems, etc."
        ),
        ["filament"] = _(
            "explainGoodsComponent-comms",
            "Some of the esoteric ship systems have specialized material requirements. These are typically satisfied with a filament component of some kind"
        ),
        ["transporter"] = _(
            "explainGoodsComponent-comms",
            "Engineers use transporter components to fix systems designed to move objects from one location to another. The movement methods range from simple belt rollers to atomic disassembly and reassembly."
        ),
        ["sensor"] = _(
            "explainGoodsComponent-comms",
            "Repair crew use sensor components to fix or repair those systems that support the Science console in gathering information about the universe around the ship."
        ),
        ["communication"] = _(
            "explainGoodsComponent-comms",
            "Technicians use communication components to fix the various systems used to connect the ship to other ships, stations and/or alien entities on the Relay console."
        ),
        ["autodoc"] = _(
            "explainGoodsComponent-comms",
            "Technicians use autodoc components to repair or maintain the automated medical facilities used on many ships in place of a medical doctor or medical staff."
        ),
        ["lifter"] = _(
            "explainGoodsComponent-comms",
            "Engineers need lifter components for a variety of systems needing to temporarily move mechanisms or portions of mechanisms against the acceleration or gravity aboard a ship."
        ),
        ["android"] = _(
            "explainGoodsComponent-comms",
            "Engineers use android components to build, repair and maintain androids that serve on ships. Repair crews on many ships consist largely of androids."
        ),
        ["nanites"] = _(
            "explainGoodsComponent-comms",
            "Technicians and engineers use nanites in a number of systems including shield, reactor, impulse engines, jump drives and warp drives. The nanites are programmed to fix things at the molecular level, often in conditions hazardous to organic beings such as humans."
        ),
        ["software"] = _(
            "explainGoodsComponent-comms",
            "With all the automated systems on a ship, technicians often need software components to upgrade or adjust systems."
        ),
        ["circuit"] = _(
            "explainGoodsComponent-comms",
            "Many systems have specialized circuits that need upgrading or replacing via one or more circuit components."
        ),
        ["battery"] = _(
            "explainGoodsComponent-comms",
            "Batteries on a ship range in size from the ones that provide most of the energy to run the ship to tiny ones used in EVA space suits. Battery components help maintain and improve these systems."
        ),
    }
    setCommsMessage(
        _(
            "explainGoodsComponent-comms",
            "These are the typical component goods available. Which one would you like a greater explanation of?"
        )
    )
...
function explainTradeGoods()
    setCommsMessage(
        _(
            "explainGoodsTrade-comms",
            "These are the typical trade goods available. Which one would you like a greater explanation of?"
        )
    )
    local trade_goods_explanation = {
        ["food"] = _(
            "explainGoodsTrade-comms",
            "Many people want or need food to sustain themselves or to enjoy. A good Relay officer will try to keep at least one food on board for the potential trade benefit. When offered for sale, it's usually very cheap."
        ),
        ["medicine"] = _(
            "explainGoodsTrade-comms",
            "Many people need medicine to cure illness or symptoms associated with illness. Medicine is almost as cheap as food. The trade benefits are usually very high. A good Relay officer will try to keep at least on medicine on board for the potential trade benefit."
        ),
        ["luxury"] = _(
            "explainGoodsTrade-comms",
            "A luxury good is anything someone considers valuable. Luxury types include jewelry, specialized clothing, rare art, etc. The trade value of luxury is not as high as food or medicine, but can still give some trade benefit. A good Relay officer will be watching for relatively low priced luxury which can be used to trade for higher priced components or minerals."
        ),
    }
...
function explainMinerals()
    setCommsMessage(
        _(
            "explainGoodsMineral-comms",
            "These are the typical mineral goods available. Which one would you like a greater explanation of?"
        )
    )
    mineral_goods_explanation = {
        ["nickel"] = _(
            "explainGoodsMineral-comms",
            "Nickel is one of the most common metals found among asteroids. As such, it has become the defacto standard metal for most of the structure of ships."
        ),
        ["platinum"] = _(
            "explainGoodsMineral-comms",
            "Platinum has electronic, explosive and medical applications, so techniciams use it to repair and upgrade a variety of ship systems."
        ),
        ["gold"] = _(
            "explainGoodsMineral-comms",
            "Civilians and military technicians value gold for a variety of reasons including use in jewelry and electronics."
        ),
        ["dilithium"] = _(
            "explainGoodsMineral-comms",
            "Due to its high potential for energy storage, engineers value dilithium in ship engines and shields."
        ),
        ["tritanium"] = _(
            "explainGoodsMineral-comms",
            "Designers and engineers use tritanium in many aspects of ship construction due to how it interacts with warp fields and jump mechanics."
        ),
        ["cobalt"] = _(
            "explainGoodsMineral-comms",
            "Engineers value cobalt due to its magnetic, alloy and energy storage applications."
        ),
    }
...
function handleWeaponRestock(weapon)
    ...
    scarcityMsg = _(
        "ammo-comms",
        " The Kraylor threat to mineral resources has reduced ordnance availablilty"
    )
    if scarceResources then
        scarcityMsg = ""
        scarcity = 0.5
    ...
    local item_amount = math.floor(
        player:getWeaponStorageMax(weapon)
            * comms_data.max_weapon_refill_amount[getFriendStatus()]
            * scarcity
    ) - player:getWeaponStorage(weapon)
    if item_amount <= 0 then
        ...
        if player:getReputationPoints() > points_per_item * item_amount then
            if player:takeReputationPoints(points_per_item * item_amount) then
                player:setWeaponStorage(
                    weapon,
                    player:getWeaponStorage(weapon) + item_amount
                )
                if
                    player:getWeaponStorage(weapon)
                    == player:getWeaponStorageMax(weapon)
            ...
            if player:getReputationPoints() > points_per_item then
                ...
                addCommsReply(_("ammo-comms", "Get just one"), function()
                    if player:takeReputationPoints(points_per_item) then
                        player:setWeaponStorage(
                            weapon,
                            player:getWeaponStorage(weapon) + 1
                        )
                        if
                            player:getWeaponStorage(weapon)
                            == player:getWeaponStorageMax(weapon)
...
function handleUndockedState()
    ...
    addCommsReply(_("station-comms", "I need information"), function()
            ...
            addCommsReply(
                ...
                function()
                    setCommsMessage(
                        _(
                            "doomsday-comms",
                            "He's got nice retirement quarters here"
                        )
                    )
                    addCommsReply(
                        _("doomsday-comms", "May we speak with him?"),
                        function()
                            setCommsMessage(
                                _(
                                    "doomsday-comms",
                                    "He won't talk with anyone from the Human Navy. He's still bitter over early retirement. You'll need to visit in person"
                                )
                            )
                            addCommsReply(_("Back"), commsStation)
                        end
            ...
            addCommsReply(
                ...
                function()
                    ...
                    addCommsReply(
                        _(
                            "doctor-comms",
                            "We need her skills. May I speak with her?"
                        ),
                        function()
                            setCommsMessage(
                                _(
                                    "doctor-comms",
                                    "She refuses to come to the communications nexus except on her terms. This usually means only outgoing communications. There is a very short list of contacts she will respond to and you are not on that list. If you wish to speak with her, you will need to dock and visit in person"
                                )
                            )
                            addCommsReply(
                                _(
                                    "doctor-comms",
                                    "The Arlenian Admiral is critically ill and she could help"
                                ),
                                function()
                                    setCommsMessage(
                                        _(
                                            "doctor-comms",
                                            "There could be a deadly plague covering all of Human and Arlenian space and she would remain in her quarters tending to her research until the plague destroyed all other personnel on the station. Even then, it might take her a few days to realize everyone else was dead. You really will need to dock to talk to her"
                                        )
                                    )
                                    addCommsReply(_("Back"), commsStation)
                                end
        ...
        if goods[comms_target] ~= nil then
            ...
            repeat
                if goods[comms_target][gi][2] > 0 then
                    goodsQuantityAvailable = goodsQuantityAvailable
                        + goods[comms_target][gi][2]
                end
                gi = gi + 1
            until gi > #goods[comms_target]
        end
        ...
        addCommsReply(
            ...
            function()
                gkMsg = _(
                    "trade-comms",
                    "Friendly stations generally have food or medicine or both. Neutral stations often trade their goods for food, medicine or luxury."
                )
                if comms_target.goodsKnowledge == nil then
                    ...
                    if #comms_target.goodsKnowledge == 0 then
                        ...
                        for gk = 1, #comms_target.goodsKnowledge do
                            addCommsReply(
                                comms_target.goodsKnowledgeType[gk],
                                function()
                                    setCommsMessage(
                                        string.format(
                                            _(
                                                "trade-comms",
                                                "Station %s in sector %s has %s%s"
                                            ),
                                            comms_target.goodsKnowledge[gk],
                                            comms_target.goodsKnowledgeSector[gk],
                                            comms_target.goodsKnowledgeType[gk],
                                            comms_target.goodsKnowledgeTrade[gk]
                                        )
                                    )
                                    addCommsReply(_("Back"), commsStation)
                                end
    ...
    if isAllowedTo(comms_target.comms_data.services.supplydrop) then
        addCommsReply(
            string.format(
                _("stationAssist-comms", "Can you send a supply drop? (%i rep)"),
                math.floor(getServiceCost("supplydrop"))
            ),
            function()
                if player:getWaypointCount() < 1 then
                    ...
                    for n = 1, player:getWaypointCount() do
                        addCommsReply(
                            string.format(
                                _("stationAssist-comms", "Waypoint %d"),
                                comms_source:getWaypointID(n)
                            ),
                            function()
                                if
                                    player:takeReputationPoints(
                                        math.floor(getServiceCost("supplydrop"))
                                    )
                                then
                                    ...
                                    setCommsMessage(
                                        string.format(
                                            _(
                                                "stationAssist-comms",
                                                "We have dispatched a supply ship toward waypoint %d"
                                            ),
                                            comms_source:getWaypointID(n)
                                        )
                                    )
    ...
    if isAllowedTo(comms_target.comms_data.services.reinforcements) then
        addCommsReply(
            string.format(
                _(
                    "stationAssist-comms",
                    "Please send Adder MK5 reinforcements! (%i rep)"
                ),
                math.floor(getServiceCost("reinforcements"))
            ),
            function()
                if player:getWaypointCount() < 1 then
                    setCommsMessage(
                                ...
                                then
                                    ...
                                    setCommsMessage(
                                        string.format(
                                            _(
                                                "stationAssist-comms",
                                                "We have dispatched %s to assist at waypoint %d"
                                            ),
                                            ship:getCallSign(),
                                            comms_source:getWaypointID(n)
                                        )
        ...
        addCommsReply(
            string.format(
                _(
                    "stationAssist-comms",
                    "Please send Phobos T3 reinforcements! (%i rep)"
                ),
                math.floor(getServiceCost("phobosReinforcements"))
            ),
            function()
                if player:getWaypointCount() < 1 then
                    setCommsMessage(
        ...
        addCommsReply(
            string.format(
                _(
                    "stationAssist-comms",
                    "Please send Stalker Q7 reinforcements! (%i rep)"
                ),
                math.floor(getServiceCost("stalkerReinforcements"))
            ),
            function()
                if player:getWaypointCount() < 1 then
                    setCommsMessage(
...
function commsShip()
    ...
    if goods[comms_target] == nil then
        goods[comms_target] =
            { goodsList[irandom(1, #goodsList)][1], 1, random(20, 80) }
    end
    comms_data = comms_target.comms_data
    setPlayers()
    for p4idx = 1, 8 do
    ...
    if
        player:isEnemy(comms_target)
        and comms_target:isFriendOrFoeIdentifiedBy(player)
    then
    ...
    addCommsReply(_("shipAssist-comms", "Defend a waypoint"), function()
        if player:getWaypointCount() == 0 then
            ...
            for n = 1, player:getWaypointCount() do
                addCommsReply(
                    string.format(
                        _("shipAssist-comms", "Defend waypoint %d"),
                        comms_source:getWaypointID(n)
                    ),
                    function()
                        ...
                        setCommsMessage(
                            string.format(
                                _(
                                    "shipAssist-comms",
                                    "We are heading to assist at waypoint %d."
                                ),
                                comms_source:getWaypointID(n)
                            )
                        )
                                ...
                                function()
                                    if goodsQuantity < 1 then
                                        setCommsMessage(
                                            _(
                                                "trade-comms",
                                                "Insufficient inventory on freighter for trade"
                                            )
                                        )
                                    else
                            ...
                            function()
                                ...
                                elseif goodsQuantity < 1 then
                                    setCommsMessage(
                                        _(
                                            "trade-comms",
                                            "Insufficient inventory on freighter"
                                        )
                                    )
                                else
                                    if
                                        not player:takeReputationPoints(
                                            goodsRep
                                        )
                                    then
                                        setCommsMessage(
                                        ...
                                        incrementPlayerGoods(goodsType)
                                        setCommsMessage(
                                            _("trade-comms", "Purchased")
                                        )
                                    end
                                end
                                addCommsReply(_("Back"), commsShip)
                            end
                        ) --end sell goods comms reply branch
                        gi = gi + 1
                    until gi > #goods[comms_target]
            ...
            if distance(player, comms_target) < 5000 then
                ...
                then
                    ...
                    repeat
                        ...
                        ) --end buy goods from freighter comms reply branch
                        gi = gi + 1
                    until gi > #goods[comms_target]
            ...
            if distance(player, comms_target) < 5000 then
                ...
                then
                    ...
                    if luxuryQuantity > 0 then
                        ...
                        setCommsMessage(
                            _("trade-comms", "Insufficient luxury to trade")
                        )
                    end
                    addCommsReply(_("Back"), commsShip)
                else -- Offer to sell goods
        ...
        elseif comms_data.friendlyness > 33 then
            ...
            addCommsReply(
                string.format(
                    _(
                        "trade-comms",
                        "Where are you headed? (cost: %.1f reputation)"
                    ),
                    destRep
                ),
                function()
...
function placeRandomListAroundPoint(
    object_type,
    amount,
    dist_min,
    dist_max,
    ...
    if onlyOneMission then
        if #plotList < maxPlotCount then
            missionCompleteReason =
                _("msgMainscreen", "Single selected mission completed")
            missionVictory = true
            endStatistics()
            victory("Human Navy")
        else
...
function startDoomsday()
    ...
    for pidx = 1, 8 do
        ...
        if p ~= nil and p:isValid() then
            p:addToShipLog(
                _(
                    "doomsdayGoal-shipLog",
                    "Intelligence tells us the Exuari have nearly completed a 'doomsday' device. Retired scientist, Gregory Unruh warned us before he retired about this possibility, but we did not take him seriously. He was working on a defense mechanism. Find professor Unruh and get the plans for his defense mechanism"
                ),
                "Magenta"
            )
        end
    ...
    primaryOrders = _(
        "doomsdayOrders-comms",
        "Find professor Unruh for doomsday defense mechanism plans"
    )
    ...
    until (professorStation ~= nil and professorStation:isValid())
        or professorStationAttemptCount > repeatExitBoundary
    if professorStationAttemptCount > repeatExitBoundary then
        if professorDiagnostic then
            print("bad professor station")
        end
        plot1 = nil
        primaryOrders = ""
    else
        if professorDiagnostic then
            print("professor station set")
        end
    ...
    for i = 1, 3 do
        ...
        until (
                clueStation ~= nil
                and clueStation:isValid()
                and clueStation ~= professorStation
                and (
                    clueStation.character == nil
                    or clueStation.character == "Morrigan Thultris"
                )
            ) or clueStationAttemptCount > repeatExitBoundary
        if clueStationAttemptCount < repeatExitBoundary then
            clueStation.character = "Gregory Unruh"
            clueStation.characterDescription = string.format(
                _("doomsday-comms", "Gregory Unruh resides on station %s"),
                professorStation:getCallSign()
            )
        end
    end
    if professorDiagnostic then
        print("done with doomsday setup")
    end
...
function checkDoomsdayEvents(delta)
    ...
    if doomsdayTimer < quarterDoom then
        if exuariRampage == nil then
            ...
            repeat
                humanPillageTarget =
                    humanStationList[math.random(1, #humanStationList)]
                pillageAttempt = pillageAttempt + 1
            until (
                    humanPillageTarget ~= nil and humanPillageTarget:isValid()
                ) or pillageAttempt > repeatExitBoundary
            if pillageAttempt <= repeatExitBoundary then
                local tx, ty = humanPillageTarget:getPosition()
                local ox, oy =
                    vectorFromAngle(random(0, 360), random(20000, 30000))
                ...
                repeat
                    ...
                    pillageAttempt = pillageAttempt + 1
                until (
                        humanPillageTarget2 ~= nil
                        and humanPillageTarget2:isValid()
                        and humanPillageTarget ~= humanPillageTarget2
                    )
                    or pillageAttempt > repeatExitBoundary
                if pillageAttempt <= repeatExitBoundary then
                ...
                if p ~= nil and p:isValid() then
                    p:addToShipLog(
                        string.format(
                            _(
                                "doomsday-shipLog",
                                "Intelligence estimates %i minutes remain before the Exuari deploy the doomsday device. Exuari and Kraylors both seem to be on a pillaging rampage"
                            ),
                            math.floor(doomsdayTimer / 60)
                        ),
                        "Magenta"
                    )
                end
    ...
    if professorProvidedPlans then
        if componentsPicked then
            primaryOrders = string.format(
                _(
                    "doomsdayOrders-comms",
                    "Get %s, %s and %s to build doomsday defense mechanism"
                ),
                component1,
                component2,
                component3
                    ...
                    if p ~= nil and p:isValid() and p.professor then
                        p:addToShipLog(
                            string.format(
                                _(
                                    "doomsday-shipLog",
                                    "The instructions list %s, %s and %s as necessary components"
                                ),
                                component1,
                                component2,
                                component3
                    ...
                    if p ~= nil and p:isValid() and p.professor then
                        p:addToShipLog(
                            string.format(
                                _(
                                    "doomsday-shipLog",
                                    "Further inspection of Gregory Unruh's plans reveal notes in the margin: %s on %s, %s on %s, %s on %s"
                                ),
                                component1,
                                component1Base:getCallSign(),
                                component2,
                                component2Base:getCallSign(),
                                component3,
                                component3Base:getCallSign()
                            ),
                            "Magenta"
                        )
                    end
                ...
                if p ~= nil and p:isValid() and p.professor then
                    ...
                    then
                        ...
                        p:addToShipLog(
                            _(
                                "doomsday-shipLog",
                                "With all three components obtained, Engineering and Science quickly complete and deploy the doomsday device defense mechanism"
                            ),
                            "Magenta"
                        )
                        plot1 = nil
    ...
    if doomsdayTimer < 0 then
        if doomsdayDeployed then
            ...
            repositionDelay = delta + 5
            if professorDiagnostic then
                print("deploy doomsday device")
            end
            doomsdayBlackHole = BlackHole()
            doomsdayBlackHole.target =
                humanStationList[math.random(1, #humanStationList)]
            doomsdayDeployed = true
        end
    end
    local doomsday_status =
        _("doomsday-tabEngineer&Engineer+", "Doomsday Device:")
    if doomsdayTimer < 0 then
        doomsday_status =
            _("doomsday-tabEngineer&Engineer+", "Doomsday Device Built")
    else
        ...
        if doomsday_minutes <= 0 then
            doomsday_status = string.format(
                _("doomsday-tabEngineer&Engineer+", "%s %i"),
                doomsday_status,
                doomsday_seconds
            )
        else
            doomsday_status = string.format(
                _("doomsday-tabEngineer&Engineer+", "%s %i:%.2i"),
                doomsday_status,
                doomsday_minutes,
                doomsday_seconds
            )
...
function getNextDoomsdayTarget()
    if professorDiagnostic then
        print("pick next doomsday target")
    end
    ...
    if professorDiagnostic then
        print("use fresh human station list")
    end
    humanStationList = tempHumanStationList
    if #humanStationList < 1 then
        missionCompleteReason =
            _("doomsday-msgMainscreen", "All human stations destroyed")
        ...
        doomsdayBlackHole.target =
            nearStations(doomsdayBlackHole, humanStationList)
    end
    if professorDiagnostic then
        print("done picking next doomsday target")
    end
end
    ...
    repeat
        ...
        if candidate ~= nil and candidate:isValid() then
            if #goods[candidate] > 0 then
                ...
                repeat
                    if
                        goods[candidate][gi][1] ~= "food"
                        and goods[candidate][gi][1] ~= "medicine"
                        and goods[candidate][gi][1] ~= "luxury"
                    then
                ...
                if #goods[candidate] > 0 then
                    ...
                    repeat
                        if
                            ...
                            and goods[candidate][gi][1] ~= "medicine"
                            and goods[candidate][gi][1] ~= "luxury"
                            and goods[candidate][gi][1] ~= component1
                        then
                    ...
                    if #goods[candidate] > 0 then
                        ...
                        repeat
                            if
                                ...
                                and goods[candidate][gi][1] ~= "luxury"
                                and goods[candidate][gi][1] ~= component1
                                and goods[candidate][gi][1] ~= component2
                            then
...
function startSickArlenianAdmiral()
    ...
    primaryOrders = _(
        "doctorOrders-comms",
        "Find Dr. Thultris for help with Arlenian Admiral Koshenz"
    )
    ...
    until (doctorStation ~= nil and doctorStation:isValid())
        or doctorStationAttemptCount > repeatExitBoundary
    if doctorStationAttemptCount > repeatExitBoundary then
        if sickAdmiralDiagnostic then
            print("bad doctor station")
        end
        plot1 = nil
        primaryOrders = ""
    else
        if sickAdmiralDiagnostic then
            print("doctor station set")
        end
        doctorSearch = true
    ...
    for i = 1, 3 do
        ...
        until (
                clueStation ~= nil
                and clueStation:isValid()
                and clueStation ~= doctorStation
                and (
                    clueStation.character == nil
                    or clueStation.character == "Gregory Unruh"
                )
            ) or clueStationAttemptCount > repeatExitBoundary
        if clueStationAttemptCount < repeatExitBoundary then
            clueStation.character = "Morrigan Thultris"
            clueStation.characterDescription = string.format(
                _("doctor-comms", "Dr. Thultris resides on station %s"),
                doctorStation:getCallSign()
            )
        end
    end
    if sickAdmiralDiagnostic then
        print("done with sick admiral setup")
    end
...
function checkSickArlenianAdmiralEvents(delta)
    ...
    if admiralTimeToLive < 0 then
        missionCompleteReason =
            _("doctor-msgMainscreen", "Arlenian Admiral Koshenz dies")
        missionVictory = false
        endStatistics()
        victory("Kraylor")
    elseif not arlenianFleet2base:isValid() then
        missionCompleteReason = string.format(
            _(
                "doctor-msgMainscreen",
                "Arlenian Admiral Koshenz dies along with the rest of the Arlenians on station %s"
            ),
            arlenian_fleet_base_2_name
        )
        missionVictory = false
        endStatistics()
        victory("Kraylor")
        ...
        for pidx = 1, 8 do
            ...
            if p ~= nil and p:isValid() then
                p:addToShipLog(
                    _(
                        "doctor-shipLog",
                        "The Arlenians say that they think the admiral has less than three minutes to live"
                    ),
                    "Magenta"
                )
            end
    ...
    if admiralTimeToLive < 250 and kraylorAfterAdmiral == nil then
        ...
        local kafx, kafy = vectorFromAngle(
            random(borderStartAngle + 240, borderStartAngle + 270),
            random(20000, 30000) - (7000 * difficulty)
        )
        local aasx, aasy = arlenianFleet2base:getPosition()
        menaceAdmiralFleet =
            spawnEnemies(aasx + kafx, aasy + kafy, 2, "Kraylor")
            ...
            if p ~= nil and p:isValid() then
                p:addToShipLog(
                    string.format(
                        _(
                            "doctor-shipLog",
                            "The Kraylors seem to have discovered the location of Admiral Koshenz. We detect Kraylor ships en route to %s"
                        ),
                        arlenianFleet2base:getCallSign()
                    ),
                    "Magenta"
                )
            end
    ...
    if doctorAssisting then
        if admiralLocationMessage == nil then
            ...
            if arlenianFleet2base ~= nil and arlenianFleet2base:isValid() then
                    ...
                    if p ~= nil and p:isValid() then
                        p:addToShipLog(
                            string.format(
                                _(
                                    "doctor-shipLog",
                                    "Admiral Koshenz is on %s in %s"
                                ),
                                arlenianFleet2base:getCallSign(),
                                arlenianFleet2base:getSectorName()
                            ),
                            "Magenta"
                        )
                    end
                ...
                primaryOrders = string.format(
                    _("doctorOrders-comms", "Take Dr. Thultris to %s in %s"),
                    arlenianFleet2base:getCallSign(),
                    arlenianFleet2base:getSectorName()
                )
            end
            ...
            if p ~= nil and p:isValid() and p.doctorAboard then
                if p:isDocked(arlenianFleet2base) then
                    p:addToShipLog(
                        _(
                            "doctor-shipLog",
                            "Dr. Thultris applied her knowledge and Admiral Koshenz is improving"
                        ),
                        "Magenta"
                    )
                    plot1 = nil
    ...
    if death_minutes <= 0 then
        admiral_status = string.format(
            _("doctor-tabScience&Operations", "%s %i"),
            admiral_status,
            death_seconds
        )
    else
        admiral_status = string.format(
            _("doctor-tabScience&Operations", "%s %i:%.2i"),
            admiral_status,
            death_minutes,
            death_seconds
        )
...
function startMiningConflict()
    ...
    for pidx = 1, 8 do
        ...
        if p ~= nil and p:isValid() then
            p:addToShipLog(
                _(
                    "BespinGoal-shipLog",
                    "Our allies, the Arlenians tell us that the Kraylors are attacking the mining facility around the planet Bespin. We rely heavily on that mineral supply for commerce and military operations. Contact the Arlenians and render assistance"
                ),
                "Magenta"
            )
        end
    ...
    primaryOrders = _(
        "BespinOrders-comms",
        "Contact Arlenians and render assistance with Kraylor attack"
    )
    bespinX, bespinY = vectorFromAngle(
        random(borderStartAngle - 30, borderStartAngle + 30),
        random(100000, 130000)
    )
    planetBespin = Planet()
        :setPosition(bespinX, bespinY)
        :setPlanetRadius(2700)
        :setDistanceFromMovementPlane(-2000)
        :setCallSign("Bespin")
    planetBespin
        :setPlanetSurfaceTexture("planets/gas-1.png")
        :setAxialRotationTime(300)
        :setDescription(
            _("scienceDescription-planet", "Gas giant suitable for mining")
        )
    stationCloudCity = SpaceStation()
        :setTemplate("Small Station")
        :setFaction("Arlenians")
        :setCommsScript("")
        :setCommsFunction(commsStation)
    stationCloudCity
        :setPosition(bespinX, bespinY + 3500)
        :setCallSign("Cloud City")
        :setDescription(_("scienceDescription-station", "Bespin Gas Mining"))
    ...
    if miningConflictDiagnostic then
        print("mining planet and station established")
    end
    local kmfx, kmfy = vectorFromAngle(
        random(borderStartAngle + 200, borderStartAngle + 250),
        random(30000, 50000)
    )
    menaceMiningFleet =
        spawnEnemies(bespinX + kmfx, bespinY + kmfy, 2, "Kraylor")
    if miningConflictDiagnostic then
        print("mining menace Kraylor fleet established")
    end
    for _, enemy in ipairs(menaceMiningFleet) do
        enemy:orderFlyTowards(bespinX, bespinY)
    end
    if miningConflictDiagnostic then
        print("menacing fleet in flight")
    end
    ...
    if miningConflictDiagnostic then
        print("Arlenian fleet in flight")
    end
    scarceResources = true
    if miningConflictDiagnostic then
        print("Done with start mining conflict function")
    end
    removeGMFunction(GMMining)
end
...
function checkMiningConflictEvents(delta)
    if stationCloudCity ~= nil and stationCloudCity:isValid() then
        if exuariMiningOpportunity == nil then
            if miningConflictDiagnostic then
                print("exuari mining opportunity")
            end
            closestToCC = closestPlayerTo(stationCloudCity)
            if miningConflictDiagnostic then
                print("closest player chosen")
            end
            if distance(closestToCC, stationCloudCity) < 30000 then
                if miningConflictDiagnostic then
                    print("player close enough to trigger Exuari activity")
                end
                ...
                humanMiningTargetStation =
                    nearStations(exuariFleet2base, humanStationList)
                    ...
                    for pidx = 1, 8 do
                        ...
                        if p ~= nil and p:isValid() then
                            p:addToShipLog(
                                _(
                                    "Bespin-shipLog",
                                    "Looks like the Exuari are taking advantage of the Kraylor mining incident by sending forces against us and the Arlenians"
                                ),
                                "Magenta"
                            )
                        end
            ...
            for pidx = 1, 8 do
                ...
                if p ~= nil and p:isValid() then
                    p:addToShipLog(
                        _(
                            "Bespin-shipLog",
                            "The Arlenians thanked us for the help with the Kraylors around Cloud City"
                        ),
                        "Magenta"
                    )
                end
        ...
        missionCompleteReason =
            _("Bespin-msgMainscreen", "Mining station Cloud City destroyed")
        missionVictory = false
        endStatistics()
        victory("Kraylor")
    end
...
function setPlayers()
    for i, p in ipairs(getActivePlayerShips()) do
        ...
        if not p.nameAssigned then
            ...
            p:setCallSign(
                tableRemoveRandom(player_ship_names[temp_player_type])
            )
...
function tableRemoveRandom(array)
    ...
    array[selected_item], array[array_item_count] =
        array[array_item_count], array[selected_item]
...
function listStatuses()
    local humanCountPercentage, humanValuePercentage, kraylorCountPercentage, kraylorValuePercentage, exuariCountPercentage, exuariValuePercentage, arlenianCountPercentage, arlenianValuePercentage, neutralCountPercentage, neutralValuePercentage =
        stationStatus()
    ...
    for j = 1, #humanFleetList do
        ...
        for _, tempEnemy in ipairs(tempFleet) do
            if tempEnemy ~= nil and tempEnemy:isValid() then
                humanMilitaryShipSurvivedCount = humanMilitaryShipSurvivedCount
                    + 1
                for k = 1, #stnl do
                    if tempEnemy:getTypeName() == stnl[k] then
                        humanMilitaryShipSurvivedValue = humanMilitaryShipSurvivedValue
                            + stsl[k]
                    end
                end
            end
    ...
    for j = 1, #kraylorFleetList do
        ...
        for _, tempEnemy in ipairs(tempFleet) do
            if tempEnemy ~= nil and tempEnemy:isValid() then
                kraylorMilitaryShipSurvivedCount = kraylorMilitaryShipSurvivedCount
                    + 1
                for k = 1, #stnl do
                    if tempEnemy:getTypeName() == stnl[k] then
                        kraylorMilitaryShipSurvivedValue = kraylorMilitaryShipSurvivedValue
                            + stsl[k]
                    end
                end
            end
    ...
    for j = 1, #exuariFleetList do
        ...
        for _, tempEnemy in ipairs(tempFleet) do
            if tempEnemy ~= nil and tempEnemy:isValid() then
                exuariMilitaryShipSurvivedCount = exuariMilitaryShipSurvivedCount
                    + 1
                for k = 1, #stnl do
                    if tempEnemy:getTypeName() == stnl[k] then
                        exuariMilitaryShipSurvivedValue = exuariMilitaryShipSurvivedValue
                            + stsl[k]
                    end
                end
            end
    ...
    for j = 1, #arlenianFleetList do
        ...
        for _, tempEnemy in ipairs(tempFleet) do
            if tempEnemy ~= nil and tempEnemy:isValid() then
                arlenianMilitaryShipSurvivedCount = arlenianMilitaryShipSurvivedCount
                    + 1
                for k = 1, #stnl do
                    if tempEnemy:getTypeName() == stnl[k] then
                        arlenianMilitaryShipSurvivedValue = arlenianMilitaryShipSurvivedValue
                            + stsl[k]
                    end
                end
            end
    ...
    local humanMilitaryShipValuePercentage = humanMilitaryShipSurvivedValue
        / rawHumanShipStrength
        * 100
    local kraylorMilitaryShipValuePercentage = kraylorMilitaryShipSurvivedValue
        / rawKraylorShipStrength
        * 100
    local exuariMilitaryShipValuePercentage = exuariMilitaryShipSurvivedValue
        / rawExuariShipStrength
        * 100
    local arlenianMilitaryShipValuePercentage = arlenianMilitaryShipSurvivedValue
        / rawArlenianShipStrength
        * 100
    return humanCountPercentage,
        humanValuePercentage,
        kraylorCountPercentage,
        kraylorValuePercentage,
        exuariCountPercentage,
        exuariValuePercentage,
        arlenianCountPercentage,
        arlenianValuePercentage,
        neutralCountPercentage,
        neutralValuePercentage,
        humanMilitaryShipValuePercentage,
        kraylorMilitaryShipValuePercentage,
        exuariMilitaryShipValuePercentage,
        arlenianMilitaryShipValuePercentage
end
function stationStatus()
    ...
    local percentages = {
        ["Human Navy"] = {
            count = 0,
            value = 0,
            count_base = #humanStationList,
            strength_base = humanStationStrength,
        },
        ["Kraylor"] = {
            count = 0,
            value = 0,
            count_base = #kraylorStationList,
            strength_base = kraylorStationStrength,
        },
        ["Exuari"] = {
            count = 0,
            value = 0,
            count_base = #exuariStationList,
            strength_base = exuariStationStrength,
        },
        ["Arlenians"] = {
            count = 0,
            value = 0,
            count_base = #arlenianStationList,
            strength_base = arlenianStationStrength,
        },
        ["Independent"] = {
            count = 0,
            value = 0,
            count_base = #neutralStationList,
            strength_base = neutralStationStrength,
        },
    }
    for i, station in ipairs(stationList) do
        if station:isValid() then
            counts[station:getFaction()] = counts[station:getFaction()] + 1
            values[station:getFaction()] = values[station:getFaction()]
                + strengths[station:getTypeName()]
        end
    end
...
function endStatistics()
    if endStatDiagnostic then
        print("starting end statistics")
    end
    ...
    if type(humanCountPercentage) == "number" then
        human_stations = string.format(
            _("msgMainscreen", "Human stations: survived:%.1f%%"),
            humanCountPercentage
        )
    end
    if type(humanValuePercentage) == "number" then
        if human_stations ~= "" then
            human_stations = string.format(
                _("msgMainscreen", "%s, strength:%.1f%%"),
                human_stations,
                humanValuePercentage
            )
        else
            human_stations = string.format(
                _("msgMainscreen", "Human stations: strength:%.1f%%"),
                humanValuePercentage
            )
        end
    end
    ...
    if type(arlenianCountPercentage) == "number" then
        arlenian_stations = string.format(
            _("msgMainscreen", "Arlenian stations: survived:%.1f%%"),
            arlenianCountPercentage
        )
    end
    if type(arlenianValuePercentage) == "number" then
        if arlenian_stations ~= "" then
            ...
            arlenian_stations = string.format(
                _("msgMainscreen", "Arlenian stations: strength:%.1f%%"),
                arlenianValuePercentage
            )
        end
    end
    ...
    if type(kraylorCountPercentage) == "number" then
        kraylor_stations = string.format(
            _("msgMainscreen", "Kraylor stations: survived:%.1f%%"),
            kraylorCountPercentage
        )
    end
    if type(kraylorValuePercentage) == "number" then
        if kraylor_stations ~= "" then
            ...
            kraylor_stations = string.format(
                _("msgMainscreen", "Kraylor stations: strength:%.1f%%"),
                kraylorValuePercentage
            )
        end
    end
    ...
    if type(exuariCountPercentage) == "number" then
        exuari_stations = string.format(
            _("msgMainscreen", "Exuari stations: survived:%.1f%%"),
            exuariCountPercentage
        )
    end
    if type(exuariValuePercentage) == "number" then
        if exuari_stations ~= "" then
            ...
            exuari_stations = string.format(
                _("msgMainscreen", "Exuari stations: strength:%.1f%%"),
                exuariValuePercentage
            )
        end
    end
    ...
    if type(neutralCountPercentage) == "number" then
        neutral_stations = string.format(
            _("msgMainscreen", "Neutral stations: survived:%.1f%%"),
            neutralCountPercentage
        )
    end
    if type(neutralValuePercentage) == "number" then
        if neutral_stations ~= "" then
            ...
            neutral_stations = string.format(
                _("msgMainscreen", "Neutral stations: strength:%.1f%%"),
                neutralValuePercentage
            )
        end
    end
    ...
    if type(humanMilitaryShipValuePercentage) == "number" then
        human_ships = string.format(
            _("msgMainscreen", "Human ships: strength:%.1f%%"),
            humanMilitaryShipValuePercentage
        )
    end
    local arlenian_ships = ""
    if type(arlenianMilitaryShipValuePercentage) == "number" then
        arlenian_ships = string.format(
            _("msgMainscreen", "Arlenian ships: strength:%.1f%%"),
            arlenianMilitaryShipValuePercentage
        )
    end
    local alliedValue = humanValuePercentage / 100 * 0.5
        + arlenianValuePercentage / 100 * 0.25
        + humanMilitaryShipValuePercentage / 100 * 0.17
        + arlenianMilitaryShipValuePercentage / 100 * 0.08
    local enemyValue = kraylorValuePercentage / 100 * 0.35
        + exuariValuePercentage / 100 * 0.35
        + kraylorMilitaryShipValuePercentage / 100 * 0.15
        + exuariMilitaryShipValuePercentage / 100 * 0.15
    local rankVal = alliedValue * 0.5 + (1 - enemyValue) * 0.5
    local reason_for_mission_end = ""
    if missionCompleteReason ~= nil then
        reason_for_mission_end = string.format(
            _("msgMainscreen", "Mission ended because %s"),
            missionCompleteReason
        )
    end
    local rank_weight = {
        { rank = _("msgMainscreen", "Ensign"), v_thresh = 0.7, d_thresh = 0.6 },
        {
    ...
    globalMessage(gMsg)
    if endStatDiagnostic then
        print("sent to the global message function")
    end
end
