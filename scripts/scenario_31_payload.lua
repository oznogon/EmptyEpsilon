...
function init()
    ...
    FactionStation = SpaceStation()
        :setTemplate("Large Station")
        :setFaction("Human Navy")
        :setPosition(0, 0)
        :setCallSign(_("mission", "Your Stn"))
    ...
    KraylorStation = SpaceStation()
        :setTemplate("Large Station")
        :setFaction("Kraylor")
        :setPosition(kx, ky)
        :setCallSign(_("mission", "Kraylor Stn"))
    ...
    PayloadShip = CpuShip()
        ...
        :setCallSign("Flowers")
        :setImpulseMaxSpeed(95)
        :setCommsFunction(PayloadCommsHandler)
    ...
    for __ = 1, Resistance.minesToSpawn do
        local x, y = RandPositionInRadius(
            MidX,
            MidY,
            FieldSize / 2 - 5000,
            FieldSize / 2 - 40000,
    ...
    Player = PlayerSpaceship()
        ...
        :setCallSign("Screamin' Firehawk")
        :setPosition(fsX + 1000, fsY + 1000)
    ...
    local payloadStartSector =
        getSectorName(PayloadShip.startX, PayloadShip.startY)
    FactionStation:sendCommsMessage(
        Player,
        _(
            "mission",
...
function ThinPath(start, finish, stepSize, maxAsteroids, maxMines)
    ...
    local angleBetween =
        math.rad(math.floor(CalculateAngle(startX, startY, finishX, finishY)))
    ...
    while distance(startX, startY, finishX, finishY) > stepSize do
        ...
        waypointX = waypointX
            + irandom(math.floor(-stepSize / 4), math.floor(stepSize / 4))
        waypointY = waypointY
            + irandom(math.floor(-stepSize / 4), math.floor(stepSize / 4))
        ClearHazardsNear(
            waypointX,
...
function InitTraffic()
    ...
    Traffic.stations =
        { HVLIStation, HomingStation, EMPStation, MineStation, NukeStation }
    ...
    for __ = 1, Resistance.pickupArtifacts do
        for __, weaponType in ipairs(weaponTypes) do
            ...
            local artifact = Artifact()
                :setPosition(x, y)
                :setDescriptions(
                    _("artifacts", "Something to salvage"),
                    _("artifacts", "It looks like a " .. weaponType)
                )
            ...
            artifact:onCollision(function(self, collider)
                ...
                local __ = math.abs(0)
                if
                    collider.components and collider.components.player_control
                then
                    collider:setWeaponStorage(
                        weaponType,
                        collider:getWeaponStorage(weaponType) + 1
                    )
                    self:destroy()
                    collider:addToShipLog(
                        _("artifacts", "Picked up a ") .. weaponType,
                        "green"
                    )
...
function CheckPayloadControl(target, faction1, faction2)
    ...
    if
        Player:isValid()
        and distance(Player, px, py) <= PayloadDistanceThreshold
    then
    ...
    for _, list in ipairs({ KraylorShips, HumanShips }) do
        for _, ship in ipairs(list) do
            if
                ship:isValid()
                and distance(ship, px, py) <= PayloadDistanceThreshold
            then
...
function CommsFactory(target, weaponType, price)
    ...
    function StationCommsHandler(comms_source, comms_target)
        if not comms_source:isDocked(comms_target) then
            setCommsMessage(
                string.format(
                    _("station-comms", "We sell %s for %d"),
                    weaponType,
                    price
                )
            )
        ...
        addCommsReply(
            string.format(
                _("station-comms", "Buy full complement of %s (%d rep)"),
                weaponType,
                price
            ),
            function()
                if comms_source:getReputationPoints() >= price then
                    comms_source:setReputationPoints(
                        comms_source:getReputationPoints() - price
                    )
                    comms_source:setWeaponStorage(
                        weaponType,
                        comms_source:getWeaponStorageMax(weaponType)
                    )
                    setCommsMessage(
                        string.format(
                            _(
                                "station-comms",
                                "You have purchased a full complement of %s."
                            ),
                            weaponType
                        )
                    )
                    ...
                    setCommsMessage(
                        _(
                            "station-comms",
                            "You do not have enough reputation points."
                        )
                    )
                end
            end
        ...
        addCommsReply(
            _("station-comms", "Gossip with the locals about salvage"),
            function()
                if #Pickups.artifacts > 0 then
                    ...
                    setCommsMessage(
                        string.format(
                            _(
                                "station-comms",
                                "We've heard about some shiny things in sector %s."
                            ),
                            sector
                        )
                    )
                    ...
                    setCommsMessage(
                        _(
                            "station-comms",
                            "No one seems to know of any more salvage to be had."
                        )
                    )
                end
            end
...
function GeneratePayloadPath(target)
    ...
    local angleBetween =
        math.floor(CalculateAngle(payloadX, payloadY, targetX, targetY))
    ...
    for i = 1, steps do
        local waypointX = payloadX
            + stepX * i
            + math.random(-wayPointDistance, wayPointDistance) / i
        local waypointY = payloadY
            + stepY * i
            + math.random(-wayPointDistance, wayPointDistance) / i
        ...
        waypointX, waypointY = RandPositionInRadius(
            startX,
            startY,
            wayPointDistance,
            wayPointDistance,
...
function IssueOrders(shipList, oppShipList, closestEnemy)
    for _, ship in ipairs(shipList) do
        ...
        if
            ship:getOrder() ~= "Attack"
            and ship:getOrder() ~= "Fly in formation"
            and ship:getOrder() ~= "Idle"
        then
            return
        end
        ...
        if
            ship:getOrder() == "Attack"
            and (
                ship:getOrderTarget() == nil
                or not ship:getOrderTarget():isValid()
            )
        then
...
function OnDamaged(self, instigator)
    ...
    if
        instigator ~= nil
        and ((instigator:getFaction() == "Humany Navy" and self:getFaction() == "Kraylor") or (instigator:getFaction() == "Kraylor" and self:getFaction() == "Humany Navy"))
        and (
            not self.lastAggroSwitch
            or getScenarioTime() - self.lastAggroSwitch > 20
        )
    then
...
function OrdersCloseToPayload(ship, closestEnemy, oppShipList)
    ...
    if
        ship:getFaction() ~= Player:getFaction()
        and distance(Player, PayloadShip) < 2.5 * PayloadDistanceThreshold
    then
        SafeOrder(ship, Player, "Attack")
    -- if another enemy is close to payload, attack them
    elseif
        closestEnemy ~= nil
        and closestEnemy:isValid()
        and distance(closestEnemy, PayloadShip)
            < 2.5 * PayloadDistanceThreshold
    then
...
function OrdersContestedPayload(ship, closestEnemy, oppShipList)
    ...
    elseif
        closestEnemy ~= nil
        and closestEnemy:isValid()
        and distance(ship, PayloadShip)
            < distance(closestEnemy, PayloadShip)
    then
        SafeOrder(ship, PayloadShip, "Fly in formation")
...
function PayloadCommsHandler(comms_source, comms_target)
    if PayloadShip.target ~= nil then
        ...
        setCommsMessage(
            _("payload-comms", "No target set for the payload ship.")
        )
    end
end
...
function SafeOrder(ship, target, order, changeTarget)
    ...
    if
        ship == nil
        or target == nil
        or not (ship:isValid() and target:isValid())
    then
        return false
    end
    ...
    if order == "Attack" then
        ...
        if
            not (
                ship:getOrder() == "Attack"
                and ship:getOrderTarget() ~= target
            ) or changeTarget
        then
...
function SpawnWave(faction)
    ...
    for i = 1, WaveSize do
        ...
        if
            (faction == "Kraylor" or faction == "Both")
            and spawnKraylorCount > 0
        then
        ...
        if
            (faction == "Human Navy" or faction == "Both")
            and spawnHumanCount > 0
        then
            spawnHumanCount = spawnHumanCount - 1
            local human = CpuShip()
                :setTemplate("Adder MK" .. irandom(3, 8))
                :setFaction("Human Navy")
                :setPosition(humanX, humanY)
...
function DeterminePayloadTarget()
    ...
    local inControl, ourCount, oppositionCount =
        CheckPayloadControl(PayloadShip, "Human Navy", "Kraylor")
    ...
    if PayloadShip:isScannedBy(Player) and PayloadShip.scanACKd == false then
        if distance(PayloadShip, Player) > PayloadDistanceThreshold then
            Player:addToShipLog(
                _(
                    "payload-comms",
                    "You need to be closer to the Payload to guide it."
                ),
                "red"
            )
        ...
        elseif inControl == 1 then
            ...
            Player:addToShipLog(
                string.format(
                    _(
                        "payload-comms",
                        "Must be clear of enemies before the Payload can move. The enemy has %d ships nearby."
                    ),
                    oppositionCount
                ),
                "red"
    ...
    if PayloadShip.target ~= nil and PayloadShip.target ~= oldTarget then
        ...
        if PayloadShip.target == FactionStation then
            Player:addToShipLog(
                _(
                    "payload-comms",
                    "DANGER, The payload is moving towards the Human Navy station!"
                ),
                "red"
            )
        elseif PayloadShip.target == KraylorStation then
            Player:addToShipLog(
                _(
                    "payload-comms",
                    "Payload is being delivered to the Kraylor station."
                ),
                "green"
            )
        end
...
function HandleNPCs(delta)
    ...
    if
        (
            ...
            or ClosestEnemies.Kraylor.ship == nil
        )
        or not (
            ClosestEnemies.HumanNavy.ship:isValid()
            and ClosestEnemies.Kraylor.ship:isValid()
        )
    then
...
function HandleNPCWaves(delta)
    ...
    if
        PayloadShip.controlledBy == "Human Navy"
        and PayloadShip.LastControlledBy ~= "Human Navy"
    then
        local timer = Resistance.turnoverWaveDelay
        if rand > 0 then
            timer = timer
                + irandom(math.floor(-timer * rand), math.floor(timer * rand))
        end
    ...
    local willSpawnIfTime = (
        PayloadShip.controlledBy ~= nil or #KraylorShips == 0
    )

    -- Display wave timer
    if string.find(getScenarioSetting("WaveTimer"), "Enabled") then
        local waveTimerMsg =
            _("wave-timer", "Next wave: " .. math.floor(WaveTimer) .. "s")
...
function HandlePayloadMovement()
    ...
    if PayloadShip.target == nil then
        if currentOrder ~= "Idle" then
            ...
            Player:addToShipLog(
                _("payload-comms", "Payload has stopped moving."),
                "red"
            )
        end
    else
        if #PayloadShip.Waypoints > 0 then
            ...
            if distance(PayloadShip, waypoint.x, waypoint.y) < 500 then
                ...
                if
                    currentOrder ~= "FlyTowards"
                    or (
                        currentOrder == "FlyTowards"
                        and not (
                            waypoint.x == targetX and waypoint.y == targetY
                        )
                    )
                then
...
function UpdateGMButtons(delta)
    ...
    addGMFunction(
        string.format(
            _("gm-buttons", "Wave Time +15 (%ds)"),
            math.floor(WaveTimer)
        ),
        function()
            WaveTimer = WaveTimer + 15
        end
    )
    addGMFunction(
        string.format(
    ...
    addGMFunction(
        string.format(_("gm-buttons", "Wave Size + (%d)"), WaveSize),
        function()
            WaveSize = WaveSize + 1
        end
    )
    addGMFunction(
        string.format(_("gm-buttons", "Wave Size - (%d)"), WaveSize),
        function()
            WaveSize = math.max(1, WaveSize - 1)
        end
    )
    addGMFunction(_("gm-buttons", "Kraylor Wave"), function()
        SpawnWave("Kraylor")
    end)
    addGMFunction(_("gm-buttons", "Human Wave"), function()
        SpawnWave("Human Navy")
    end)
    addGMFunction(_("gm-buttons", "Both Waves"), function()
        SpawnWave("Both")
    end)
    addGMFunction(_("gm-buttons", "New Payload Route"), function()
        PayloadShip.Waypoints = GeneratePayloadPath(PayloadShip.target)
    end)
end
...
function UpdateTraffic()
    ...
    for i = #Traffic.docked_ships, 1, -1 do
        ...
        if ship:isValid() and getScenarioTime() - ship.docked_at >= 60 then
            ...
            ship.dx, ship.dy = RandPositionInRadius(
                MidX,
                MidY,
                FieldSize + 5000,
                FieldSize,
    ...
    local x, y =
        RandPositionInRadius(MidX, MidY, FieldSize + 5000, FieldSize, 0, 360)
