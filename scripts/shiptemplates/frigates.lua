...
template:setDescription(
    _(
        [[The Phobos T3, just like the Atlantis, is the workhorse of almost any navy. It's extremely easy to modify, which makes retro-fitting this ship a breeze. Its basic stats aren't impressive, but due to its modular nature, it's fairly easy to produce in large quantities.]]
    )
)
template:setHull(70)
...
variation:setDescription(
    _(
        [[Inspired by the Phobos T3 design, the Elara P2 is nearly identical. With the addition of a warp drive and stronger front shields, the Elara P2 poses a greater threat than the Phobos]]
    )
)
...
template:setDescription(
    _(
        [[The Nirvana R5 is an anti-fighter cruiser. It has several rapid-firing, low-damage point-defense weapons to quickly take out starfighters.]]
    )
)
...
variation:setDescription(
    _(
        [[An improved version of the Nirvana R5 with faster turning speed and firing rates.]]
    )
)
...
template:setDescription(
    _(
        [[A heavy artillery cruiser, the Storm fires bunches of missiles from forward facing tubes.]]
    )
)
...
template:setDescription(
    _(
        [[A light artillery cruiser, the Piranha F12 is the smallest ship to exclusively fire from broadside weapon tubes.]]
    )
)
template:setHull(70)
template:setShields(30, 30)
...
variation =
    template:copy("Piranha F12.M"):setLocaleName(_("ship", "Piranha F12.M"))
variation:setDescription(
    _(
        [[This modified Piranha F12 is in all respects the same vessel except for special weapon tube modifications that allow it to fire nukes in addition to its normal loadout. However, these changes reduce its overall missile storage capacity.]]
    )
)
...
variation:setDescription(
    _(
        [[The first version of the Piranha was not popular due to its meager firepower and odd tube configuration. The result was a huge financial failure.]]
    )
)
...
template:setDescription(
    _(
        [[The Stalker is a strike ship designed to swoop into battle, deal damage quickly, and get out fast. The Q7 model is fitted with a warp drive.]]
    )
)
template:setHull(50)
...
variation:setDescription(
    _(
        [[The Stalker is a strike ship designed to swoop into battle, deal damage quickly, and get out fast. The R7 model is fitted with a jump drive.]]
    )
)
...
template:setDescription(
    _(
        [[The Ranus U sniper is built to deal a large amounts of damage quickly and from a distance before escaping. It's the only basic frigate that carries nuclear weapons, even though it's also the smallest of all frigate-class ships.]]
    )
)
template:setHull(30)
