...
Stand down or prepare to donate your corpses toward our nutrition.]]
            ...
            taunt_option = _(
                "shipEnemy-comms",
                "<Transmits 'The Itsy-Bitsy Spider' on all wavelengths>"
            )
            taunt_success_reply = _(
                "shipEnemy-comms",
        ...
        addCommsReply(taunt_option, function(comms_source, comms_target)
...
function getStatusReport(ship)
    ...
    elseif shields == 2 then
        ...
        for n = 0, shields - 1 do
            msg = msg
                .. string.format(
                    _("shipAssist-comms", "Shield %d: %d%%\n"),
                    n,
                    math.floor(
                        ship:getShieldLevel(n) / ship:getShieldMax(n) * 100
                    )
                )
        end
    end
        ...
        if ship:getWeaponStorageMax(missile_type) > 0 then
            msg = msg
                .. string.format(
                    ...
                    missile_type,
                    math.floor(ship:getWeaponStorage(missile_type)),
                    math.floor(ship:getWeaponStorageMax(missile_type))
                )
        end
    end
