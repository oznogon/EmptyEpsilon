...
function init()
    ...
    radius = 6300
    planet1 = Planet()
        :setPosition(6300 + 35000, 0)
        :setPlanetRadius(radius)
        :setDistanceFromMovementPlane(-2000)
        :setPlanetSurfaceTexture("planets/planet-earth.png")
        :setPlanetCloudTexture("planets/clouds-2.png")
        :setPlanetAtmosphereTexture("planets/atmosphere.png")
        :setPlanetAtmosphereColor(0.0, 0.0, 0.5)
        :setDescriptions(_("Earth"), _("The blue planet"))
        :setCallSign(_("Earth"))
    ...
    geo_1 = SpaceStation()
        ...
        :setFaction("Human Navy")
        :setCallSign("Geo-1")
        :setPosition(33064, -49665)
        :setDescription(_("A supply station in geostationary orbit"))
...
function geo_comm()
    if comms_source:isDocked(comms_target) then
        if
            comms_source:getHull() < comms_source:getHullMax() - 1
            or comms_source:getEnergyLevel()
                < comms_source:getEnergyLevelMax() - 1
        then
        ...
        setCommsMessage(
            _(
                'Good day, officer. If you need repairs or to replenish your energy, please dock with us. For more background information about your mission, see the "Mission data" section in the science database.'
            )
        )
    end
end
...
function mission_start_state(delta)
    if
        player1:getReputationPoints() >= reputation_threshold
        and debris_interactions >= interaction_threshold
    then
    ...
    if not areBeamShieldFrequenciesUsed() then
        globalMessage(
            _(
                "Shield frequencies are required for this scenario. Please enable frequencies in extra settings."
            )
        )
    end
...
function unusual_readings(delta)
    spyprobe = CpuShip()
        ...
        :setHullMax(100)
        :setHull(100)
        :setPosition(48885, -45317)
        :orderIdle()
    spyprobe:setDescriptions(
        _("An abandoned satellite"),
        _(
            "An old military satellite. Capturing frequency is blocked. Behaviour unknown. Recommendation: Jump not closer than 10U, then advance using the impulse drive."
        )
    )
    geo_1:sendCommsMessage(
        player1,
        _("We are getting strange readings from sector ")
            .. spyprobe:getSectorName()
            .. _(
                ". It looks like the source is an abandoned satellite. Please investigate, but be careful."
            )
    )
...
function start_havoc(delta)
    ...
    elseif explosion_timer > 5 and state_step == 0 then
        geo_1:sendCommsMessage(
            player1,
            _(
                [[The satellite started to attack objects in its proximity! That way, more fragments will be created that may harm other satellites. You must stop it! Try to NOT destroy it, target its impulse drive instead.]]
            )
        )
...
function spyprobe_disabled(delta)
    ...
    dormant_spyprobe = Artifact()
        :setPosition(x, y)
        :setCallSign("MiSat")
        :setDescriptions(
            _(
                "A deactivated military satellite. Scan to get the capturing frequency."
            ),
            _("Capturing frequency:") .. " " .. freq
        )
        :setScanningParameters(1, 2)
    dormant_spyprobe
        :setModel("combatsat")
        :setRadarTraceIcon("combatsat.png")
        :setRadarTraceScale(1)
...
function order_dock(delta)
    if dock_message_sent == 0 then
        ...
        geo_1:sendCommsMessage(
            player1,
            _(
                [[It looks like the old satellite was hit by a piece of space debris and thus reactivated. This also caused it to malfunction.
...
After that, you will receive further orders.]]
    ...
    if
        player1:isDocked(geo_1)
        and dock_message_sent == 1
        and player1:isCommsInactive()
    then
        player2:commandDock(geo_1)
        initSatNetwork()
        message_sat_network = geo_1:sendCommsMessage(
            player1,
            _(
                [[Bad news: The satellite woke up a whole group of military satellites that should have been out of service for ages. If we don't do anything against them, they will slowly but surely destroy all objects they can find. The debris will spread all over the orbit, destroying all our communications satellites.
...
We are currently making a plan to stop this. Please stay docked until you get new orders.
    ...
    if
        dock_message_sent == 2
        and player1:getEnergyLevel() > player1:getEnergyLevelMax() - 2
        and player1:isCommsInactive()
    then
        if player1:hasPlayerAtPosition("Weapons") then
            message_sat_deactivate = geo_1:sendCommsMessage(
                player1,
                _(
...
function change_ship()
    ...
    geo_1:sendCommsMessage(
        player2,
        _(
            [[Welcome to the new ship. It is smaller and more unsuspicious. On the downside, it is also less robust. Instead of a jump drive, it is equipped with a so-called warp drive.
...
function towards_commandnode(delta)
    if
        distance(player2, geo_1) > 10000
        and not cloud_hint
        and player2:hasPlayerAtPosition("Operations")
    then
        geo_1:sendCommsMessage(
            player2,
            _(
                [[The dust cloud is causing large electromagnetic interferences. Which means that as soon you are far enough away from the station, you can guess it's direction by looking at the red line at the edge of your radar screen.]]
            )
        )
        cloud_hint = true
    end
    ...
    if distance(player2, command_node) < 1001 then
        ...
        player2:addCustomButton(
            "Engineering",
            "activate_transmitter_btn",
            _("Activate transmitter"),
            activate_transmitter
        )
        player2:addCustomButton(
            "Engineering+",
            "activate_transmitter_btn_plus",
            _("Activate transmitter"),
            activate_transmitter
        )
...
function activate_transmitter()
    ...
    if player2:hasPlayerAtPosition("Relay") then
        geo_1:sendCommsMessage(
            player2,
            _(
                [[As soon as your transmitter is fully charged, the weapons officer has to sync the shields with the transmitter (a Button will appear on the console). Then, you yourself on Relay will have to send the signal. (There will be a button for this as well.) Good luck!]]
            )
        )
    else
        geo_1:sendCommsMessage(
    ...
    player2:addCustomInfo(
        "Engineering",
        "activate_transmitter_info",
        _("Transmitter is charging..")
    )
    player2:addCustomInfo(
        "Engineering+",
        "activate_transmitter_info_plus",
        _("Transmitter is charging..")
    )
    escalation = 20
...
function boot_transmitter(delta)
    ...
    if
        charge_timer > 20
        and transmitter_charge > (transmitter_txt + transmitter_step)
    then
        transmitter_txt = math.floor(transmitter_txt + transmitter_step)
        player2:addCustomInfo(
            "Engineering",
            "activate_transmitter_info",
            _("Transmitter charging") .. ": " .. transmitter_txt .. "%"
        )
        player2:addCustomInfo(
            "Engineering+",
            "activate_transmitter_info_plus",
    ...
    if charge_timer > 80 and escalation == 80 then
        player2:addCustomMessage(
            "Operations",
            "send_button_message",
            _(
                "If not done yet, you should now change the headline of your sidebar from 'scan' to 'other', so you can send the signal as soon as it is available."
            )
        )
    ...
    if charge_timer > 90 then
        ...
        player2:addCustomInfo(
            "Weapons",
            "connect_to_shields_info",
            _("Transmitter:")
        )
        player2:addCustomInfo(
            "Tactical",
            "connect_to_shields_info_tactical",
            _("Transmitter:")
        )
        player2:addCustomButton(
            "Weapons",
            "connect_to_shields_btn",
            _("Sync with shields"),
            connect_to_shields
        )
        player2:addCustomButton(
...
function send_signal()
    ...
    BeamEffect()
        :setSource(player2, 0, 0, 0)
        :setTarget(command_node, 0, 0)
        :setDuration(3)
        :setRing(false)
        :setTexture("texture/electric_sphere_texture.png")
...
function init_player2()
    ...
    player2:addCustomInfo(
        "Engineering+",
        "out_of_reach_info_plus",
        _("Out of reach")
    )
    player2:addCustomInfo(
        "Operations",
        "transmitter_unlinked_info",
        _("Transmitter is not linked yet")
    )
...
function initSatNetwork()
    ...
    command_node = WarpJammer()
        :setPosition(90000, -12000)
        :setRange(2500)
        :setCallSign("Control")
        :setDescription(
            _(
                "This is the command node that controls the rogue satellites. We have to shut it down!"
            )
        )
    command_node:onDestruction(
        function() -- fallback in case the command node somehow gets destroyed, so the scenario is still winnable
            command_node = Artifact()
                :setPosition(90000, -12000)
                :setCallSign("Control")
...
function placeRandomFreq(amount, x1, y1, x2, y2, random_amount)
    ...
    for n = 1, amount do
        ...
        debris = Artifact()
            ...
            :setDescriptions(
                _(
                    "A piece of space junk. Scan to find out the capturing frequency"
                ),
                _("Capturing frequency:") .. " " .. freq
            )
            :setScanningParameters(1, 2)
        ...
        debris
            ...
            :setFaction("Human Navy")
            :setRadarTraceColor(255, 235, 170)
            ...
            if shieldfreq == art.freq and player:getShieldsActive() == true then
                ...
                if player:getReputationPoints() == 20 then
                    geo_1:sendCommsMessage(
                        player1,
                        _(
                            [[Very good so far! Don't worry, you don't have to clean up all of the marked space junk in your first test run, but we still need quite a few of them before we call it a day.]]
                        )
                    )
...
function placeProbesAroundPoint(amount, dist_min, dist_max, x0, y0)
    ...
    for n = 1, amount do
        ...
        probe[n] = CpuShip()
            ...
            :orderIdle()
            :setCallSign("IC" .. n + 5)
            :setCommsFunction(no_reply)
        probe[n]:setDescriptions(
            _("An old military satellite"),
            _("An old military satellite. Capturing frequency is blocked.")
        )
...
function placeArtifactsAroundPoint(amount, dist_min, dist_max, x0, y0)
    for n = 1, amount do
        ...
        sat = Artifact()
            :setPosition(x, y)
            :setDescriptions(
                _("An operational satellite"),
                _("This satellite is fully operational. Do not capture!")
            )
            :setScanningParameters(1, 2)
        sat:setModel("cubesat")
            :setCallSign(callsign)
            :setRadarTraceIcon("satellite.png")
            :setRadarTraceScale(1)
