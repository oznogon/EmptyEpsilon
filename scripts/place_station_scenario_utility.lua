...
function placeStation(x, y, name, faction, size, diagnostic)
    ...
    if y == nil then
        print("The second parameter that the function placeStation expects is a y coordinate. Nil is not valid.")
        return nil
    end
    ...
    for _, system in ipairs(SYSTEMS) do
        station.comms_data.system_repair[system] = random(1, 100) <= system_chance
        station.comms_data.coolant_pump_repair[system] = random(1, 100) <= system_chance
    end
    return station
end
...
local function selectFromGroup(group, label, diagnostic, size)
    ...
    if diagnostic then
        print("Place station diagnostic: pick station returned group:", group, "...and station:", station, station:getCallSign(), "name is " .. tostring(label))
    end
...
function pickStation(name, diagnostic, size)
    ...
    if name == nil then
        ...
        return nil
    elseif name == "Random" or name == "RandomHumanNeutral" or name == "RandomGenericSinister" then
        local filter
        if name == "Random" then
            filter = function() return true end
        elseif name == "RandomHumanNeutral" then
            filter = function(g) return g ~= "Generic" and g ~= "Sinister" end
        else
            filter = function(g) return g == "Generic" or g == "Sinister" end
        end
        local candidates = {}
        for group, stations in pairs(station_pool) do
            if filter(group) then
                for n, d in pairs(stations) do
                    table.insert(candidates, { group = group, name = n, details = d })
                end
            end
        end
        ...
        if #candidates == 0 then
            if diagnostic then
                print("Place station diagnostic: Pick station returned nothing, name is " .. name)
            end
            return nil
        end
        ...
        if diagnostic then
            print("Place station diagnostic: pick station returned group:", pick.group, "...and station:", station, station:getCallSign(), "name is " .. name)
        end
    ...
    station_pool = {
        ["Science"] = {
            ["Asimov"] = stationBuild{
                ...
                goods = { tractor = stationGood(48), repulsor = stationGood(48) },
            ...
            ["Armstrong"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station is named after the 19th-century astronaut as well as the fictionlized stations that followed. The station initially constructed spaceworthy vessels, but over time it transitioned into specializing in propulsion systems."),
            },
            ["Broeck"] = stationBuild{
                goods = { warp = stationGood(36) },
                trade = { food = random(1, 100) <= 14, medicine = false, luxury = random(1, 100) < 62 },
                buy = { [randomMineral()] = math.random(40, 200) },
                ...
                history = _("stationStory-comms", "This station is named after Chris Van Den Broeck, who did some initial research into the possibility of warp drives in the late 20th century on Earth."),
            },
            ["Coulomb"] = stationBuild{
                reputation_neutral = 3.0,
                goods = { circuit = stationGood(50) },
                trade = { food = random(1, 100) <= 35, medicine = false, luxury = random(1, 100) < 82 },
                buy = { [randomMineral()] = math.random(40, 200) },
            ...
            ["Heyes"] = stationBuild{
                ...
                trade = { food = random(1, 100) <= 32, medicine = false, luxury = true },
                buy = { [randomMineral()] = math.random(40, 200) },
                ...
                history = _("stationStory-comms", "The station is named after Tony Heyes, the inventor of some of the earliest electromagnetic sensors in the mid-20th century on Earth in the United Kingdom, which aided in mobility for blind humans."),
            },
            ["Hossam"] = stationBuild{
                reputation_neutral = 3.0,
                goods = { nanites = stationGood(90) },
                trade = { food = random(1, 100) < 24, medicine = random(1, 100) < 44, luxury = random(1, 100) < 63 },
                ...
                history = _("stationStory-comms", "This station is named after the Israeli nanotechnologist Hossam Haick, from the early 21st century on Earth."),
            },
            ["Maiman"] = stationBuild{
                weapons_override = { HVLI = false },
                reputation_neutral = 3.0,
                goods = { beam = stationGood(70) },
                trade = { food = random(1, 100) <= 75, medicine = true, luxury = false },
                buy = { [randomMineral()] = math.random(40, 200) },
            ...
            ["Malthus"] = stationBuild{
                reputation_neutral = 3.0,
                trade = { food = random(1, 100) <= 65, medicine = false, luxury = false },
            ...
            ["Marconi"] = stationBuild{
                ...
                trade = { food = random(1, 100) <= 53, medicine = false, luxury = true },
                description = _("scienceDescription-station", "Energy beam components"),
                general = _("stationGeneralInfo-comms", "We manufacture energy beam components."),
                history = _("stationStory-comms", "Station named after Guglielmo Marconi, an Italian inventor from early 20th-century Earth who, along with Nikola Tesla, claimed to have invented a death ray or particle beam weapon."),
            },
            ["Miller"] = stationBuild{
                reputation_neutral = 3.0,
                goods = { optic = stationGood(60) },
                trade = { food = random(1, 100) <= 68, medicine = false, luxury = false },
            ...
            ["Shawyer"] = stationBuild{
                ...
                trade = { food = random(1, 100) <= 42, medicine = false, luxury = true },
        ...
        ["History"] = {
            ["Archimedes"] = stationBuild{
                ...
                history = _("stationStory-comms", "This station was named after Archimedes, who, according to legend, used a series of adjustable focal length mirrors to focus sunlight on a Roman naval fleet invading Syracuse and set fire to it."),
            },
            ...
            ["Hayden"] = stationBuild{
                ...
                trade = { food = random(1, 100) <= 85, medicine = false, luxury = false },
            ...
            ["Madison"] = stationBuild{
                ...
                general = _("stationGeneralInfo-comms", "Take in a game or two, or perhaps see a show"),
                history = _("stationStory-comms", "Named after Madison Square Garden from 21st-century Earth, this station was designed to serve similar purposes in space as a venue for sports and entertainment."),
            },
            ["Rutherford"] = stationBuild{
                ...
                history = _("stationStory-comms", "This station was named after Rutherford Appleton Laboratory, a national research institution in the United Kingdom on Earth, which conducted preliminary research into the feasability of generating an energy shield in the late 20th century."),
            },
            ["Toohie"] = stationBuild{
                reputation_neutral = 3.0,
                goods = { shield = stationGood(90) },
                trade = { food = random(1, 100) <= 21, medicine = false, luxury = true },
                ...
                history = _("stationStory-comms", "This station was named after Alexander Toohie, one of the earliest researchers in shield technology, back when it was considered impractical to construct shields due to the physics involved."),
            },
        },
        ["Alt Sci Fi"] = {
            ["Cortex Gate"] = stationBuild{
                ...
                reputation_neutral = 2.0,
                goods = { battery = stationGood(66), software = stationGood(115) },
                ...
                history = _("stationStory-comms", "The station was founded by a collective of electrical and software engineers who distrusted third-party systems. They built their own from scratch, and demand has kept them busy since."),
            },
            ["Bulwark"] = stationBuild{
                ...
                history = _("stationStory-comms", "Originally a military outpost, the station's fabrication bays were repurposed for defensive systems production after an armistice. The quality of their work has never declined."),
            },
            ["Wavecrest"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station was first built as a deep-space relay outpost during the early days of interstellar exploration. As the sector grew, so did this station, eventually evolving into a full manufacturing hub for communication systems."),
            },
            ["Mechadyne"] = stationBuild{
                weapons_override = { Homing = false },
                reputation_neutral = 3.0,
                goods = { robotic = stationGood(90) },
                trade = { food = random(1, 100) <= 35, medicine = false, luxury = true },
                ...
                history = _("stationStory-comms", "A consortium of cyberneticists and mechanical engineers pooled their resources to create a dedicated research center for autonomous systems. Their breakthroughs in actuator design and decision algorithms are used across the sector."),
            },
            ["Tensilica"] = stationBuild{
                reputation_neutral = 2.0,
                goods = { filament = stationGood(42) },
                description = _("scienceDescription-station", "Advanced Material components"),
                general = _("stationGeneralInfo-comms", "We fabricate several different kinds of materials critical to various space industries, such as shipbuilding, station construction, and mineral extraction"),
                history = _("stationStory-comms", "The station began as a materials science lab investigating high-tensile alloys for deep-space construction. Their discoveries led to a full production line that now supplies construction firms across the sector."),
            },
            ["Thrust Harbor"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station was founded by a team of propulsion engineers who believed that conventional thruster design had room for dramatic improvement. Their iterative refinements have made them a trusted name in sublight travel."),
            },
            ["Autonom"] = stationBuild{
                ...
                history = _("stationStory-comms", "Founded by a cooperative of engineers who saw the growing demand for synthetic laborers and companions. Their modular design philosophy makes repairs and upgrades straightforward."),
            },
            ["Beamhold"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station was founded by the lead engineers from a defunct transporter manufacturing conglomerate. When the parent company folded, they pooled their severance and built a better facility from the ground up."),
            },
            ["Lucky Draw"] = stationBuild{
                ...
                history = _("stationStory-comms", "What started as a single card table in a maintenance bay grew organically into one of the busiest entertainment hubs in the sector. The original table is still on display in the main atrium."),
            },
            ["Rosetta"] = stationBuild{
                ...
                history = _("stationStory-comms", "Psychologists, sociologists, and xenobiologists founded this station as a dedicated institute for interspecies communication. The curriculum has been adopted by several naval academies."),
            },
            ["Astra"] = stationBuild{
                weapons = { Homing = true, HVLI = true, Mine = true, Nuke = false, EMP = false },
                weapon_cost = {
                ...
                general = _("stationGeneralInfo-comms", "We never tell you the odds, because they're always in your favor!"),
                history = _("stationStory-comms", "A front for an interstellar crime syndicate that launders and fences much of its ill-gotten goods through this seedy hall of games."),
            },
            ["Crossway"] = stationBuild{
                goods = { luxury = stationGood(60) },
                trade = { food = true, medicine = true, luxury = false },
                description = _("scienceDescription-station", "Trading station"),
                general = _("stationGeneralInfo-comms", "Come to Crossway Bazaar for all your trade and commerce needs and desires!"),
                history = _("stationStory-comms", "Independent traders from a dozen systems formed a cooperative to establish this central marketplace. Over time, permanent storefronts replaced the temporary stalls, but the spirit of the bazaar remains."),
            },
            ["Auton Yard"] = stationBuild{
                ...
                description = _("scienceDescription-station", "Android components"),
                general = _("stationGeneralInfo-comms", "Androids and their parts, maintenance, and recycling."),
                history = _("stationStory-comms", "The station grew out of a small repair shop that specialized in synthetic personnel. As demand for android labor increased, the facility expanded into manufacturing and recycling."),
            },
            ["Flux Gate"] = stationBuild{
                reputation_neutral = 3.0,
                goods = { transporter = stationGood(76) },
                trade = { food = random(1, 100) < 13, medicine = true, luxury = random(1, 100) < 43 },
                description = _("scienceDescription-station", "Transporter components"),
                ...
                history = _("stationStory-comms", "The station was founded by a veteran logistics coordinator who saw an opportunity to improve transporter reliability. Their precision components are now standard equipment on many civilian vessels."),
            },
            ["Concord"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station was conceived during a fragile peace negotiation between human factions. The delegates agreed that a permanent center for diplomatic study would help prevent future conflicts."),
            },
            ["Liftmaster"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station was built around a heavy-equipment machine shop that had been operating for decades. When the original owners retired, the foreman bought the operation and expanded into full-scale manufacturing."),
            },
            ["Crane"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station was started by a cargo-hauler cooperative that grew tired of overpaying for replacement parts. They bought their own manufacturing equipment and soon began selling to other haulers."),
            },
            ["Carousel"] = stationBuild{
                reputation_neutral = 2.0,
                goods = { luxury = stationGood(math.random(30, 80)) },
                description = _("scienceDescription-station", "Routine maintenance and entertainment"),
                general = _("stationGeneralInfo-comms", "Stop by for repairs. Take in one of our juggling shows featuring the four-armed jugglers from the outer ring."),
                history = _("stationStory-comms", "Traveling entertainers and independent mechanics converged at this waypoint, turning a simple rest stop into a vibrant community of performers and repair specialists."),
            },
            ["Onom"] = stationBuild{
                reputation_neutral = 3.0,
                goods = { android = stationGood(73) },
                ...
                history = _("stationStory-comms", "The station was established by a group of engineers who believed that synthetic lifeforms should be accessible to all. Their open-source designs have been adapted for everything from domestic service to hazardous environment exploration."),
            },
            ["Sentinel"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station was founded by a group of defense contractors who foresaw the growing need for automated turret and point-defense systems. Their early-warning algorithms have saved countless vessels from ambush."),
            },
            ["Logi Lodge"] = stationBuild{
                ...
                history = _("stationStory-comms", "Shipping companies founded this station to coordinate supply routes across their sectors. Its routing algorithms and communication relays keep traffic flowing smoothly."),
            },
            ["Filamentropolis"] = stationBuild{
                ...
                history = _("stationStory-comms", "After discovering a unique high-yield refining process, the station's founders cornered the market on structural filaments. The technique remains a closely guarded trade secret."),
            },
            ["Orbitalwerks"] = stationBuild{
                ...
                general = _("stationGeneralInfo-comms", "We work on all aspects of astronaval shipbuilding and maintenance."),
                history = _("stationStory-comms", "Many of this station's ship models are researched, designed, and built entirely on site. Their designs seek to make the spacefaring experience as simple as possible, given the tremendous capabilities of the modern vessel."),
            },
            ["KwikDock"] = stationBuild{
                ...
                description = _("scienceDescription-station", "Ship building and maintenance facility"),
                general = _("stationGeneralInfo-comms", "If your motivator's shot, there's no cheaper replacement in this quadrant than here."),
                history = _("stationStory-comms", "Despite its run-down appearance, this station is home to some of the best salvage refurbishment techs and mechanics. A favorite maintenance stop among crews of outdated and out-of-support vessels."),
            },
            ["Warpwell"] = stationBuild{
                ...
                history = _("stationStory-comms", "The station was founded by physicists and engineers who dedicated their careers to mastering faster-than-light travel. Their precision components are trusted by every major shipyard in known space."),
            },
        },
        ["Spec Sci Fi"] = {
