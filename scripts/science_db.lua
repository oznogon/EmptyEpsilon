...
item:setLongDescription(
    _(
        [[An asteroid is a minor planet, usually smaller than a few kilometers. Larger variants are sometimes referred to as planetoids.]]
    )
)
...
item:setLongDescription(
    _(
        [[A black hole is a point of supercondensed mass with a gravitational pull so powerful that not even light can escape it. It has no locally detectable features, and can only be seen indirectly by blocking the view and distorting its surroundings, creating a strange circular mirror image of the galaxy. The black disc in the middle marks the event horizon, the boundary where even light can't escape it anymore.
...
item:setLongDescription(
    _(
        [[Nebulae are the birthing places of new stars. These gas fields, usually created by the death of an old star, slowly form new stars due to the gravitational pull of its gas molecules.
...
item:setLongDescription(
    _(
        [[A planetary-mass object is large, dense, near-spherical astronomical body comprised of various forms of matter. Most planets are either terrestrial, like Earth, or giants, like the gas giant Jupiter or ice giant Neptune.
...
technologies:setLongDescription(
    _(
        [[This database covers ship systems and technologically created phenomena that spacefaring crews might encounter in densely populated regions of space.
...
item:setLongDescription(
    _(
        [[Ships with shield and beam systems can often configure their terahertz-frequency radiation. By manipulating these frequencies, savvy ship crews can maximize their beam weapon efficiency while reducing their enemies'.
...
item:setLongDescription(
    _(
        [[Thanks to our covert operations teams, we've acquired enough information about our enemies' computer systems to repurpose our long-range communications systems into intrusion and exploitation tools.
...
item:addKeyValue(_("Thermal"), _("Green (middle band)"))
item:addKeyValue(_("Gravitational"), _("Blue (inner band)"))
item:setLongDescription(
    _(
        [[The outer ring of long-range Science radar screens contain three colored bands that represent raw sensor inputs monitored on that heading. Ships and space phenomena emit energies or exhibit characteristics that your sensor suite translates into icons on your radar, but these raw rings can provide more information to a well-trained eye.

For tactical purposes, heat generated by ship systems is registered in raw thermal sensor data as thermal (green) sources. If a ship's systems are under- or over-powered, its electrical (red) readings change accordingly.
...
item:setLongDescription(
    _(
        [[Whether your mission involves scientific investigation, deep-space exploration, or military operations, remember to take advantage of any scan probes distributed for your ship.
...
weapons:setLongDescription(
    _(
        [[This database covers only the basic versions of weapons used throughout the galaxy.]]
    )
)
...
item:setLongDescription(
    _(
        [[Beam weapons emit an instantaneous, focused burst of energy or matter at a single target within a target arc. Many ships equip beam weapons for their precision and versatility.
...
item:setLongDescription(
    _(
        [[Missiles are weapons launched from tubes. Most are self-propelled ranged weapons with various capabilities, such as differing ranges, self-guided pursuit of a designated target, explosive warheads that damage everything within a radius, and varying types and amounts of damage.
...
local homing_stats =
    { _("Kinetic"), 35, 5, 30 / 1000, 200 / 1000, 10, 27.0, 1200 / 1000 }
local nuke_stats =
    { _("Kinetic"), 160, 30, 1000 / 1000, 200 / 1000, 10, 27.0, 500 / 1000 }
local emp_stats =
    { _("EMP"), 160, 30, 1000 / 1000, 200 / 1000, 10, 27.0, 500 / 1000 }
local hvli_stats = { _("Kinetic"), 10, 10, 20 / 1000, 500 / 1000, 0, 13.5, 0 }
local mine_stats =
    { _("Kinetic"), 160, 30, 1000 / 1000, 100 / 1000, 0, 10.0, 1000 / 1000 }
...
local function populateMissileStats(__item, __keys, __stats)
    ...
    for idx, key in ipairs(__keys) do
        ...
        if
            type(__stats[idx]) == "number"
            and (
                key == _("Damage at center")
        ...
        then
            for size_idx, size in ipairs(missile_modifier_sizes) do
                size_value = size_value
                    .. __stats[idx] * missile_modifier_values[size_idx]
                    .. " ("
                    .. size
                    .. ") "
...
local function directionLabel(direction)
    name = "?"
    if math.abs(angleDifference(0.0, direction)) <= 45 then
        name = _("database direction", "Front")
    end
    if math.abs(angleDifference(90.0, direction)) < 45 then
        name = _("database direction", "Right")
    end
    if math.abs(angleDifference(-90.0, direction)) < 45 then
        name = _("database direction", "Left")
    end
    if math.abs(angleDifference(180.0, direction)) <= 45 then
        name = _("database direction", "Rear")
    end
    return name
end
...
function __fillDefaultDatabaseData()
    ...
    local faction_database =
        ScienceDatabase():setName(_("database", "Factions"))
    for name, info in pairs(__faction_info) do
        local entry =
            faction_database:addEntry(info.components.faction_info.locale_name)
        ...
        for name2, info2 in pairs(__faction_info) do
            table.insert(
                faction_names2,
                {
                    string.lower(info2.components.faction_info.locale_name),
                    name2,
                }
            )
        end
        -- Sort internal names by localized names
        table.sort(faction_names2, function(f1, f2)
            return f1[1] < f2[1]
        end)
            ...
            if info ~= info2 then
                ...
                for idx, relation in ipairs(info.components.faction_info) do
                    if relation.other_faction == info2 then
                        if relation.relation == "neutral" then
                            stance = _("stance", "Neutral")
                        end
                        if relation.relation == "enemy" then
                            stance = _("stance", "Enemy")
                        end
                        if relation.relation == "friendly" then
                            stance = _("stance", "Friendly")
                        end
                    end
                end
            end
    ...
    ship_database:setLongDescription(
        _(
            "Spaceships are vessels capable of withstanding the dangers of travel through deep space. They can fill many functions and vary broadly in size, from small tugs to massive dreadnoughts."
        )
    )
    -- Populate the Stations top-level entry.
    local stations_database =
        ScienceDatabase():setName(_("database", "Stations"))
    stations_database:setLongDescription(
        _(
            "Space stations are permanent, immobile structures ranging in scale from small outposts to city-sized communities. Many provide restocking and repair services to neutral and friendly ships."
        )
    )
    ...
    for idx, name in ipairs(template_names) do
        ...
        if not ship_template.__hidden then
            ...
            if ship_template.docking_port ~= nil then
                class_name = ship_template.docking_port.dock_class
                subclass_name = ship_template.docking_port.dock_subclass
            end
            local entry = nil
            ...
            if ship_template.__type == "station" then
                entry =
                    stations_database:addEntry(ship_template.typename.localized)
            else
                entry = class_database_entries[class_name]:addEntry(
                    ship_template.typename.localized
                )
            ...
            if ship_template.physics then
                if type(ship_template.physics.size) == "table" then
                    entry:addKeyValue(
                        _("database", "Size"),
                        math.floor(ship_template.physics.size[1])
                    )
                else
                    entry:addKeyValue(
                        _("database", "Size"),
                        math.floor(ship_template.physics.size)
                    )
                end
            end
            ...
            if ship_template.impulse_engine then
                entry:addKeyValue(
                    _("database", "Move speed"),
                    string.format(
                        "%.1f u/min.",
                        ship_template.impulse_engine.max_speed_forward
                            * 60
                            / 1000
                    )
                )
                entry:addKeyValue(
                    _("database", "Reverse move speed"),
                    string.format(
                        "%.1f u/min.",
                        ship_template.impulse_engine.max_speed_reverse
                            * 60
                            / 1000
                    )
                )
            ...
            if ship_template.maneuvering_thrusters then
                entry:addKeyValue(
                    _("database", "Turn speed"),
                    string.format(
                        "%.1f deg./sec.",
                        ship_template.maneuvering_thrusters.speed
                    )
                )
            end
            if ship_template.warp_drive then
                entry:addKeyValue(
                    _("database", "Warp speed"),
                    string.format(
                        "%.1f u/min.",
                        ship_template.warp_drive.speed_per_level * 60 / 1000
                    )
                )
            end
            if ship_template.jump_drive then
                entry:addKeyValue(
                    _("database", "Jump range"),
                    string.format(
                        "%.0f - %.0f u",
                        (ship_template.jump_drive.min_distance or 5000) / 1000,
                        (ship_template.jump_drive.max_distance or 20000) / 1000
                    )
                )
            end
            ...
            if ship_template.missile_tubes then
                ...
                for i, missile_type in ipairs(MISSILE_TYPES) do
                    ...
                    if
                        ship_template.missile_tubes["max_" .. missile_type_lower]
                    then
                        local capacity =
                            ship_template.missile_tubes["max_" .. missile_type_lower]
                        if capacity > 0 then
