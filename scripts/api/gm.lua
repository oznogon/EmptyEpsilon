function getSpawnableGMObjects()
    ...
    result[#result + 1] = {
        function()
            return VisualAsteroid()
        end,
        _("create", "Visual asteroid"),
        _("create", "Various"),
        "",
        "radar/blip.png",
    }
    ...
    result[#result + 1] = {
        ...
        _("create", "Camera"),
        _("create", "Various"),
        _("create", "Cinematic camera viewpoint"),
        "radar/blip.png",
    }
        ...
        if not v.__hidden then
            if v.__type == "playership" then
                result[#result + 1] = {
                    ...
                    _("create", "Player ship"),
                    v.__description,
                    v.radar_trace.icon,
                }
            elseif v.__type == "station" then
                result[#result + 1] = {
                    __spawnStationFunc(v.typename.type_name),
                    v.typename.localized,
                    _("create", "Station"),
                    v.__description,
                    v.radar_trace.icon,
                ...
                result[#result + 1] = {
                    __spawnCpuShipFunc(v.typename.type_name),
                    v.typename.localized,
                    _("create", "CPU ship"),
                    v.__description,
                    v.radar_trace.icon,
...
function __spawnStationFunc(key)
    return function()
        return SpaceStation():setTemplate(key):setRotation(random(0, 360))
    end
end
...
function getEntityExportString(entity)
    ...
    if entity.components.player_control and entity.components.typename then
        ...
        for k, v in pairs(__ship_templates) do
            if
                v.__type == "playership"
                and v.typename.type_name
                    == entity.components.typename.type_name
            then
                return "PlayerSpaceship():setTemplate('"
                    .. k
                    .. "')"
                    .. __exportShipChanges(entity, v, "full")
            end
        end
    end
    if entity.components.ai_controller and entity.components.typename then
        ...
        for k, v in pairs(__ship_templates) do
            if
                (v.__type == "ship" or v.__type == nil)
                and v.typename.type_name
                    == entity.components.typename.type_name
            then
                return "CpuShip():setTemplate('"
                    .. k
                    .. "')"
                    .. __exportShipChanges(entity, v)
            end
        end
    end
    if
        entity.components.typename
        and entity.components.physics
        and entity.components.physics.type == "static"
    then
        -- Likely a station
        for k, v in pairs(__ship_templates) do
            if
                ...
                and v.typename.type_name
                    == entity.components.typename.type_name
            then
                return "SpaceStation():setTemplate('"
                    .. k
                    .. "')"
                    .. __exportShipChanges(entity, v)
            end
    ...
    if
        entity.components.spin
        and entity.components.avoid_object
        and entity.components.explode_on_touch
    then
    ...
    if
        entity.components.spin
        and entity.components.mesh_render
        and not entity.components.physics
        and not entity.components.avoid_object
    then
        return "VisualAsteroid()" .. __exportBasics(entity)
    end
    -- Mine has delayed_explode_on_touch and constant_particle_emitter
    if
        entity.components.delayed_explode_on_touch
        and entity.components.constant_particle_emitter
    then
    ...
    if
        entity.components.gravity
        and entity.components.billboard_render
        and entity.components.gravity.damage
    then
    ...
    if
        entity.components.gravity
        and entity.components.billboard_render
        and not entity.components.gravity.damage
    then
    ...
    if entity.components.pickup then
        ...
        if
            (p.give_energy and p.give_energy > 0)
    ...
    if
        entity.components.mesh_render
        and entity.components.mesh_render.mesh
        and string.sub(entity.components.mesh_render.mesh, 1, 13)
            == "mesh/Artifact"
    then
...
function __exportBasics(entity, default_scan_state)
    ...
    if entity.components.callsign then
        local cs = entity.components.callsign.callsign
            :gsub("\\", "\\\\")
            :gsub("'", "\\'")
    ...
    if ss then
        ...
        for n = 1, #ss do
            ...
            if
                entry.faction
                and entry.faction.components
                and entry.faction.components.faction_info
            then
...
function __exportShipChanges(entity, template, default_scan_state)
    ...
    if ie then
        if
            not t_ie
            or ie.max_speed_forward ~= t_ie.max_speed_forward
            or ie.max_speed_reverse ~= t_ie.max_speed_reverse
        then
            if ie.max_speed_forward == ie.max_speed_reverse then
                extras = extras
                    .. string.format(
                        ":setImpulseMaxSpeed(%.1f)",
                        ie.max_speed_forward
                    )
            else
                extras = extras
                    .. string.format(
                        ":setImpulseMaxSpeed(%.1f, %.1f)",
                        ie.max_speed_forward,
                        ie.max_speed_reverse
                    )
            end
        end
        if
            not t_ie
            or ie.acceleration_forward ~= t_ie.acceleration_forward
            or ie.acceleration_reverse ~= t_ie.acceleration_reverse
        then
            if ie.acceleration_forward == ie.acceleration_reverse then
                extras = extras
                    .. string.format(
                        ":setAcceleration(%.1f)",
                        ie.acceleration_forward
                    )
            else
                extras = extras
                    .. string.format(
                        ":setAcceleration(%.1f, %.1f)",
                        ie.acceleration_forward,
                        ie.acceleration_reverse
                    )
            end
        end
        if
            ie.sound
            and ie.sound ~= ""
            and (not t_ie or ie.sound ~= t_ie.sound)
        then
    ...
    if
        ...
        and (
            not t_man_thrusters
            or man_thrusters.speed ~= t_man_thrusters.speed
        )
    then
        extras = extras
            .. string.format(":setRotationMaxSpeed(%.1f)", man_thrusters.speed)
    end
    ...
    if
        ...
        and (
            not t_cmt
            or cmt.boost_speed ~= t_cmt.boost_speed
            or cmt.strafe_speed ~= t_cmt.strafe_speed
        )
    then
        extras = extras
            .. string.format(
                ":setCombatManeuver(%.0f, %.0f)",
                cmt.boost_speed,
                cmt.strafe_speed
            )
    end
    ...
    if wd then
        ...
        if wd.speed_per_level ~= t_warp_speed then
            extras = extras
                .. string.format(":setWarpSpeed(%.0f)", wd.speed_per_level)
        end
    end
    ...
    if bw then
        for i = 1, #bw do
            ...
            if
                not tb
                or b.arc ~= tb.arc
                or b.direction ~= tb.direction
                or b.range ~= tb.range
                or b.cycle_time ~= tb.cycle_time
                or b.damage ~= tb.damage
            then
                extras = extras
                    .. string.format(
                        ":setBeamWeapon(%d, %.1f, %.1f, %.0f, %.1f, %.1f)",
                        idx,
                        b.arc,
                        b.direction,
                        b.range,
            ...
            if b.turret_arc and b.turret_arc ~= 0 then
                if
                    not tb
                    or b.turret_arc ~= tb.turret_arc
                    or b.turret_direction ~= tb.turret_direction
                    or b.turret_rotation_rate ~= tb.turret_rotation_rate
                then
                    extras = extras
                        .. string.format(
                            ":setBeamWeaponTurret(%d, %.1f, %.1f, %.1f)",
                            idx,
                            b.turret_arc,
                            b.turret_direction,
            ...
            if
                b.energy_per_beam_fire
                and b.energy_per_beam_fire ~= tb_energy
            then
                extras = extras
                    .. string.format(
                        ":setBeamWeaponEnergyPerFire(%d, %.2f)",
                        idx,
                        b.energy_per_beam_fire
                    )
            end
            ...
            if
                b.heat_per_beam_fire
                and math.abs(b.heat_per_beam_fire - tb_heat) > 1e-5
            then
                extras = extras
                    .. string.format(
                        ":setBeamWeaponHeatPerFire(%d, %.3f)",
                        idx,
                        b.heat_per_beam_fire
                    )
            end
            ...
            then
                ...
                extras = extras
                    .. string.format(
                        ":setBeamWeaponArcColor(%d, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f)",
                        idx,
                        ac[1] / 255.0,
                        ac[2] / 255.0,
                        ac[3] / 255.0,
            ...
            if
                b.damage_type
                and b.damage_type ~= "energy"
                and (not tb or b.damage_type ~= tb.damage_type)
            then
                extras = extras
                    .. string.format(
                        ":setBeamWeaponDamageType(%d, '%s')",
                        idx,
                        b.damage_type
                    )
            end
    ...
    if ai then
        if
            ai.new_name
            and ai.new_name ~= ""
            and ai.new_name ~= "default"
            and ai.new_name ~= t_ai.new_name
        then
        ...
        elseif orders == "defend_location" and loc then
            extras = extras
                .. string.format(
                    ":orderDefendLocation(%.0f, %.0f)",
                    loc[1],
                    loc[2]
                )
        elseif orders == "fly_towards" and loc then
            extras = extras
                .. string.format(":orderFlyTowards(%.0f, %.0f)", loc[1], loc[2])
        elseif orders == "fly_towards_blind" and loc then
            extras = extras
                .. string.format(
                    ":orderFlyTowardsBlind(%.0f, %.0f)",
                    loc[1],
                    loc[2]
                )
...
function __countShipCrew(entity)
    ...
    for _, crew in ipairs(getEntitiesWithComponent("internal_crew")) do
        if
            crew.components.internal_crew
            and crew.components.internal_crew.ship == entity
        then
            n = n + 1
        end
    end
...
function __exportPlanet(entity)
    ...
    if pr then
        ...
        if pr.atmosphere_color then
            extras = extras
                .. string.format(
                    ":setPlanetAtmosphereColor(%.2f, %.2f, %.2f)",
                    pr.atmosphere_color[1],
                    pr.atmosphere_color[2],
                    pr.atmosphere_color[3]
                )
        end
        ...
        if
            pr.cloud_size
            and pr.size
            and math.abs(pr.cloud_size - pr.size * 1.05) > 1
        then
            extras = extras
                .. string.format(":setPlanetCloudRadius(%.0f)", pr.cloud_size)
        end
        if
            pr.distance_from_movement_plane
            and pr.distance_from_movement_plane ~= 0
        then
            extras = extras
                .. string.format(
                    ":setDistanceFromMovementPlane(%.0f)",
                    pr.distance_from_movement_plane
                )
        end
    end
    if entity.components.spin then
        ...
        if rate and rate ~= 0 then
            extras = extras
                .. string.format(":setAxialRotationTime(%.2f)", 360.0 / rate)
        end
    end
    return extras
end
...
function __exportWormHole(entity)
    ...
    if g and g.wormhole_target then
        ...
        if tx ~= 0 or ty ~= 0 then
            extras = extras
                .. string.format(":setTargetPosition(%.0f, %.0f)", tx, ty)
        end
    end
    return extras
end
...
function __exportSupplyDrop(entity)
    ...
    if p then
        ...
        if p.give_homing and p.give_homing > 0 then
            extras = extras
                .. string.format(
                    ":setWeaponStorage('Homing', %d)",
                    p.give_homing
                )
        end
        if p.give_nuke and p.give_nuke > 0 then
            extras = extras
                .. string.format(":setWeaponStorage('Nuke', %d)", p.give_nuke)
        end
        if p.give_mine and p.give_mine > 0 then
            extras = extras
                .. string.format(":setWeaponStorage('Mine', %d)", p.give_mine)
        end
        if p.give_emp and p.give_emp > 0 then
            extras = extras
                .. string.format(":setWeaponStorage('EMP', %d)", p.give_emp)
        end
        if p.give_hvli and p.give_hvli > 0 then
            extras = extras
                .. string.format(":setWeaponStorage('HVLI', %d)", p.give_hvli)
        end
    end
    return extras
end
...
function __exportZone(entity)
    ...
    if
        z.color
        and (z.color[1] ~= 255 or z.color[2] ~= 255 or z.color[3] ~= 255)
    then
        extras = extras
            .. string.format(
                ":setColor(%d, %d, %d)",
    ...
    if z.skybox and z.skybox ~= "" then
        ...
        if fade ~= 0 then
            extras = extras
                .. string.format(":setLocalSkybox('%s', %.0f)", z.skybox, fade)
        else
    ...
    if z.points and #z.points > 0 then
        ...
        local tx, ty = 0, 0
        if
            entity.components.transform and entity.components.transform.position
        then
