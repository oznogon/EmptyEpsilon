...
--- A CpuShip is an AI-controlled ship entity.
...
--- AI behaviors are defined in the AIController component and its related systems.
--- order... functions also broadcast their orders over friendly comms.
--- Autonomous combat AI orders use the ship's short- and long-range radar ranges to acquire targets, which can be affected by radar-blocking entities.
...
--- ship = CpuShip():setTemplate("Riposte"):setPosition(10000,3000):setFaction("Human Navy"):orderRoaming():setAI("evasive"):setScanned(true)
--- @type creation
function CpuShip()
...
--- Sets the default combat AI state for this ship.
...
--- Combat AI state can be set per ship, defined in the ShipTemplate, or left to "default".
...
--- If the entity is a dockable entity, this ship moves directly toward it and docks with it as soon as possible.
...
--- This ship also targets anything its given entity targets.
--- If the entity being followed is destroyed, this ship reverts to Roaming orders.
