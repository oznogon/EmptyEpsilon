...
--- Example:
--- -- Creates a player Atlantis of the Human Navy faction
...
--- Example:
--- ship:addToShipLog("Acknowledged", "yellow") -- adds "Acknowledged" in yellow to the `player` ship's log
...
--- Example:
--- ship:transferPlayersAtPositionToShip("helms", player2) -- transfer all crew on Helms to `player2`
function Entity:transferPlayersAtPositionToShip(station, other_ship)
    transferPlayersFromShipToShip(self, other_ship, station)
end
--- Returns whether a player occupies the given crew position on this ship.
--- Example:
--- ship:hasPlayerAtPosition("helms")
function Entity:hasPlayerAtPosition(station)
    return hasPlayerCrewAtPosition(self, station)
end

--- Returns whether this ship's comms are not in use.
...
--- Returns whether this ship is being hailed by another entity.
--- Example:
--- ship:isCommsBeingHailed()
...
--- Returns whether comms to this ship were broken off by the other entity.
--- Example:
--- ship:isCommsBroken()
...
--- Returns whether comms between this ship and an entity were intentionally closed.
--- Example:
--- ship:isCommsClosed()
...
--- Returns whether this ship is engaged in text chat with either the GM or another player ship.
--- Example:
--- ship:isCommsChatOpen()
...
--- Energy levels are relevant only if the ship also has a Reactor component.
--- Example:
...
--- Energy capacity is relevant only if the ship also has a Reactor component.
--- Example:
...
--- Returns how much energy is consumed per second by this ship's shields while active.
--- Energy consumption is relevant only if the ship also has a Reactor component.
--- Example:
...
--- Coolant values are relevant only if the ship also has the Coolant component.
--- Example:
...
--- Sets the number of scan probes stocked by this ship.
...
--- Example:
--- ship:removeCustom("show_coolant_max") -- removes the custom item named "show_coolant_max"
...
--- Examples:
--- ship:commandTargetRotation(0) -- command the ship toward a heading of 90 degrees
--- heading = 180; ship:commandTargetRotation(heading - 90) -- command the ship toward a heading of 180 degrees
...
--- Example:
--- ship:commandImpulse(0.5) -- command this ship to engage forward half impulse
...
--- Example:
--- ship:commandJump(25000) -- initiate a 25U jump on the current heading
...
--- Example:
--- ship:commandAbortJump() -- aborts a jump if in progress
...
--- Example:
--- ship:commandFireTube(0) -- command firing tube 0
...
--- Example:
--- ship:commandFireTubeAtTarget(0, enemy) -- command firing tube 0 at target `enemy`
...
--- Example:
--- ship:commandSetShields(true) -- command raising shields
...
--- Example:
--- ship:commandMainScreenSetting("tactical") -- command setting the main screen view to tactical radar
...
--- Example:
--- ship:commandMainScreenOverlay("hidecomms") -- command setting the main screen view to hide the comms overlay
...
--- Example:
--- ship:commandSetSystemPowerRequest("impulse", 1.0) -- command setting the impulse drive power to 100%
...
--- Example:
--- ship:commandSetSystemCoolantRequest("impulse", 10.0) -- command setting the impulse drive coolant to 100%
...
--- Example:
--- ship:commandAnswerCommHail(false) -- commands to reject an active incoming hail
...
--- Example:
--- ship:commandSendComm(0) -- commands to select the first option in a comms dialogue
...
--- Example:
--- ship:commandSendCommPlayer("I will destroy you!") -- commands to send this message in the active text chat
...
--- Example:
--- ship:commandSetBeamFrequency(2)
...
--- Example:
--- ship:commandSetShieldFrequency(10) -- calibrate shield frequency to 600THz
...
--- Example:
--- ship:commandActivateSelfDestruct()
...
--- Example:
--- ship:commandCancelSelfDestruct()
...
--- Example:
--- ship:commandConfirmDestructCode(0, 46223) -- commands submitting 46223 as self-destruct confirmation code A
...
--- Example:
--- ship:commandCombatManeuverBoost(0.5) -- commands boosting forward at half combat maneuver capacity
...
--- Example:
--- ship:commandLaunchProbe(1000, 2000) -- commands launching a scan probe to coordinates x 1000, y 2000
...
--- Example:
--- ship:commandSetScienceLink(probe_object) -- link ScanProbe `probe` to this ship's science
...
--- Example:
--- ship:commandSetAlertLevel("red") -- commands red alert
...
--- ship:onProbeLaunch(function (player, probe)
...
--- ship:onProbeLink(function (player, probe)
...
--- ship:onProbeUnlink(function (player, probe)
