...
---
--- If this faction considers another faction to be hostile, it can target and fire weapons at it, and CPU ships with certain orders might pursue it.
...
--- If friendly, this faction acts as neutral but also shares short-range radar with player ships in Relay, and can grant reputation points to player ships of the same faction.
---
...
--- faction:setGMColor(255,128,255) -- uses purple icons for this faction's entities in GM and Spectator views
...
--- Sets the given faction to appear as hostile to entities of this faction.
--- For example, Ships of this faction can target and fire at ships of the given faction.
...
function Entity:setEnemy(other_faction)
    if self.components.faction_info == nil then
        error("setEnemy can only be called on factions.", 2)
    end
    if other_faction.components.faction_info == nil then
        error("setEnemy can only be called on factions.", 2)
    end
...
--- Sets the given faction to appear as friendly to entities of this faction.
--- For example, Player ships of this faction can gain reputation with it.
...
function Entity:setFriendly(other_faction)
    if self.components.faction_info == nil then
        error("setFriendly can only be called on factions.", 2)
    end
    if other_faction.components.faction_info == nil then
        error("setFriendly can only be called on factions.", 2)
    end
...
--- Sets the given faction to appear as neutral to entities of this faction.
...
function Entity:setNeutral(other_faction)
    if self.components.faction_info == nil then
        error("setNeutral can only be called on factions.", 2)
    end
    if other_faction.components.faction_info == nil then
        error("setNeutral can only be called on factions.", 2)
    end
...
function Entity:__setFactionRelation(other_faction, relation)
    ...
    self.components.faction_info[#self.components.faction_info + 1] =
        { other_faction = other_faction, relation = relation }
end
