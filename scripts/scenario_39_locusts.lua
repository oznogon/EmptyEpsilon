...
function init()
    ...
    print(
        string.format(
            "    ----    Scenario: Locust Swarm    ----    Version %s    ----    Tested with EE version %s    ----",
            scenario_version,
            ee_version
        )
    )
...
function setVariations()
    local swarm_config = {
        ["Tiny"] = {
            rings = 3,
            slots = 36,
            estimate = _("orders-comms", "thirty"),
        },
        ["Small"] = {
            rings = 4,
            slots = 58,
            estimate = _("orders-comms", "fifty"),
        },
        ["Normal"] = {
            rings = 5,
            slots = 88,
            estimate = _("orders-comms", "eighty"),
        },
        ["Large"] = {
            rings = 6,
            slots = 124,
            estimate = _("orders-comms", "one hundred"),
        },
        ["Huge"] = {
            rings = 7,
            slots = 166,
            estimate = _("orders-comms", "one hundred and fifty"),
        },
    }
    ...
    local basis_config = {
        ...
        ["Hornet"] = {
            template = "MT52 Hornet",
            desc = _("orders-comms", "hornet"),
        },
        ["Drone"] = {
            template = "Ktlitan Drone",
            desc = _("orders-comms", "drone"),
        },
    }
...
function constructEnvironment()
    ...
    repeat
        local ox, oy = vectorFromAngleNorth(
            random(0, 360),
            random(2000, 66000) + random(2000, 66000) + random(2000, 66000)
        )
        ox = ox + center_x
        oy = oy + center_y
        ...
        if #type_list > 0 then
            ...
            if insert_type == "WarpJammer" then
                closest_distance = math.min(closest_distance * 0.95, 20000)
            ...
            elseif insert_type == "mineblob" then
                ...
                placeMinefieldBlob(ox, oy, closest_distance * 0.37)
            elseif insert_type == "asteroidblob" then
                closest_distance = math.min(closest_distance, 15000)
                placeAsteroidBlob(ox, oy, closest_distance * 0.37)
    ...
    repeat
        local ox, oy =
            vectorFromAngleNorth(random(0, 360), random(6000, 200000))
        ox = ox + center_x
        oy = oy + center_y
        local obj_list = getObjectsInRadius(ox, oy, 20000)
        local closest_distance = 20000
        ...
        if #type_list > 0 then
            ...
            if insert_type == "Planet" and planet_count < 3 then
                ...
                if selected_planet ~= nil then
                    local scattered_planet = Planet():setPlanetRadius(
                        closest_distance / 2
                    ):setPosition(ox, oy)
                    scattered_planet:setCallSign(
                        selected_planet.name[math.random(
                            1,
                            #selected_planet.name
                        )]
                    )
                    scattered_planet:setPlanetSurfaceTexture(
                        selected_planet.texture.surface
                    )
                    if selected_planet.texture.atmosphere ~= nil then
                        scattered_planet:setPlanetAtmosphereTexture(
                            selected_planet.texture.atmosphere
                        )
                    end
                    if selected_planet.texture.cloud ~= nil then
                        scattered_planet:setPlanetCloudTexture(
                            selected_planet.texture.cloud
                        )
                    end
                    if selected_planet.color ~= nil then
                        scattered_planet:setPlanetAtmosphereColor(
                    ...
                    if selected_moon ~= nil then
                        local scattered_moon = Planet():setPlanetRadius(
                            closest_distance / 8
                        ):setPosition(
                            ox,
                            oy + closest_distance - closest_distance / 16
                        )
                        scattered_moon:setCallSign(
                            selected_moon.name[math.random(
                                1,
                                #selected_moon.name
                            )]
                        )
                        scattered_moon:setPlanetSurfaceTexture(
                            selected_moon.texture.surface
                        )
                        scattered_moon:setAxialRotationTime(random(500, 900))
                        scattered_moon:setOrbit(
                            scattered_planet,
                            random(100, 200)
                        )
                    end
            ...
            elseif insert_type == "WormHole" and wormhole_count < 3 then
                ...
                wh:setTargetPosition(center_x, center_y)
                    :onTeleportation(wormholeTax)
            ...
            elseif insert_type == "Station" then
                ...
                if spaced_station then
                    ...
                    if #faction_selection_list > 0 then
                        selected_faction = faction_selection_list[math.random(
                            1,
                            #faction_selection_list
                        )]
                    else
                        filled_out = true
                    end
                    if station_defend_dist[station_size] < closest_distance then
                        ...
                        if
                            random(1, 100)
                            < defense_platforms[station_size].chance
                        then
                            local dp_angle = random(0, 360)
                            for j = 1, defense_platforms[station_size].count do
                                local dp_x, dp_y = vectorFromAngle(
                                    dp_angle,
                                    defense_platforms[station_size].dist
                                )
                                local dp = CpuShip()
                                    :setTemplate("Defense platform")
                                    :setFaction(selected_faction)
                                    :setPosition(ox + dp_x, oy + dp_y)
                                    :orderStandGround()
                                dp:setCallSign(
                                    string.format(
                                        "%sDP%i%s",
                                        faction_letter[selected_faction],
                                        j,
                                ...
                                dp_angle = (
                                    ...
                                    + (
                                        360
                                        / defense_platforms[station_size].count
                                    )
                                ) % 360
                            end
                        end
                    elseif
                        defense_platforms[station_size].dist < closest_distance
                    then
                        local name_group = "RandomHumanNeutral"
                        local tsa = Artifact():setFaction(selected_faction)
                        local tpa = Artifact():setFaction(player_faction)
    ...
    for i, station in ipairs(station_list) do
        ...
        elseif tpa:isFriendly(station) then
            beginning_friendly_station_count = beginning_friendly_station_count
                + 1
        else
            beginning_neutral_station_count = beginning_neutral_station_count
                + 1
        end
    end
    tpa:destroy()
    for i = 1, math.random(10, 15) do
        local ox, oy = vectorFromAngle(
            random(0, 360),
            random(2000, 66000) + random(2000, 66000) + random(2000, 66000)
        )
        ...
        if random(1, 100) < 77 then
            ...
            if random(1, 100) < 41 then
                ...
                if random(1, 100) < 32 then
                    nx, ny = vectorFromAngle(
                        n2_angle + random(120, 240),
                        random(5000, 10000)
                    )
                    Nebula():setPosition(center_x + ox + nx, center_y + oy + ny)
                end
...
function placeAsteroidBlob(x, y, field_radius)
    ...
    repeat
        ...
        repeat
            ...
            local asteroid_space = (base_asteroid:getSize() + size)
                * random(1.05, 1.25)
...
function setPlayers(p)
    ...
    local player_spawn_x, player_spawn_y =
        vectorFromAngleNorth(random(0, 360), random(0, 5000))
...
function updatePlayerSoftTemplate(p)
    ...
    p:addCustomButton(
        "Operations",
        p.swarm_status_ops,
        _("buttonOperations", "Locust Status"),
        function()
            string.format("")
...
function locustStatus(p, console)
    ...
    if ship_count == 1 then
        out = string.format(
            _("msgRelay", "One Locust remains in sector %s"),
            closest_ship:getSectorName()
        )
    else
        out = string.format(
    ...
    p.locust_status_message[console] =
...
function wormholeTax(self, teleportee)
    string.format("")
    if
        isObjectType(teleportee, "CpuShip")
        or isObjectType(teleportee, "PlayerSpaceship")
    then
        teleportee:setSystemHealth(
            "beamweapons",
            teleportee:getSystemHealth("beamweapons") - 0.5
        )
        teleportee:setSystemHealth(
            "missilesystem",
            teleportee:getSystemHealth("missilesystem") - 0.5
        )
...
function testFormation()
    ...
    for i = 0, 5 do
        ...
        table.insert(
            third_ring,
            { ship = ship, x = r5x, y = r5y, dist = dist, angle = angle }
        )
        ...
        table.insert(
            fourth_ring,
            { ship = ship, x = r7x, y = r7y, dist = dist, angle = angle }
        )
        ...
        for j = 1, 4 do
            local r9x, r9y =
                vectorFromAngleNorth(60 * i + 120, ship_spacing * j)
            r9x = r9x + r8x
            r9y = r9y + r8y
            ...
            table.insert(
                fifth_ring,
                { ship = ship, x = r9x, y = r9y, dist = dist, angle = angle }
            )
        end
        ...
        if i > 0 then
            ...
            table.insert(
                second_interspersed_ring,
                { ship = ship, x = r3x, y = r3y, dist = dist, angle = angle }
            )
        end
    end
    print("First ring:")
    ...
    local ship = CpuShip()
        ...
        :setPosition(swarm_x, swarm_y)
        :setHeading(flight_angle)
    local leader_ship = ship
    for i, form in ipairs(hex_ring_positions) do
        local form_x, form_y = vectorFromAngleNorth(
            flight_angle + form.angle,
            form.dist * spawn_distance
        )
        local form_prime_x, form_prime_y =
            vectorFromAngle(form.angle, form.dist * spawn_distance)
        ship = CpuShip()
            :setTemplate("MT52 Hornet")
            :setPosition(swarm_x + form_x, swarm_y + form_y)
            :setHeading(flight_angle)
            :orderFlyFormation(leader_ship, form_prime_x, form_prime_y)
    end
...
function commsBriefing()
    ...
    addCommsReply(_("orders-comms", "How many is numerous?"), function()
        setCommsMessage(
            string.format(_("orders-comms", "Approximately %s"), swarm_estimate)
        )
        addCommsReply(
            _("orders-comms", "That's quite a few. What kind of ships?"),
            function()
                setCommsMessage(
                    string.format(
                        _(
                            "orders-comms",
                            "They look like %ss. The computer calls them locusts. They're very fast."
                        ),
                        locust_template_desc
                    )
                )
                addCommsReply(
                    _("orders-comms", "Should we be worried?"),
                    function()
                        setCommsMessage(
                            _(
                                "orders-comms",
                                "Individually, they should be easy to handle. As a group, they could be a problem."
                            )
                        )
                        addCommsReply(
                            _(
                            ...
                            function()
                                setCommsMessage(
                                    _(
                                        "orders-comms",
                                        "Get rid of them. Every last one of them."
                                    )
                                )
...
function commsGatherBriefing()
    ...
    addCommsReply(_("orders-comms", "Tell us about it."), function()
        setCommsMessage(
            _(
                "orders-comms",
                "All the Exuari on our sensors have started moving."
            )
        )
        addCommsReply(_("orders-comms", "Where are they going?"), function()
            ...
            addCommsReply(
                _(
                    "orders-comms",
                    "That could make it harder for us to get rid of them."
                ),
                function()
                    setCommsMessage(
                        _(
                            "orders-comms",
                            "How perceptive of you! We are so glad you are out there working to rid us of these Locusts. Keep up the good work!"
                        )
                    )
                    comms_target:setCommsFunction(nil)
                    comms_target:setCommsScript("comms_station.lua")
...
function getBriefingStation(p)
    ...
    if station_briefing == nil then
        if #briefing_stations > 0 then
            station_briefing =
                briefing_stations[math.random(1, #briefing_stations)]
        else
            station_briefing = alternate_briefing_stations[math.random(
                1,
                #alternate_briefing_stations
            )]
        end
    end
...
function update(delta)
            ...
            if #swarm_ships > 0 then
                ...
                for i, station in pairs(station_list) do
                    if station ~= nil and station:isValid() then
                        ...
                        if tpa:isEnemy(station) then
                            final_enemy_station_count = final_enemy_station_count
                                + 1
                        elseif tpa:isFriendly(station) then
                            final_friendly_station_count = final_friendly_station_count
                                + 1
                        else
                            final_neutral_station_count = final_neutral_station_count
                                + 1
                        end
                    end
                end
                ...
                globalMessage(
                    string.format(
                        ...
                        surviving_station_count,
                        final_friendly_station_count,
                        beginning_friendly_station_count,
                        final_neutral_station_count,
                        beginning_neutral_station_count,
                        final_enemy_station_count,
                        beginning_enemy_station_count
                    )
                )
                victory("Human Navy")
            end
    ...
    if
        ...
        and player ~= nil
        and player:isValid()
        and friendly_stations < 1
    then
        globalMessage(
            _("msgMainscreen", "All friendly stations have been destroyed.")
        )
        victory("Exuari")
    end
