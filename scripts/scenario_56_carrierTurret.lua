...
function init()
    ...
    if #offsetList > 0 then
        ...
        if start_count > max_repeat_loop then
            print(
                "exceeded max repeat loop when counting starting positions in init function"
            )
        end
    end
    ...
    playerCarrier =
        PlayerSpaceship():setFaction("Human Navy"):setTemplate(carrier_template)
    playerCarrier
        :setPosition(startx, starty)
        :setRotation(-90)
        :commandTargetRotation(-90)
        :setRepairDocked(true)
    if diagnostic then --make carrier ridiculously powerful for mission diagnostic test purposes
        playerCarrier
            :setBeamWeapon(0, 10, 0, 1500.0, 1.0, 104)
            :setBeamWeapon(1, 10, 180, 1500.0, 1.0, 104)
            :setBeamWeaponHeatPerFire(
                0,
                playerCarrier:getBeamWeaponHeatPerFire(0) * 0.01
            )
            :setBeamWeaponHeatPerFire(
                1,
                playerCarrier:getBeamWeaponHeatPerFire(1) * 0.01
            )
        playerCarrier
            :setBeamWeaponTurret(0, 270, 0, 6)
            :setBeamWeaponTurret(1, 270, 180, 6)
            :setBeamWeaponEnergyPerFire(0, 1)
            :setBeamWeaponEnergyPerFire(1, 1)
    end
    ...
    if fighter_template == "Formax" then
        ...
        playerBlade
            :setWeaponTubeDirection(0, 0)
            :setTubeLoadTime(0, 10)
            :setWeaponTubeExclusiveFor(0, "HVLI")
            :setTubeSize(0, "small")
        playerBlade
            :setWeaponTubeDirection(1, 180)
            :setTubeLoadTime(1, 15)
    ...
    elseif fighter_template == "Foil" then
        ...
        playerBlade
            :setWeaponTubeDirection(0, 180)
            :setTubeLoadTime(0, 20)
            :setWeaponTubeExclusiveFor(0, "Mine")
        playerBlade:setWeaponStorageMax("Mine", 1):setWeaponStorage("Mine", 1)
    else
        playerBlade = PlayerSpaceship()
            :setFaction("Human Navy")
            :setTemplate(fighter_template)
            :setJumpDrive(false)
            :setWarpDrive(false)
    end
    playerBlade
        :setPosition(startx - 240, starty)
        :commandDock(playerCarrier)
        :setRotation(-180)
        :commandTargetRotation(-180)
    playerBlade
        :setLongRangeRadarRange(10000)
        :setAutoCoolant(true)
        :commandSetAutoRepair(true)
    playerBlade.normal_long_range_radar = 10000
    playerBlade:addCustomButton(
        "Tactical",
        "shield",
        _("buttonTactical", "Shield"),
        function()
    ...
    if fighter_template_2 == "Red Jacket" then
        ...
        playerPoint = PlayerSpaceship()
            :setFaction("Human Navy")
            :setTemplate(fighter_template_2)
            :setJumpDrive(false)
            :setWarpDrive(false)
    end
    playerPoint
        :setPosition(startx + 125, starty)
        :commandDock(playerCarrier)
        :setRotation(0)
    ...
    setMovingAsteroids()
    plot2choices =
        { upgradeShipSpin, locateTargetEnemyBase, rescueDyingScientist }
...
function setVariations()
    ...
    local murphy_config = {
        ["Very Easy"] = { number = 0.25, rep = 80 },
        ["Easy"] = { number = 0.5, rep = 50 },
    ...
    gameTimeLimit = getScenarioTime()
        + time_config[getScenarioSetting("Timed")].length * 60
    ...
    if GMStartPlot2upgradeShipSpin ~= nil then
        addGMFunction(GMStartPlot2upgradeShipSpin, function()
            ...
            if match then
                ...
                addGMMessage(
                    _("msgGM", "The upgrade ship spin plot is not available.")
                )
            end
        ...
        addGMFunction(GMStartPlot2locateTargetEnemyBase, function()
            ...
            if match then
                ...
                addGMMessage(
                    _(
                        "msgGM",
                        "The locate target enemy base plot is no longer available."
                    )
                )
            end
        ...
        addGMFunction(GMStartPlot2rescueDyingScientist, function()
            ...
            if match then
                ...
                addGMMessage(
                    _(
                        "msgGM",
                        "The rescue dying scientist plot is no longer available."
                    )
                )
            end
        ...
        addGMFunction(GMStartPlot3upgradeBeamDamage, function()
            ...
            if match then
                ...
                addGMMessage(
                    _(
                        "msgGM",
                        "The upgrade beam damage plot is no longer available."
                    )
                )
            end
        ...
        addGMFunction(GMStartPlot3tractorDisabledShip, function()
            ...
            if match then
                ...
                addGMMessage(
                    _(
                        "msgGM",
                        "The tractor disabled ship plot is no longer available."
                    )
                )
            end
        ...
        addGMFunction(GMStartPlot3addTubeToShip, function()
            ...
            if match then
                ...
                addGMMessage(
                    _(
                        "msgGM",
                        "The add tube to ship plot is no longer available."
                    )
                )
            end
...
function buildStations()
    ...
    for j = 1, starting_friendly_station_count do
        ...
        local pStation =
            placeCTStation(psx, psy, "RandomHumanNeutral", stationFaction)
...
function transportPlot()
    if transport_spawn_time == nil then
        transport_spawn_time = getScenarioTime()
            + transportSpawnDelay
            + random(5, 45)
    end
    if getScenarioTime() > transport_spawn_time then
        ...
        if clean_count > max_repeat_loop then
            print(
                "exceeded max repeat loop when check for clean list in transportPlot function"
            )
        end
        if clean_list and #transportList < #stationList then
            ...
            local transport = CpuShip()
                :setTemplate(name)
                :setFaction(transport_faction)
                :setCommsFunction(commsShip)
                :setCallSign(generateCallSign(nil, transport_faction))
            ...
            if enemy_count > max_repeat_loop then
                print(
                    "exceeded max repeat loop when picking random station in transportPlot function"
                )
            end
            ...
            local ox, oy =
                vectorFromAngle(random(0, 360), random(25000, 40000), true)
                    ...
                    if #station.defense_fleet > 0 then
                        for j, ship in ipairs(station.defense_fleet) do
                            if ship ~= nil and ship:isValid() then
                                ...
                                then
                                    ship.self_preservation_decision_time = getScenarioTime()
                                        + random(4, 9)
                                end
                                if
                                    getScenarioTime()
                                    > ship.self_preservation_decision_time
                                then
                                    ship.self_preservation_decision_time = nil
                                    if
                                        ship.my_station ~= nil
                                        and ship.my_station:isValid()
                                    then
                                        local function shipHealthy(ship)
                                            ...
                                            if
                                                ship:getBeamWeaponRange(0) > 0
                                            then
                                                if
                                                    ship:getSystemHealth(
                                                        "beamweapons"
                                                    )
                                                    < ship:getSystemHealthMax(
                                                        "beamweapons"
                                        ...
                                        local function shipFull(ship)
                                            ...
                                            then
                                                if
                                                    ship:getWeaponStorage(
                                                        "Homing"
                                                    )
                                                    < ship:getWeaponStorageMax(
                                                        "Homing"
                                                    )
                                                then
                                                    return false
                                        ...
                                        if ship:isDocked(ship.my_station) then
                                            if
                                                ship.my_station:areEnemiesInRange(
                                                    10000
                                                )
                                            then
                                                ship:orderDefendTarget(
                                                    ship.my_station
                                                )
                                            else
                                                if
                                                    ship_healthy and ship_full
                                                then
                                                    ship:orderDefendTarget(
                                                        ship.my_station
                                                    )
                                            ...
                                            then
                                                ...
                                                if not ship_healthy then
                                                    ...
                                                    if not ship_full then
                                                        if
                                                            not ship.my_station:areEnemiesInRange(
                                                                15000
                                                            )
                                                        then
                                                            if
                        ...
                        if station.respawn_defense_fleet_time == nil then
                            station.respawn_defense_fleet_time = getScenarioTime()
                                + 120
                                + random(1, 60)
                        end
                        if
                            getScenarioTime()
                            > station.respawn_defense_fleet_time
                        then
        ...
        if not playerBlade:isValid() then
            ...
            addCommsReply(
                string.format(
                    _(
                        "station-comms",
                        "Get replacement %s fighter (%i reputation)"
                    ),
                    fighter_template,
                    replacement_cost
                ),
                function()
                    if comms_source:takeReputationPoints(replacement_cost) then
                        ...
                        setCommsMessage(
                            _(
                                "station-comms",
                                "A replacement fighter has been provided"
                            )
                        )
                    else
                        setCommsMessage(
        ...
        if not playerCarrier:isValid() then
            ...
            addCommsReply(
                string.format(
                    _(
                        "station-comms",
                        "Get replacement %s carrier (%i reputation)"
                    ),
                    carrier_template,
                    replacement_cost
                ),
                function()
                    if comms_source:takeReputationPoints(replacement_cost) then
                        ...
                        setComsMessage(
                            _(
                                "station-comms",
                                "A replacement carrier has been provided"
                            )
                        )
                    else
                        setCommsMessage(
    ...
    if goodCount > 0 then
        ...
        if not cargo_hold_empty then
            ...
            for i, p in ipairs(getActivePlayerShips()) do
                if
                    p == comms_target
                    and comms_source:isDocked(p)
                    and p.cargo > 0
                then
                    addCommsReply(
                        string.format(
                            _("trade-comms", "Transfer cargo to %s"),
                            p:getCallSign()
                        ),
                        function()
                            setCommsMessage(
...
function handleUndockedState()
    ...
    if not cargoHoldEmpty then
        ...
        for i, p in ipairs(getActivePlayerShips()) do
            if
                p == comms_target
                and distance(p, comms_source) < 1500
                and p.cargo > 0
            then
                addCommsReply(
                    string.format(
                    ...
                    function()
                        setCommsMessage(
                            _("trade-comms", "What would you like to transfer?")
                        )
                        for good, quantity in pairs(comms_source.goods) do
                            if quantity > 0 then
                                addCommsReply(good_desc[good], function()
...
function commsMissionChanges()
    if plot3name == "awaitingTractor" then
        if
            comms_target == tractorStation
            and comms_source == playerCarrier
            and distance(comms_source, comms_target) < 2000
        then
            addCommsReply(
                _("upgrade-comms", "Install tractor equipment"),
                function()
                    if playerCarrier:hasPlayerAtPosition("Engineering") then
                        tractorIntegrationMsg = "tractorIntegrationMsg"
                        playerCarrier:addCustomMessage(
                            "Engineering",
                            tractorIntegrationMsg,
                            string.format(
                                _(
                                    "-msgEngineer",
                                    "The tractor equipment has been transported aboard %s. You need to make the final connections for full installation"
                                ),
                                playerCarrier:getCallSign()
                            )
                        )
                    end
                    if playerCarrier:hasPlayerAtPosition("Engineering+") then
                    ...
                    playerCarrier:addCustomButton(
                        "Engineering",
                        tractorIntegrationButton,
                        _("-buttonEngineer", "Connect Tractor"),
                        connectTractor
                    )
                    tractorIntegrationButtonPlus =
                        "tractorIntegrationButtonPlus"
                    playerCarrier:addCustomButton(
                        "Engineering+",
                        tractorIntegrationButtonPlus,
                    ...
                    setCommsMessage(
                        _(
                            "upgrade-comms",
                            "Tractor equipment transferred to engine room"
                        )
                    )
                end
            )
    ...
    if plot3name == "tubeOfficer" then
        if
            comms_target == addTubeStation
            and distance(comms_source, comms_target) < 1500
        then
            tubePartQuantity = 0
            if comms_source.goods ~= nil then
                for good, quantity in pairs(comms_source.goods) do
            ...
            if tubePartQuantity > 0 then
                addCommsReply(
                    string.format(
                        _(
                            "upgrade-comms",
                            "Give %s to Boris Eggleston for additional tube"
                        ),
                        good_desc[tubePart]
                    ),
                    function()
                        if comms_source.tubeAdded then
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "You already have the extra tube"
                                )
                            )
                        else
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "I can install it point to the front or to the rear. Which would you prefer?"
                                )
                            )
                            addCommsReply(
                                _("upgrade-comms", "Front"),
                                function()
                                    comms_source.goods[tubePart] = comms_source.goods[tubePart]
                                        - 1
                                    comms_source.cargo = comms_source.cargo + 1
                                    ...
                                    setCommsMessage(
                                        _(
                                            "upgrade-comms",
                                            "You now have an additional homing torpedo tube pointing forward"
                                        )
                                    )
                                    mission_milestones = mission_milestones + 1
                                    local tube_count =
                                    ...
                                    if tube_count > 0 then
                                        ...
                                        for i = 1, tube_count do
                                            local tube_size =
                                                comms_source:getTubeSize(i - 1)
                                            if tube_size == "small" then
                                                comms_source.tube_size = comms_source.tube_size
                                                    .. "S"
                                            end
                                            ...
                                            if tube_size == "large" then
                                                comms_source.tube_size = p.tube_size
                                                    .. "L"
                                            end
                                        end
                                    end
                            ...
                            addCommsReply(_("upgrade-comms", "Rear"), function()
                                ...
                                comms_source:setWeaponTubeDirection(
                                    originalTubes,
                                    180
                                )
                                setCommsMessage(
                                    _(
                                        "upgrade-comms",
                                        "You now have an additional homing torpedo tube pointing to the rear"
                                    )
                                )
                                mission_milestones = mission_milestones + 1
                                local tube_count =
                ...
                addCommsReply(
                    _("upgrade-comms", "May I speak with Boris Eggleston?"),
                    function()
                        setCommsMessage(
                            _(
                                "upgrade-comms",
                        ...
                        addCommsReply(
                            _(
                                "upgrade-comms",
                                "Can you really add another weapons tube to our ship?"
                            ),
                            function()
                                setCommsMessage(
                                    string.format(
                                        _(
                                            "upgrade-comms",
                                            "Definitely. But I'll need %s before I can do it"
                                        ),
                                        good_desc[tubePart]
                                    )
                                )
    ...
    if plot3name == "beamPhysicist" then
        if
            comms_target == beamDamageStation
            and distance(comms_source, comms_target) < 1500
        then
            beamPart1Quantity = 0
            beamPart2Quantity = 0
            ...
            if beamPart1Quantity > 0 and beamPart2Quantity > 0 then
                addCommsReply(
                    string.format(
                        _(
                            "upgrade-comms",
                            "Give %s and %s to Frederico Booker for upgrade"
                        ),
                        good_desc[beamPart1],
                        good_desc[beamPart2]
                    ),
                    function()
                        if comms_source.beamDamageUpgrade then
                            ...
                            if comms_source:getBeamWeaponRange(0) > 0 then
                                comms_source.goods[beamPart1] = comms_source.goods[beamPart1]
                                    - 1
                                comms_source.goods[beamPart2] = comms_source.goods[beamPart2]
                                    - 1
                                ...
                                setCommsMessage(
                                    _(
                                        "upgrade-comms",
                                        "Your ship beam weapons now deal 50% more damage"
                                    )
                                )
                                mission_milestones = mission_milestones + 1
                            else
                                setCommsMessage(
                                    _(
                                        "upgrade-comms",
                                        "Your ship has no beam weapons to upgrade"
                                    )
                                )
                            end
                        end
                ...
                addCommsReply(
                    _("upgrade-comms", "Talk to Frederico Booker"),
                    function()
                        setCommsMessage(
                            string.format(
                                _(
                                    "upgrade-comms",
                                    "[Frederico Booker]\nGreetings, %s. What brings you to %s to talk to me?"
                                ),
                                comms_source:getCallSign(),
                                beamDamageStation:getCallSign()
                            )
                        )
                        addCommsReply(
                            _(
                                "upgrade-comms",
                                "Can you upgrade our beam weapons systems?"
                            ),
                            function()
                                setCommsMessage(
                                    string.format(
                                        _(
                                            "upgrade-comms",
                                            "[Frederico Booker]\nOh, you've heard about my research and the practical results? I can certainly upgrade the damage dealt by your beam weapons systems, but you'll need to provide me with %s and %s before I can complete the job"
                                        ),
                                        good_desc[beamPart1],
                                        good_desc[beamPart2]
                                    )
    ...
    if plot2name == "spinScientist" then
        if
            comms_target == spinScientistStation and not spinUpgradeAvailable
        then
            addCommsReply(
                _("upgrade-comms", "Speak with Paulina Lindquist"),
                function()
                    setCommsMessage(
                        string.format(
                            _(
                                "upgrade-comms",
                                "Greetings, %s, what can I do for you?"
                            ),
                            comms_source:getCallSign()
                        )
                    )
                    addCommsReply(
                        _(
                            "upgrade-comms",
                            "Do you want to apply your engine research?"
                        ),
                        function()
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "Over the years, I've discovered that research does not pay very well. Do you have some kind of compensation in mind? Gold, platinum or luxury would do nicely."
                                )
                            )
                            ...
                            if giftQuantity > 0 then
                                addCommsReply(
                                    _(
                                        "upgrade-comms",
                                        "Offer compensation from cargo aboard"
                                    ),
                                    function()
                                        local giftList = {}
                                        ...
                                        comms_source.goods[gift] = comms_source.goods[gift]
                                            - 1
                                        comms_source.cargo = comms_source.cargo
                                            + 1
                                        setCommsMessage(
                                            string.format(
                                                _(
                                                    "upgrade-comms",
                                                    "Thanks for the %s. I've transmitted instructions on conducting the upgrade. Any friendly station can do the upgrade, but you'll need to provide impulse cargo for the upgrade"
                                                ),
                                                good_desc[gift]
                                            )
                                        )
                                        ...
                                        plot2reminder = _(
                                            "orders-comms",
                                            "Get spin upgrade from friendly station for impulse"
                                        )
    ...
    if plot2name == "friendlyClue" then
        if comms_target == friendlyClueStation then
            addCommsReply(
                _("intelligence-comms", "Speak with Herbert Long"),
                function()
                    setCommsMessage(
                        string.format(
                            _(
                                "intelligence-comms",
                                "Well, if it isn't the good ship, %s! What brings you to %s?"
                            ),
                            comms_source:getCallSign(),
                            friendlyClueStation:getCallSign()
                        )
                    )
                    addCommsReply(
                        _(
                            "intelligence-comms",
                            "Please share your enemy base information"
                        ),
                        function()
                            setCommsMessage(
                                string.format(
                                    _(
                                        "intelligence-comms",
                                        "That's old news. Wouldn't you rather know about %s's leadership woes or the latest readings on unique stellar phenomenae in the area?"
                                    ),
                                    friendlyClueStation:getCallSign()
                                )
                            )
                            addCommsReply(
                                _(
                                    "intelligence-comms",
                                    "No, I just want to know about the enemy base"
                                ),
                                function()
                                    setCommsMessage(
                                        string.format(
                                            _(
                                                "intelligence-comms",
                                                "Well, that's easy. The name of the base is %s. Enjoy your stay on %s!"
                                            ),
                                            targetEnemyStation:getCallSign(),
                                            friendlyClueStation:getCallSign()
                                        )
                                ...
                                function()
                                    setCommsMessage(
                                        string.format(
                                            _(
                                                "intelligence-comms",
                                                "There are so many requests for transfers, %s may be understaffed by next week."
                                            ),
                                            friendlyClueStation:getCallSign()
                                        )
                                    )
                            ...
                            addCommsReply(
                                _(
                                    "intelligence-comms",
                                    "What kind of unique stellar phenomenae?"
                                ),
                                function()
                                    setCommsMessage(
                                        string.format(
                                            _(
                                                "intelligence-comms",
                                                "While we were in %s spying on %s, we picked up readings in a nebula hinting at the formation of a new star."
                                            ),
                                            targetEnemyStation:getSectorName(),
                                            targetEnemyStation:getCallSign()
                                        )
    ...
    if
        ...
        and comms_source:isFriendly(comms_target)
        and distance(comms_source, comms_target) < 2000
    then
        impulseQuantity = 0
        if comms_source.goods ~= nil then
        ...
        if impulseQuantity > 0 then
            addCommsReply(
                _("upgrade-comms", "Upgrade ship maneuverability for impulse"),
                function()
                    if comms_source.spinUpgrade then
                        setCommsMessage(
                            _("upgrade-comms", "You already have the upgrade")
                        )
                    else
                        comms_source.spinUpgrade = true
                        comms_source.goods["impulse"] = comms_source.goods["impulse"]
                            - 1
                        comms_source.cargo = comms_source.cargo + 1
                        ...
                        setCommsMessage(
                            _(
                                "upgrade-comms",
                                "Maneuverability upgraded by 50%"
                            )
                        )
                        mission_milestones = mission_milestones + 1
                    end
...
function addCommandLogButton(p)
    ...
    p:addCustomButton(
        "Relay",
        p.command_log_button_rel,
        _("buttonRelay", "Command Log"),
        function()
            string.format("")
    ...
    p:addCustomButton(
        "Operations",
        p.command_log_button_ops,
        _("buttonOperations", "Command Log"),
        function()
            string.format("")
...
function showPlayerCommandLog(p, console)
    local out = string.format(
        _("msgRelay", "Command log for %s (clock - message):"),
        p:getCallSign()
    )
    local sorted_log = {}
    ...
    p.show_player_command_log_msg =
        string.format("show_player_command_log_msg_%s", console)
        ...
        command_log = {
            {
                ...
                short = string.format(
                    _(
                        "msgRelay",
                        "Ktlitans are harassing us. Destroy their primary station, %s. Respond to other requests if desired. Protect carrier to reduce mission duration. Home station is %s."
                    ),
                    targetEnemyStation:getCallSign(),
                    homeStation:getCallSign()
                ),
            ...
            },
            {
                ...
                long = string.format(
                    _(
                        "goal-incCall",
                        "Paulina Lindquist, a noted research engineer recently published her latest theories on engine design. We believe her theories have practical applications to our ship maneuvering systems. You may want to talk to her about putting her theories into practice on our naval vessels. She's currently stationed on %s in %s."
                    ),
                    spinScientistStation:getCallSign(),
                    spinScientistStation:getSectorName()
                ),
                short = string.format(
                    _(
                        "msgRelay",
                        "Visit Paulina Lindquist on station %s in sector %s regarding ship spin upgrade."
                    ),
                    spinScientistStation:getCallSign(),
                    spinScientistStation:getSectorName()
                ),
            ...
            },
            {
                ...
                long = string.format(
                    _(
                        "goal-incCall",
                        "Herbert Long believes he has a lead on some information about the enemy base that has been the source of the harassing ships. He's stationed on %s in %s."
                    ),
                    friendlyClueStation:getCallSign(),
                    friendlyClueStation:getSectorName()
                ),
                short = string.format(
                    _(
                        "msgRelay",
                        "Talk to Herbert Long on %s in %s regarding enemy base."
                    ),
                    friendlyClueStation:getCallSign(),
                    friendlyClueStation:getSectorName()
                ),
            ...
            },
            {
                ...
                long = string.format(
                    _(
                        "goal-incCall",
                        "[%s in %s] Medical emergency: Engineering research scientist Terrence Forsythe has contracted a rare medical condition. After contacting nearly every station in the area, we found that doctor Geraldine Polaski on %s has the expertise and facilities to help. However, we don't have the necessary transport to get Terrence there in time - he has only a few minutes left to live. Can you take Terrence to %s?"
                    ),
                    scientistStation:getCallSign(),
                    scientistStation:getSectorName(),
                ...
                short = string.format(
                    _(
                        "msgRelay",
                        "Transport Terrence Forsythe from %s in %s to %s before he dies"
                    ),
                    scientistStation:getCallSign(),
                    scientistStation:getSectorName(),
                    doctorStation:getCallSign()
            ...
            },
            {
                ...
                long = string.format(
                    _(
                        "goal-incCall",
                        "Retired naval officer, Boris Eggleston has taken his expertise in miniaturization and come up with a way to add a missile tube to naval vessels. He's vacationing on %s in %s"
                    ),
                    addTubeStation:getCallSign(),
                    addTubeStation:getSectorName()
                ),
                short = string.format(
                    _(
                        "msgRelay",
                        "Get extra weapons tube from Boris Eggleston on %s in %s"
                    ),
                    addTubeStation:getCallSign(),
                    addTubeStation:getSectorName()
                ),
            ...
            },
            {
                ...
                long = string.format(
                    _(
                        "goal-incCall",
                        "There's a physicist turned maintenance technician named Frederico Booker that has developed some innovative beam weapon technology that could increase the damage produced by our beam weapons. He's based on %s in %s"
                    ),
                    beamDamageStation:getCallSign(),
                    beamDamageStation:getSectorName()
                ),
                short = string.format(
                    _(
                        "msgRelay",
                        "Talk to Frederico Booker on %s in %s about a beam upgrade"
                    ),
                    beamDamageStation:getCallSign(),
                    beamDamageStation:getSectorName()
                ),
            ...
            if not log.sent then
                ...
                for j, p in ipairs(getActivePlayerShips()) do
                    ...
                    if met then
                        if
                            p.command_log ~= nil
                            and p.command_log[i] ~= nil
                            and not p.command_log[i].received
                        then
                            if log.method == "relay" then
                                local long_msg = log.long
                                p:addCustomMessage(
                                    "Relay",
                                    "command_log_message_rel",
                                    long_msg
                                )
                                p:addCustomMessage(
                                    "Operations",
                                    "command_log_message_ops",
                                    long_msg
                                )
                                addCommandLogButton(p)
                                ...
                                if availableForComms(p) then
                                    if selected_message_station == nil then
                                        for k, station in
                                            ipairs(message_stations)
                                        do
                                            if
                                                station ~= nil
                                                and station:isValid()
                                            then
                                                selected_message_station =
                                                    station
                                                break
                                            else
                                                message_stations[k] =
                                    ...
                                    if selected_message_station ~= nil then
                                        local long_msg = log.long
                                        if i == 3 then
                                            long_msg = string.format(
                                                _(
                                                    "centralcommand-incCall",
                                                    "%s (%s until intersection)"
                                                ),
                                                long_msg,
                                                p.nova_demo_time_remaining
                                            )
                                        end
                                        selected_message_station:sendCommsMessage(
                                            p,
                                            long_msg
                                        )
                                        addCommandLogButton(p)
                                        p.command_log[i].received = true
                                        p.command_log[i].stamp =
                                            getScenarioTime()
                                    else
...
function commsShip()
    ...
    if comms_target.comms_data.goods == nil then
        comms_target.comms_data.goods = {}
        comms_target.comms_data.goods[commonGoods[math.random(1, #commonGoods)]] =
            { quantity = math.random(1, 3), cost = random(20, 80) }
        local shipType = comms_target:getTypeName()
        if shipType:find("Freighter") ~= nil then
            if
                shipType:find("Goods") ~= nil
            ...
            then
                ...
                if good_count > max_repeat_loop then
                    print(
                        "exceeded max repeat loop when counting goods in commsShip function"
                    )
                end
            end
        end
...
function friendlyComms()
    ...
    if shipType:find("Freighter") ~= nil then
        ...
        if hasWeapons then
            addCommsReply(
                _("shipAssist-comms", "Attack nearby enemy target"),
                function()
                    localTargetCount = 0
                    enemyTargetList = {}
                    etlCount = 0
                    for idx, obj in
                        ipairs(comms_target:getObjectsInRange(15000))
                    do
                        if comms_target:isEnemy(obj) then
                            addCommsReply(obj:getCallSign(), function()
                                setCommsMessage(
                                    string.format(
                                        _("shipAssist-comms", "Attacking %s"),
                                        obj:getCallSign()
                                    )
                                )
                    ...
                    if localTargetCount == 0 then
                        setCommsMessage(
                            _("shipAssist-comms", "No enemy targets nearby")
                        )
                    else
                        setCommsMessage(
                            _("shipAssist-comms", "Which enemy target?")
                        )
                    end
                    addCommsReply(_("Back"), commsShip)
                end
...
function freighterComms()
        ...
        if distance(comms_source, comms_target) < 5000 then
            ...
            then
                ...
                if goodsQuantity > 0 then
                    addCommsReply(
                        _("trade-comms", "What kind of cargo are you carrying?"),
                        function()
    ...
    elseif comms_target.comms_data.friendlyness > 33 then
        ...
        addCommsReply(
            string.format(
                _("trade-comms", "Where are you headed? (cost: %i reputation)"),
                destRep
            ),
            function()
...
function cargoTransfer()
    if
        ...
        and playerCarrier.cargo > 0
        and playerBlade:isValid()
        and playerBlade:isDocked(playerCarrier)
        and playerBlade.cargo < playerBlade.maxCargo
    then
        if bladeTransferButton == nil then
            ...
            playerBlade:addCustomButton(
                "Relay",
                bladeTransferButton,
                _("-buttonRelay", "Transfer Cargo"),
                bladeCargoTransfer
            )
            bladeTransferButtonOp = "bladeTransferButtonOp"
            playerBlade:addCustomButton(
                "Operations",
                bladeTransferButtonOp,
                _("-buttonOperations", "Transfer Cargo"),
                bladeCargoTransfer
            )
        end
    else
    ...
    if
        playerCarrier:isValid()
        and playerCarrier.cargo > 0
        and playerPoint:isValid()
        and playerPoint:isDocked(playerCarrier)
        and playerPoint.cargo < playerPoint.maxCargo
    then
    ...
    if
        playerBlade:isValid()
        and playerBlade.cargo > 0
        and playerCarrier:isValid()
        and playerBlade:isDocked(playerCarrier)
        and playerCarrier.cargo < playerCarrier.maxCargo
    then
        if
            carrier2bladeTransferButton == nil
            and carrier2bladeTransferButtonList == nil
        then
            carrier2bladeTransferButton = "carrier2bladeTransferButton"
            playerCarrier:addCustomButton(
                "Relay",
                carrier2bladeTransferButton,
                string.format("+Cargo to %s", playerBlade:getCallSign()),
            ...
            playerCarrier:addCustomButton(
                "Operations",
                carrier2bladeTransferButtonOps,
                string.format("+Cargo to %s", playerBlade:getCallSign()),
                carrier2bladeTransfer,
                50
            )
        ...
        if carrier2bladeTransferButtonList ~= nil then
            for i, good in ipairs(carrier2bladeTransferButtonList) do
                ...
                playerCarrier:removeCustom(
                    string.format("blade_good%sops", good)
                )
            end
        end
    ...
    then
        if
            ...
            and carrier2pointTransferButtonList == nil
        then
            carrier2pointTransferButton = "carrier2pointTransferButton"
            playerCarrier:addCustomButton(
                "Relay",
                carrier2pointTransferButton,
                string.format("+Cargo to %s", playerPoint:getCallSign()),
            ...
            playerCarrier:addCustomButton(
                "Operations",
                carrier2pointTransferButtonOps,
                string.format("+Cargo to %s", playerPoint:getCallSign()),
                carrier2pointTransfer,
                60
            )
        ...
        if carrier2pointTransferButtonList ~= nil then
            for i, good in ipairs(carrier2pointTransferButtonList) do
                ...
                playerCarrier:removeCustom(
                    string.format("point_good%sops", good)
                )
            end
        end
...
function carrier2bladeTransfer()
    ...
    for good, quantity in pairs(playerCarrier.goods) do
        if quantity > 0 then
            ...
            playerCarrier:addCustomButton(
                "Relay",
                string.format("blade_good%s", good),
                string.format(
                    "%s to %s",
                    good_desc[good],
                    playerBlade:getCallSign()
                ),
                function()
                    string.format("")
                    carrier2bladeTransferGood(good)
                end,
                50 + good_button_index
            )
            playerCarrier:addCustomButton(
                "Operations",
                string.format("blade_good%sops", good),
    ...
    playerCarrier:addCustomButton(
        "Relay",
        "exitBladeCargoTransfer",
        string.format("-%s Cargo", playerBlade:getCallSign()),
        function()
            if carrier2bladeTransferButtonList ~= nil then
                for i, good in ipairs(carrier2bladeTransferButtonList) do
                    playerCarrier:removeCustom(
...
function carrier2bladeTransferGood(good)
    ...
    playerCarrier:addCustomMessage(
        "Relay",
        "blade_cargo_transfer_msg_rel",
        string.format(
            "One %s transferred from %s to %s.",
            good_desc[good],
            playerCarrier:getCallSign(),
            playerBlade:getCallSign()
        )
...
function carrier2pointTransfer()
    ...
    for good, quantity in pairs(playerCarrier.goods) do
        if quantity > 0 then
            ...
            playerCarrier:addCustomButton(
                "Relay",
                string.format("point_good%s", good),
                string.format(
                    "%s to %s",
                    good_desc[good],
                    playerPoint:getCallSign()
                ),
                function()
                    string.format("")
                    carrier2pointTransferGood(good)
                end,
                60 + good_button_index
            )
            playerCarrier:addCustomButton(
                "Operations",
                string.format("point_good%sops", good),
    ...
    playerCarrier:addCustomButton(
        "Relay",
        "exitPointCargoTransfer",
        string.format("-%s Cargo", playerPoint:getCallSign()),
        function()
            if carrier2pointTransferButtonList ~= nil then
                for i, good in ipairs(carrier2pointTransferButtonList) do
                    playerCarrier:removeCustom(
...
function bladeCargoTransfer()
    ...
    if quantityToTransfer <= playerCarrier.cargo then
        ...
        if playerBlade:hasPlayerAtPosition("Relay") then
            ...
            playerBlade:addCustomMessage(
                "Relay",
                bladeCargoTransferredMsg,
                string.format(
                    _("-msgRelay", "Cargo aboard %s transferred to %s"),
                    playerBlade:getCallSign(),
                    playerCarrier:getCallSign()
                )
            )
        ...
        if playerBlade:hasPlayerAtPosition("Operations") then
            ...
            playerBlade:addCustomMessage(
                "Operations",
                bladeCargoTransferredMsgOp,
                string.format(
                    _("-msgOperations", "Cargo aboard %s transferred to %s"),
                    playerBlade:getCallSign(),
                    playerCarrier:getCallSign()
                )
            )
        end
        playerCarrier:addToShipLog(
            string.format(
                "Cargo transferred from %s",
                playerBlade:getCallSign()
            ),
            "Magenta"
        )
        ...
        if playerBlade:hasPlayerAtPosition("Relay") then
            insufficientCarrierCargoSpaceMsg =
                "insufficientCarrierCargoSpaceMsg"
            playerBlade:addCustomMessage(
                "Relay",
                insufficientCarrierCargoSpaceMsg,
                string.format(
                    _(
                        "-msgRelay",
                        "Insufficient space on %s to accept your cargo transfer"
                    ),
                    playerCarrier:getCallSign()
                )
            )
        ...
        if playerBlade:hasPlayerAtPosition("Operations") then
            insufficientCarrierCargoSpaceMsgOp =
                "insufficientCarrierCargoSpaceMsgOp"
            playerBlade:addCustomMessage(
                "Operations",
                insufficientCarrierCargoSpaceMsgOp,
...
function pointCargoTransfer()
    ...
    if quantityToTransfer <= playerCarrier.cargo then
        ...
        if playerPoint:hasPlayerAtPosition("Relay") then
            ...
            playerPoint:addCustomMessage(
                "Relay",
                pointCargoTransferredMsg,
                string.format(
                    _("-msgRelay", "Cargo aboard %s transferred to %s"),
                    playerPoint:getCallSign(),
                    playerCarrier:getCallSign()
...
function updatePlayerInventoryButton(p)
    ...
    if goodCount > 0 then --add inventory button when cargo acquired
        ...
        p:addCustomButton(
            "Single",
            "inventory_button_pil",
            _("inventory-buttonPilot", "Inventory"),
            function()
                string.format("")
                local out = playerShipCargoInventory(p)
                p:addCustomMessage("Operations", "inventory_message", out)
...
function playerShipCargoInventory(p)
    local out = string.format(
        _("inventory-msgRelay", "%s Current cargo:"),
        p:getCallSign()
    )
    local goodCount = 0
    if p.goods ~= nil then
...
function threadedPursuit(delta)
    ...
    if ef3 == nil then
        ...
        ef3 = spawnEnemies(scx + cpx, scy + cpy, 0.8)
...
function betweenPlot2fleet()
    if mission_diagnostic then
        print(
            "between plot 2 fleet - ef2 count:",
            #ef2,
            "clock:",
            getScenarioTime()
        )
...
function upgradeShipSpin(delta)
    ...
    if getScenarioTime() > plot2DelayTimer then
        if
            spinScientistStation == nil or not spinScientistStation:isValid()
        then
            local station_candidate_pool = {}
            for i, station in ipairs(stationList) do
                local tp = nil
                ...
                if
                    station:isValid()
                    and not station:isFriendly(tp)
                    and not station:isEnemy(tp)
                then
                    table.insert(station_candidate_pool, station)
                end
            end
            ...
            if spinScientistStation == nil then
                print(
                    "nil spin scientist station. candidate pool size:",
                    #station_candidate_pool
                )
            end
            for i, item in ipairs(command_log) do
                if item.name == "spin scientist" then
                    item.long = string.format(
                        _(
                            "goal-incCall",
                            "Paulina Lindquist, a noted research engineer recently published her latest theories on engine design. We believe her theories have practical applications to our ship maneuvering systems. You may want to talk to her about putting her theories into practice on our naval vessels. She's currently stationed on %s in %s."
                        ),
        ...
        plot2reminder = string.format(
            _(
                "orders-comms",
                "Visit Paulina Lindquist on %s in %s regarding ship spin upgrade."
            ),
            spinScientistStation:getCallSign(),
            spinScientistStation:getSectorName()
        )
        ...
        if mission_diagnostic then
            print(
                "upgrade ship spin (plot 2), switch to spin scientist - plot 2 delay timer expired"
            )
        end
    else
        if mission_diagnostic then
            if md_time_upgrade_spin == nil then
                ...
                print(
                    "upgrade ship spin (plot 2), switch to spin scientist - plot 2 delay timer not yet expired"
                )
            end
...
function spinScientist(delta)
    if mission_diagnostic then
        if md_time_spin == nil then
            ...
            if spinUpgradeAvailable then
                print(
                    "spin scientist upgrade available - waiting for all players to get the upgrade"
                )
            else
    ...
    if spinUpgradeAvailable then
        ...
        if
            spinPlayers == #getActivePlayerShips()
            or not spinScientistStation:isValid()
        then
            betweenPlot2fleet()
            getPlayerShip(-1):addReputationPoints(rep_bump)
        end
            ...
            for i, p in ipairs(getActivePlayerShips()) do
                p:addToShipLog(
                    string.format(
                        _(
                            "goal-shipLog",
                            "Paulina Lindquist, has been reassigned to station %s in %s."
                        ),
                        spinScientistStation:getCallSign(),
                        spinScientistStation:getSectorName()
                    ),
    ...
    if getScenarioTime() > plot2DelayTimer then
        ...
        if mission_diagnostic then
            if md_time_locate_target_enemy_base == nil then
                ...
                print(
                    "locate target enemy base - plot 2 delay timer not yet expired"
                )
            end
...
function friendlyClue(delta)
    ...
    if not friendlyClueStation:isValid() then
        ...
        for i, p in ipairs(getActivePlayerShips()) do
            p:addToShipLog(
                string.format(
                    _(
                        "goal-shipLog",
                        "Herbert Long has been reassigned to station %s in %s."
                    ),
                    friendlyClueStation:getCallSign(),
                    friendlyClueStation:getSectorName()
                ),
        ...
        plot2reminder = string.format(
            _(
                "orders-comms",
                "Visit Herbert Long on %s in %s regarding enemy base."
            ),
            friendlyClueStation:getCallSign(),
            friendlyClueStation:getSectorName()
        )
        ...
        if mission_diagnostic then
            print(
                "friendly clue for plot 2 - establish new friendly clue station:",
                friendlyClueStation:getCallSign()
            )
        end
    end
...
function rescueDyingScientist(delta)
    ...
    hullArtifact = Artifact()
        :setPosition(asx + wx, asy + wy)
        :setModel("artifact2")
        :allowPickup(false)
    ...
    hullArtifact
        :setDescriptions(
            _(
                ...
                "Artificially manufactured object of unknown purpose"
            ),
            _(
                "scienceDescription-artifact",
    ...
    if mission_diagnostic then
        print(
            "rescue dying scientist (plot 2) - scientist station established:",
            scientistStation:getCallSign()
        )
    end
end
        ...
        if getScenarioTime() > scientistDeathTimer then
            ...
            if mission_diagnostic then
                if md_time_get_sick_scientist == nil then
                    ...
                    print(
                        "get sick scientist (plot 2) - check for scientist death, scientist station validity, and player proximity to scientist station"
                    )
                end
        ...
        if scientistStation:isValid() then
            for i, p in ipairs(getActivePlayerShips()) do
                if distance(p, scientistStation) < 1500 then
                    ...
                    pickupMsg = string.format(
                        _(
                            "goal-shipLog",
                            "[%s] Terrence Forsythe has been transported aboard your ship. Please take him to Dr. Polaski on %s. "
                        ),
                        scientistStation:getCallSign(),
                        doctorStation:getCallSign()
                    )
                    minutesToLive = math.floor(
                        (scientistDeathTimer - getScenarioTime()) / 60
                    )
                    if minutesToLive == 0 then
                        pickupMsg = string.format(
                            _(
                                "goal-shipLog",
                                "%sWe believe he has less than a minute to live."
                            ),
                            pickupMsg
                        )
                    elseif minutesToLive == 1 then
                        pickupMsg = string.format(
                            _(
                                "goal-shipLog",
                                "%sWe think he has about a minute before he dies"
                            ),
                            pickupMsg
                        )
                    else
                        pickupMsg = string.format(
                            _(
                                "goal-shipLog",
                                "%sHe probably has about %i minutes to live"
                            ),
                            pickupMsg,
                            minutesToLive
                        )
                    ...
                    table.insert(command_log, {
                        ...
                        long = string.format(
                            _(
                                "goal-incCall",
                                "Terrence Forsythe transported aboard %s from station %s to be transported quickly to Dr. Polaski on station %s."
                            ),
                            playerWithScientist:getCallSign(),
                            scientistStation:getCallSign(),
                            doctorStation:getCallSign()
                        ),
                        short = string.format(
                            _(
                                "msgRelay",
                                "%s should take dying scientist to Dr. Polaski on station %s in %s"
                            ),
                            playerWithScientist:getCallSign(),
                            doctorStation:getCallSign(),
                            doctorStation:getSectorName()
                        ),
                        time = 0,
                        sent = false,
                        received = false,
            ...
            if pickupMsg ~= nil then
                for i, p in ipairs(getActivePlayerShips()) do
                    if p ~= playerWithScientist then
                        p:addToShipLog(
                            string.format(
                                _("goal-shipLog", "Terrence Forsyth aboard %s"),
                                playerWithScientist:getCallSign()
                            ),
                            "Magenta"
                        )
            ...
            for i, p in ipairs(getActivePlayerShips()) do
                p:addToShipLog(
                    _(
                        "goal-shipLog",
                        "The station with Dr. Polaski aboard has been destroyed."
                    ),
                    "Magenta"
                )
            end
        ...
        if doctorStation:isValid() then
            if playerWithScientist ~= nil and playerWithScientist:isValid() then
                if distance(playerWithScientist, doctorStation) < 1500 then
                    playerWithScientist:addToShipLog(
                        string.format(
                            _(
                                "goal-shipLog",
                                "[%s] We have received your emergency medical transport of research scientist Terrence Forsythe. Doctor Geraldine Polaski has stabalized his condition."
                            ),
                            doctorStation:getCallSign()
                        ),
                        "138,43,226"
                    ...
                    table.insert(command_log, {
                        ...
                        short = string.format(
                            _(
                                "msgRelay",
                                "Terrence Forsythe delivered to Dr. Polaski on station %s"
                            ),
                            doctorStation:getCallSign()
                        ),
                        time = 0,
            ...
            for i, p in ipairs(getActivePlayerShips()) do
                p:addToShipLog(
                    string.format(
                        _("goal-shipLog", "%s has been destroyed"),
                        doctorStation:getCallSign()
                    ),
                    "Magenta"
                )
...
function keyToArtifact(delta)
    if getScenarioTime() > scientistRecoveryTimer then
        keyMsg = string.format(
            _(
                "goal-shipLog",
                "[Terrence Forsythe] I can never repay you for saving my life. However, you might be able to use the practical results of my latest research. Near %s in %s you'll find a prototype for ship system integration. This prototype allows for the rapid and semi-automated repair of hull damage as directed by your Engineer. I have transmitted the key to allow you to use this prototype."
            ),
            scientistStation:getCallSign(),
            scientistStation:getSectorName()
        )
        ...
        plot2reminder = string.format(
            _("orders-comms", "Recover hull repair prototype near %s in %s."),
            scientistStation:getCallSign(),
            scientistStation:getSectorName()
        )
        ...
        table.insert(command_log, {
            ...
            short = string.format(
                _("msgRelay", "Recover hull repair prototype near %s in %s."),
                scientistStation:getCallSign(),
                scientistStation:getSectorName()
            ),
            time = 0,
...
function recoverHullArtifact(delta)
    ...
    for i, p in ipairs(getActivePlayerShips()) do
        ...
        if
            tempPlayerType == "Benedict"
            or tempPlayerType == "Kiriya"
            or tempPlayerType == "Saipan"
        then
...
function installAutoHullRepair()
    ...
    playerWithAutoHullRepair:addCustomMessage(
        "Engineering",
        hullUseMsg,
        string.format(
            _(
                "-msgEngineer",
                "Hull repair prototype installed. Limited to %i uses"
            ),
            repairHullUses
        )
    )
    ...
    playerWithAutoHullRepair:addCustomButton(
        "Engineering",
        repairHullButton,
        string.format(_("-buttonEngineer", "Repair Hull (%i)"), repairHullUses),
        repairHull
    )
    repairHullButtonPlus =
        string.format("repairHullButtonPlus%i", repairHullUses)
    playerWithAutoHullRepair:addCustomButton(
        "Engineering+",
        repairHullButtonPlus,
        string.format(_("-buttonEngineer+", "Repair Hull (%i)"), repairHullUses),
...
function repairHull()
    if
        playerWithAutoHullRepair:getHull()
        < playerWithAutoHullRepair:getHullMax()
    then
        ...
        if repairHullUses > 0 then
            repairHullButton =
                string.format("repairHullButton%i", repairHullUses)
            playerWithAutoHullRepair:addCustomButton(
                "Engineering",
                repairHullButton,
                string.format(
                    _("-buttonEngineer", "Repair Hull (%i)"),
                    repairHullUses
                ),
                repairHull
            )
...
function scientistDies()
    for i, p in ipairs(getActivePlayerShips()) do
        p:addToShipLog(
            _("goal-shipLog", "Terrence Forsythe has perished"),
            "Magenta"
        )
    end
    betweenPlot2fleet()
                ...
                if station.comms_data ~= nil then
                    if station.comms_data.goods ~= nil then
                        for j, good in ipairs(tube_cargo) do
                            if
                                station.comms_data.goods[good] ~= nil
                                and station.comms_data.goods[good].quantity
                                    > 0
                            then
        ...
        for i, p in ipairs(getActivePlayerShips()) do
            p:addToShipLog(
                string.format(
                    _(
                        "goal-shipLog",
                        "Boris Eggleston changed his vacation spot to %s in %s"
                    ),
                    addTubeStation:getCallSign(),
                    addTubeStation:getSectorName()
                ),
...
function upgradeBeamDamage(delta)
    ...
    if getScenarioTime() > plot3DelayTimer then
        ...
        local beam_cargo =
            { "gold", "platinum", "tritanium", "dilithium", "cobalt" }
        ...
        for i, p in ipairs(getActivePlayerShips()) do
            p:addToShipLog(
                string.format(
                    _(
                        "goal-shipLog",
                        "Frederico Booker has moved to station %s in %s"
                    ),
                    beamDamageStation:getCallSign(),
                    beamDamageStation:getSectorName()
                ),
...
function tractorDisabledShip(delta)
    ...
    if getScenarioTime() > plot3DelayTimer then
        if playerCarrier:isValid() then
            ...
            local tpx, tpy =
                vectorFromAngle(random(0, 360), random(40000, 45000))
            local strikeShipNames = {
                "Cropper",
                "Dunner",
                "Forthwith",
                "Stellar",
            ...
            tractorShip
                ...
                :orderStandGround()
                :setCallSign(tableSelectRandom(strikeShipNames))
            for i, p in ipairs(getActivePlayerShips()) do
                p:addToShipLog(
                    string.format(
                        _(
                            "goal-shipLog",
                            "[%s] Help requested: Our engines are damaged beyond our ability to repair. We are located in %s."
                        ),
                        tractorShip:getCallSign(),
                        tractorShip:getSectorName()
                    ),
                    "#556b2f"
                )
            end
            ...
            table.insert(command_log, {
                ...
                long = string.format(
                    _(
                        "goal-incCall",
                        "%s requests help stating: Our engines are damaged beyond our ability to repair. We are located in sector %s."
                    ),
                    tractorShip:getCallSign(),
                    tractorShip:getSectorName()
                ),
                short = string.format(
                    _(
                        "msgRelay",
                        "Help %s in sector %s with their damaged engines"
                    ),
                    tractorShip:getCallSign(),
                    tractorShip:getSectorName()
                ),
        ...
        if playerCarrier:isValid() then
            playerCarrier:addToShipLog(
                string.format(
                    _(
                        "goal-shipLog",
                        "Station %s in %s has tractor equipment you can use to tractor %s in for repairs"
                    ),
                    tractorStation:getCallSign(),
                    tractorStation:getSectorName(),
                    tractorShip:getCallSign()
                ),
                "Magenta"
            )
            plot3reminder = string.format(
                _(
                    "orders-comms",
                    "Install tractor equipment in %s from station %s in %s"
                ),
                playerCarrier:getCallSign(),
                tractorStation:getCallSign(),
                tractorStation:getSectorName()
...
function awaitingTractor(delta)
    if playerCarrier:isValid() then
        if tractorShip:isValid() then
            ...
            if
                tractorInstalled
                and distance(playerCarrier, tractorShip) < 5000
            then
                enableTractorOn()
                enableTractorOff()
                if tractorShip:isDocked(playerCarrier) then
                    if tractorPlayerDockMsg == nil then
                        ...
                        if playerCarrier:hasPlayerAtPosition("Weapons") then
                            playerCarrier:addCustomMessage(
                                "Weapons",
                                tractorPlayerDockMsg,
                                string.format(
                                    _(
                                        "-msgWeapons",
                                        "%s has been tractored to %s and is now docked"
                                    ),
                                    tractorShip:getCallSign(),
                                    playerCarrier:getCallSign()
                                )
                    ...
                    if repairStation == nil then
                        ...
                        if repairStation ~= nil then
                            playerCarrier:addToShipLog(
                                string.format(
                                    _(
                                        "goal-shipLog",
                                        "Tractor %s to %s to be repaired"
                                    ),
                                    tractorShip:getCallSign(),
                                    repairStation:getCallSign()
                                ),
                                "Magenta"
                            )
                            plot3reminder = string.format(
                                _("orders-comms", "Tractor %s to %s"),
                                tractorShip:getCallSign(),
                                repairStation:getCallSign()
                            )
                            table.insert(command_log, {
                        ...
                        if repairStation:isValid() then
                            if tractorShip:isValid() then
                                if
                                    distance(repairStation, tractorShip)
                                        < 1500
                                    or playerCarrier:getDockedWith()
                                        == repairStation
                                then
                                    playerCarrier:addToShipLog(
                                        string.format(
                                            _(
                                                "goal-shipLog",
                                                "Station %s says: Thanks for bringing %s in for repairs, %s. We'll take it from here"
                                            ),
                                            repairStation:getCallSign(),
                                            tractorShip:getCallSign(),
                                            playerCarrier:getCallSign()
                                    ...
                                    plot3reminder = nil
                                    plot3 = awaitRepairs
                                    getPlayerShip(-1):addReputationPoints(
                                        rep_bump
                                    )
                                    table.insert(command_log, {
                                        ...
                                        short = string.format(
                                            _(
                                                "msgRelay",
                                                "%s turned over to %s for repairs."
                                            ),
                                            tractorShip:getCallSign(),
                                            repairStation:getCallSign()
                                        ),
                                    ...
                                    then
                                        playerCarrier:addToShipLog(
                                            string.format(
                                                _(
                                                    "goal-shipLog",
                                                    "[%s] Greetings, %s. You'll need to tractor %s to within 1.5U before we can start repairs"
                                                ),
                                                repairStation:getCallSign(),
                                                playerCarrier:getCallSign(),
                                                tractorShip:getCallSign()
...
function connectTractor()
    ...
    if playerCarrier:hasPlayerAtPosition("Engineering") then
        ...
        playerCarrier:addCustomMessage(
            "Engineering",
            tractorConfirmationMsg,
            _(
                "-msgEngineer",
                "The tractor equipment has been fully integrated with ship systems. It is ready for the weapons officer to activate at the appropriate time"
            )
        )
    end
    if playerCarrier:hasPlayerAtPosition("Engineering+") then
    ...
    if playerCarrier:hasPlayerAtPosition("Weapons") then
        ...
        playerCarrier:addCustomMessage(
            "Weapons",
            wTractorConfirmationMsg,
            string.format(
                _(
                    "-msgWeapons",
                    "Tractor equipment installed.\nWhen %s is in range of the disabled ship, %s, You can engage the tractor system via the (Tractor On) button. This system works with %s's on board systems to draw and maneuver %s to %s until it is docked"
                ),
                playerCarrier:getCallSign(),
                tractorShip:getCallSign(),
                tractorShip:getCallSign(),
    ...
    if getScenarioTime() > repairDelayTimer then
        ...
        for i, p in ipairs(getActivePlayerShips()) do
            p:addToShipLog(
                string.format(
                    _(
                        "goal-shipLog",
                        "[%s] Our engines have been repaired and we stand ready to assist"
                    ),
                    tractorShip:getCallSign()
                ),
                "#556b2f"
        ...
        table.insert(command_log, {
            ...
            long = string.format(
                _(
                    "goal-incCall",
                    "[From %s] Our engines have been repaired and we stand ready to assist"
                ),
                tractorShip:getCallSign()
            ),
            short = string.format(
                _("msgRelay", "%s is ready to help us in our mission."),
                tractorShip:getCallSign()
            ),
            time = 0,
            sent = false,
...
function doNotPush(delta)
    ...
    if playerCarrier:hasPlayerAtPosition("Science") then
        ...
        playerCarrier:addCustomMessage(
            ...
            _(
                "-msgScience",
                "Internal security sensors indicate ship systems hacked by unknown source. For a moment I thought I heard evil laughter"
            )
        )
    end
...
function setPlayers()
    ...
    for i, pobj in ipairs(getActivePlayerShips()) do
        ...
        if not pobj.nameAssigned then
            ...
            if
                tempPlayerType == "Striker"
                or tempPlayerType == "MP52 Hornet"
                or tempPlayerType == "ZX-Lindworm"
            then
            ...
            local ship_name =
                tableRemoveRandom(player_ship_names_for[tempPlayerType])
            if ship_name ~= nil then
                pobj:setCallSign(ship_name)
            else
                pobj:setCallSign(
                    tableSelectRandom(player_ship_names_for["Leftovers"])
                )
                pobj.shipScore = 24
                pobj.maxCargo = 5
            end
...
function spawnEnemies(xOrigin, yOrigin, danger, enemyFaction)
    ...
    while enemyStrength > 0 do
        ...
        ship = CpuShip()
            ...
            :setCallSign(generateCallSign(nil, enemyFaction))
            :orderRoaming()
...
function endStatistics()
    ...
    gMsg = string.format(
        _("msgMainscreen", "%i out of %i friendly stations survived."),
        friendly_station_count,
        startingFriendlyStations
    )
    gMsg = string.format(
        _("msgMainscreen", "%s\n%i out of %i neutral stations survived."),
        gMsg,
        neutral_station_count,
        startingNeutralStations
    )
    gMsg = string.format(
        _("msgMainscreen", "%s\n%i out of %i enemy stations survived."),
        gMsg,
        enemy_station_count,
        startingEnemyStations
    )
    gMsg = string.format(
        _("msgMainscreen", "%s\nMission milestones completed:%i"),
        gMsg,
        mission_milestones
    )
    rankVal = friendly_station_count / startingFriendlyStations * 0.6
        + neutral_station_count / startingNeutralStations * 0.2
        + (1 - enemy_station_count / startingEnemyStations) * 0.2
    if missionVictory then
        ...
        gMsg = string.format(
            _(
                "msgMainscreen",
                "%s\nRank after several military leaders disappeared: %s"
            ),
            gMsg,
            rank
        )
    ...
    gMsg = string.format(
        _("msgMainscreen", "%s\nGame duration: %s"),
        gMsg,
        formatTime(getScenarioTime())
    )
    gMsg = string.format(
        _(
            "msgMainscreen",
            "%s\nSelected difficulty options: Enemy power: %s, Murphy: %s"
        ),
        gMsg,
        getScenarioSetting("Enemies"),
        getScenarioSetting("Murphy")
    )
    gMsg = string.format(
        _("msgMainscreen", "%s\nCarrier type: %s"),
        gMsg,
        getScenarioSetting("Carrier")
    )
    if getScenarioSetting("Carrier") == "Random" then
        gMsg =
            string.format(_("msgMainscreen", "%s (%s)"), gMsg, carrier_template)
    end
    gMsg = string.format(
        _("msgMainscreen", "%s\nFighter type 1: %s"),
        gMsg,
        getScenarioSetting("Fighter1")
    )
    if getScenarioSetting("Fighter1") == "Random" then
        gMsg =
            string.format(_("msgMainscreen", "%s (%s)"), gMsg, fighter_template)
    end
    gMsg = string.format(
        _("msgMainscreen", "%s\nFighter type 2: %s"),
        gMsg,
        getScenarioSetting("Fighter2")
    )
    ...
    for i, p in ipairs(getActivePlayerShips()) do
        ...
        if p.tube_size ~= nil then
            local tube_size_banner = string.format(
                _("tabWeaponsTacticalSingle", "%s tubes: %s"),
                p:getCallSign(),
                p.tube_size
            )
            if #p.tube_size == 1 then
                tube_size_banner = string.format(
                    _("tabWeaponsTacticalSingle", "%s tube: %s"),
                    p:getCallSign(),
                    p.tube_size
                )
            end
