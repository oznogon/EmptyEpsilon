...
function init()
    ...
    my_ship = CpuShip()
        :setCallSign(string.format("%s %s", station_name, name))
        :setCommsScript("")
        :setCommsFunction(commsDefendShip)
        :setFactionId(faction_id)
        :setPosition(position_x, position_y)
        :setTemplate(template)
        :setScanned(true)
        :orderDefendTarget(my_station)
end
function shipHealthy()
    if my_ship:getHull() < my_ship:getHullMax() then
        return false
    end
    if
        my_ship:getSystemHealth("reactor")
        < my_ship:getSystemHealthMax("reactor")
    then
        return false
    end
    if
        my_ship:getSystemHealth("impulse")
        < my_ship:getSystemHealthMax("impulse")
    then
        return false
    end
    if
        my_ship:getSystemHealth("maneuver")
        < my_ship:getSystemHealthMax("maneuver")
    then
        return false
    end
    if my_ship:getBeamWeaponRange(0) > 0 then
        if
            my_ship:getSystemHealth("beamweapons")
            < my_ship:getSystemHealthMax("beamweapons")
        then
            return false
        end
    end
    if my_ship:getWeaponTubeCount() > 0 then
        if
            my_ship:getSystemHealth("missilesystem")
            < my_ship:getSystemHealthMax("missilesystem")
        then
            return false
        end
    end
    if my_ship:hasWarpDrive() then
        if
            my_ship:getSystemHealth("warp")
            < my_ship:getSystemHealthMax("warp")
        then
            return false
        end
    end
    if my_ship:hasJumpDrive() then
        if
            my_ship:getSystemHealth("jumpdrive")
            < my_ship:getSystemHealthMax("jumpdrive")
        then
            return false
        end
    end
    if my_ship:getShieldCount() > 0 then
        if
            my_ship:getSystemHealth("frontshield")
            < my_ship:getSystemHealthMax("frontshield")
        then
            return false
        end
    end
    if my_ship:getShieldCount() > 1 then
        if
            my_ship:getSystemHealth("rearshield")
            < my_ship:getSystemHealthMax("rearshield")
        then
            return false
        end
    end
...
function shipFull()
    if my_ship:getWeaponTubeCount() > 0 then
        if
            my_ship:getWeaponStorage("Homing")
            < my_ship:getWeaponStorageMax("Homing")
        then
            return false
        end
        if
            my_ship:getWeaponStorage("HVLI")
            < my_ship:getWeaponStorageMax("HVLI")
        then
            return false
        end
        if
            my_ship:getWeaponStorage("EMP") < my_ship:getWeaponStorageMax("EMP")
        then
            return false
        end
        if
            my_ship:getWeaponStorage("Nuke")
            < my_ship:getWeaponStorageMax("Nuke")
        then
            return false
        end
    end
    ...
    if my_station ~= nil and my_station:isValid() then
        ...
        if check_timer < 0 then
            ...
            if inactivity_count > inactivity_max then
                my_station.comms_data.idle_defense_fleet[name] =
                    my_ship:getTypeName()
                my_ship:destroy()
                destroyScript()
            end
