...
function init()
    ...
    print(
        string.format(
            "    ----    Scenario: Fermi 500    ----    Version %s    ----    Tested with EE version %s    ----",
            scenario_version,
            ee_version
        )
    )
    ...
    repeat
        ...
        leg3length =
            distance(racePoint3x, racePoint3y, racePoint4x, racePoint4y)
        raceLength = (leg1length + leg2length + leg3length + leg4length)
            / 1000
            * 3
...
function mainGMButtonsDuringPause()
    ...
    addGMFunction(
        string.format(_("buttonGM", "+Start Delay: %i"), raceStartDelay / 60),
        setStartDelay
    )
    addGMFunction(
        string.format(_("buttonGM", "+Patience: %i"), patienceTimeLimit / 60),
    ...
    if predefined_player_ships ~= nil then
        addGMFunction(_("buttonGM", "Random PShip Names"), function()
            addGMMessage(
                _(
                    "msgGM",
                    "Player ship names will be selected at random.\nControl codes will be randomly generated"
                )
            )
            predefined_player_ships = nil
            mainGMButtons()
    ...
    addGMFunction(_("buttonGM", "Show Statistics"), function()
        local out =
            _("msgGM", "Not much to show since the game is still paused")
        print(out)
        local player_list = getActivePlayerShips()
        local player_count =
            string.format(_("msgGM", "Total player ships: %i"), #player_list)
        print(player_count)
        out = string.format("%s\n%s", out, player_count)
        for index, p in ipairs(player_list) do
            local player_line = string.format(
                _("msgGM", "%2i Name:%s, Type:%s"),
                index,
                p:getCallSign(),
                p:getTypeName()
            )
...
function setPatienceTimeLimit()
    ...
    if patienceTimeLimit < 3000 then
        addGMFunction(
            string.format(
                _("buttonGM", "%i Patience + -> %i"),
                patienceTimeLimit / 60,
                (patienceTimeLimit + 300) / 60
            ),
            function()
...
function setStartDelay()
    ...
    if raceStartDelay < 1200 then
        addGMFunction(
            string.format(
                _("buttonGM", "%i Delay + -> %i"),
                raceStartDelay / 60,
                (raceStartDelay + 60) / 60
            ),
            function()
    ...
    addGMFunction(_("buttonGM", "Show statistics"), function()
        ...
        for name, details in pairs(stats.ship) do
            ...
            out = string.format(
                ...
                out,
                details.lap_count,
                details.waypoint_goal,
                details.drone_points
            )
        end
        if raceStartDelay < 0 then
            if player_count == original_player_count then
                out = string.format(
                    _(
                        "msgGM",
                        "%s\n\nWith %i racers, we have the following points awarded for final race place:"
                    ),
                    out,
                    player_count
                )
                ...
                out = string.format(
                    _(
                        "msgGM",
                        "%s\n\nWith %i racers remaining from the original %i registrants, we have the following points awarded for final race place:"
                    ),
                    out,
                    player_count,
...
function setStations()
    ...
    stationTimer
        ...
        :setDescription(_("scienceDescription-station", "Race Timing Facility"))
        :setCallSign("Timer")
    table.insert(stationList, stationTimer)
    --Vaiken
    stationVaiken = placeStation(
        random(-10000, 5000),
        random(5000, 9000),
        "Vaiken",
        "Human Navy",
    ...
    local xMarconi, yMarconi =
        vectorFromAngle(marconiAngle, random(12500, 15000))
    stationMarconi = placeStation(
        xMarconi,
        yMarconi,
        "Marconi",
        "Independent",
    ...
    --Alcaleica
    xAlcaleica, yAlcaleica =
        vectorFromAngle(muddAngle + random(60, 120), random(12500, 15000))
    stationAlcaleica = placeStation(
        xAlcaleica,
        yAlcaleica,
        "Alcaleica",
        "Independent",
    ...
    --California
    stationCalifornia = placeStation(
        random(-90000, -70000),
        random(-15000, 25000),
        "California",
        "Human Navy",
    ...
    stationVactel =
        placeStation(xVactel, yVactel, "Vactel", "Independent", "Small Station")
    ...
    stationDeer =
        placeStation(xDeer, yDeer, "Deer", "Independent", "Small Station")
    ...
    --Research-11
    stationResearch11 = placeStation(
        random(-75000, -55000),
        random(-50000, -25000),
        "Research-11",
        "Independent",
    ...
    stationGrap =
        placeStation(xGrap, yGrap, "Grap", "Independent", "Small Station")
    ...
    stationGrup =
        placeStation(xGrup, yGrup, "Grup", "Independent", "Small Station")
    ...
    createRandomAlongArc(
        Asteroid,
        afd + longGrupEnd,
        xGrup + xLongAngleGrup,
        yGrup + yLongAngleGrup,
        longGrup,
        negAxisGrup,
        negAxisGrup + longGrupEnd,
        spreadGrup
    )
    local xShortAngleGrup, yShortAngleGrup =
        vectorFromAngle(axisGrup, shortGrup)
    ...
    createRandomAlongArc(
        Asteroid,
        afd + shortGrupEndQ,
        xGrup + xShortAngleGrup,
        yGrup + yShortAngleGrup,
        shortGrup,
        shortGrupEnd,
        negAxisGrup,
        spreadGrup
    ...
    local xGanalda, yGanalda =
        vectorFromAngle(ganaldaAngle, random(120000, 150000))
    stationGanalda =
        placeStation(xGanalda, yGanalda, "Ganalda", "Kraylor", "Medium Station")
    ...
    stationEmpok =
        placeStation(xEmpok, yEmpok, "Empok Nor", "Exuari", "Large Station")
    ...
    stationTic =
        placeStation(xTic, yTic, "Ticonderoga", "Kraylor", "Medium Station")
    ...
    station_upgrade_list = {
        {
            station = stationZefram,
            upgrade = _("upgrade-comms", "jump drive"),
            desc = _(
                "upgrade-comms",
                "We can upgrade your jump drive maximum range for nanites or robotic goods"
            ),
        },
        {
            station = stationCarradine,
            upgrade = _("upgrade-comms", "impulse drive"),
            desc = string.format(
                _(
                    "upgrade-comms",
                    "We can increase the speed of your impulse engines by %.2f percent for tritanium or dilithium"
                ),
                impulseBump
            ),
        },
        {
            station = spinStation,
            upgrade = _("upgrade-comms", "maneuver"),
            desc = string.format(
                _(
                    "upgrade-comms",
                    "We can increase the your rotate speed by %.2f percent for %s"
                ),
                spinBump,
                good_desc[spinComponent]
            ),
        },
        {
            station = stationMarconi,
            upgrade = _("upgrade-comms", "beam range"),
            desc = string.format(
                _(
                    "upgrade-comms",
                    "We can increase the range of your beam weapons by %.2f percent for %s"
                ),
                beamRangeBump,
                good_desc[beamComponent]
            ),
        },
        {
            station = tubeStation,
            upgrade = _("upgrade-comms", "extra missile tube"),
            desc = string.format(
                _(
                    "upgrade-comms",
                    "We can add a homing missile tube to your ship for %s"
                ),
                good_desc[tubeComponent]
            ),
        },
        {
            station = shieldStation,
            upgrade = _("upgrade-comms", "shield"),
            desc = string.format(
                _(
                    "upgrade-comms",
                    "We can upgrade your shields by %.2f percent for %s"
                ),
                shieldBump,
                good_desc[shieldComponent]
            ),
        },
        {
            station = stationNefatha,
            upgrade = _("upgrade-comms", "energy capacity"),
            desc = string.format(
                _(
                    "upgrade-comms",
                    "We can upgrade your energy capacity by 25 percent for %s"
                ),
                good_desc[energyComponent]
            ),
        },
    }
    if hazards then
...
function handleDockedState()
                ...
                if raceStartDelay > 0 then
                    ordMsg = ordMsg
                        .. string.format(
                            _(
                                "orders-comms",
                                "\n%i Seconds remain until start of race"
                            ),
                            raceStartDelay
                        )
                else
                    if comms_source.goal ~= nil then
                        ordMsg = ordMsg
                            .. string.format(
                                _(
                                    "orders-comms",
                                    "\nImmediate goal: race waypoint %i"
                                ),
                                comms_source.goal
                            )
                    end
    ...
    addCommsReply(_("upgrade-comms", "Do you upgrade spaceships?"), function()
        for i, station_info in ipairs(station_upgrade_list) do
            if station_info.station == comms_target then
                ...
                setCommsMessage(
                    _("upgrade-comms", "We don't upgrade spaceships")
                )
            end
        end
        addCommsReply(_("Back"), commsStation)
    ...
    addCommsReply(
        ...
        function()
            ...
            local out = _(
                "cartographyOffice-comms",
                "Without looking at you, the clerk tells you that the cartographers are out of the office for the race. They left this list of stations that provide upgrades for any racer that dropped by:"
            )
            if comms_target:isFriendly(comms_source) then
                for i, station_info in ipairs(station_upgrade_list) do
                    if station_info.station:isValid() then
                        out = string.format(
                            _(
                                "upgrade-comms",
                                "%s\nSector:%s Station:%s Upgrade:%s"
                            ),
                            out,
                            station_info.station:getSectorName(),
                            station_info.station:getCallSign(),
                            station_info.upgrade
                        )
                    end
                end
            else
                out = _(
                    "cartographyOffice-comms",
                    "Without looking at you, the clerk tells you that the cartographers are out of the office for the race. They left this list of neutral station locations in the area and what goods they might sell for any racer that dropped by:"
                )
                for i, station in ipairs(stationList) do
                    if
                        station:isValid()
                        and not station:isFriendly(comms_source)
                        and not station:isEnemy(comms_source)
                    then
                        out = string.format(
                            _("cartographyOffice-comms", "%s\n%s %s"),
                            out,
                        ...
                        if station.comms_data == nil then
                            out = string.format(
                                _("cartographyOffice-comms", "%s: none"),
                                out
                            )
                        else
    ...
    if comms_source.goods ~= nil then
        for good, quantity in pairs(comms_source.goods) do
            if quantity > 0 then
                ...
                goods_in_cargo_hold = string.format(
                    _("trade-comms", "%s\n%s: %s"),
                    goods_in_cargo_hold,
                    good_desc[good],
                    quantity
    ...
    if commerce_available then
        addCommsReply(_("trade-comms", "Buy, sell, trade"), function()
            local oMsg = string.format(
                _(
                    "trade-comms",
                    "Goods or components available for sale here:%s"
                ),
                goods_for_sale
            )
            if player_has_goods then
                oMsg = string.format(
                    _("trade-comms", "%s\nGoods in cargo hold:%s"),
                    oMsg,
                    goods_in_cargo_hold
                )
            else
            ...
            if station_sells then
                ...
                for good, good_data in pairs(comms_target.comms_data.goods) do
                    if good_data["quantity"] > 0 then
                        addCommsReply(
                            string.format(
                                _("trade-comms", "Buy a %s for %s reputation"),
                                good_desc[good],
                                good_data["cost"]
                            ),
                            function()
                                if not comms_source:isDocked(comms_target) then
                                    setCommsMessage(
                                        _(
                                            "trade-comms",
                                            "You have to be docked to complete the transaction."
                                        )
                                    )
                                    return
                                end
                                if comms_source.cargo < 1 then
                                    setCommsMessage(
                                        string.format(
                                            _(
                                                "trade-comms",
                                                "Not enough room on %s to purchase %s"
                                            ),
                    ...
                    for i, upgrade_good in ipairs(upgrade_goods) do
                        if
                            upgrade_good == good
                            and comms_source:getReputationPoints()
                                < good_data.cost
                        then
                ...
                if #alt_goods > 0 then
                        ...
                        if contract_completed then
                            for i, alt_good in ipairs(alt_goods) do
                                addCommsReply(
                                    string.format(
                                        _(
                                            "trade-comms",
                                            "Accept %s for removing asteroid"
                                        ),
                                        alt_good
                                    ),
                                    function()
                                        ...
                                        if
                                            comms_source.goods[alt_good] == nil
                                        then
                                            comms_source.goods[alt_good] = 0
                                        end
                                        comms_source.goods[alt_good] = comms_source.goods[alt_good]
                                            + 1
                                        comms_source.cargo = comms_source.cargo
                                            - 1
                                        comms_target.comms_data.goods[alt_good].quantity =
                                            math.max(
                                                0,
                                                comms_target.comms_data.goods[alt_good].quantity
                                                    - 1
                                        ...
                                        if
                                            #comms_source.asteroid_contract < 1
                                        then
                            ...
                            if contract_match then
                                addCommsReply(
                                    _(
                                        "trade-comms",
                                        "Where's the asteroid you want us to remove?"
                                    ),
                                    function()
                                        local a_bearing = angleHeading(
                                            comms_station,
                                            player_contract.a
                                        )
                                        local a_dist = distance(
                                            comms_target,
                                            player_contract.a
                                        ) / 1000
                                        setCommsMessage(
                                            string.format(
                                                _(
                                                    "trade-comms",
                                                    "Bearing %.1f, distance %.1fu from %s"
                                                ),
                                                a_bearing,
                                                a_dist,
                                                comms_target:getCallSign()
                    ...
                    elseif comms_source.enemy_station_contract ~= nil then
                        ...
                        for i, pesc in
                            ipairs(comms_source.enemy_station_contract)
                        do
                        ...
                        if contract_completed then
                            ...
                            if contract_match then
                                addCommsReply(
                                    "What station did you want us to remove?",
                                    function()
                                        setCommsMessage(
                                            string.format(
                                                "%s in sector %s",
                                                player_contract.station:getCallSign(),
                                                player_contract.station:getSectorName()
                                            )
                                        )
                                    end
                                )
                        ...
                        for i, obj in ipairs(objects) do
                            if isObjectType(obj, "Asteroid") then
                                ...
                                table.insert(
                                    asteroids,
                                    { a = obj, dist = distance(ax, ay, sx, sy) }
                                )
                            end
                        end
                        if #asteroids > 0 then
                            addCommsReply(
                                _(
                                    "trade-comms",
                                    "Can I get goods without spending reputation?"
                                ),
                                function()
                                    local out =
                                        _("trade-comms", "We could give you")
                                    if #alt_goods > 1 then
                                        local alt_good_list_string = ""
                                        for i, alt_good in ipairs(alt_goods) do
                                            if alt_good_list_string == "" then
                                                alt_good_list_string =
                                                    good_desc[alt_good]
                                            else
                                                alt_good_list_string =
                                                    string.format(
                                    ...
                                    local a_bearing =
                                        angleHeading(sx, sy, ax, ay)
                                    out = string.format(
                                        _(
                                            "trade-comms",
                                            "%s if you removed the asteroid at bearing %.1f, distance %.1fu for us. It's been mined out and has become a navigation hazard."
                                        ),
                                        out,
                                        a_bearing,
                                        asteroids[1].dist / 1000
                                    )
                                    setCommsMessage(out)
                                    addCommsReply(
                                        _(
                                            "trade-comms",
                                            "Agree to remove the asteroid"
                                        ),
                                        function()
                                            if
                                                comms_source.asteroid_contract
                                            ...
                                            setCommsMessage(
                                                _(
                                                    "trade-comms",
                                                    "Come back and ask to buy your desired resource after you have removed the asteroid."
                                                )
                                            )
                                        end
                                    )
                            ...
                            addCommsReply(
                                ...
                                function()
                                    ...
                                    addCommsReply(
                                        _(
                                            "trade-comms",
                                            "Would you exchange goods if we destroyed a station?"
                                        ),
                                        function()
                                            ...
                                            for i, station in
                                                ipairs(enemy_stations)
                                            do
                                                if station:isValid() then
                                                    local sx, sy =
                                                        station:getPosition()
                                                    table.insert(
                                                        sorted_enemy_stations,
                                                        {
                                                            station = station,
                                                            dist = distance(
                                                                tx,
                                            ...
                                            setCommsMessage(
                                                string.format(
                                                    _(
                                                        "trade-comms",
                                                        "We could make an exchange if %s were destroyed. It's the closest and thus disrupts our business the most."
                                                    ),
                                                    sorted_enemy_stations[1].station:getCallSign()
                                                )
                                            )
                                            addCommsReply(
                                                string.format(
                                                ...
                                                function()
                                                    ...
                                                    table.insert(
                                                        comms_source.enemy_station_contract,
                                                        {
                                                            station = sorted_enemy_stations[1].station,
                                                            contractor = comms_target,
                                                        }
                                                    )
                                                    setCommsMessage(
                                                        string.format(
                                                            _(
                                                                "trade-comms",
                                                                "Come back and ask to buy your desired resource after %s has been removed."
                                                            ),
                                                            sorted_enemy_stations[1].station:getCallSign()
                                                        )
                                                    )
    ...
    if goods[comms_target] ~= nil then
        addCommsReply(_("trade-comms", "Buy, sell, trade"), function()
            oMsg = _(
                "trade-comms",
                "Goods or components available here: quantity, cost in reputation\n"
            )
    ...
    if comms_target == stationZefram then
        if comms_source:hasJumpDrive() then
            if comms_source.goods ~= nil then
                if
                    comms_source.goods.nanites ~= nil
                    and comms_source.goods.nanites > 0
                then
                    addCommsReply(
                        _(
                            "upgrade-comms",
                            "Provide nanites for jump drive upgrade"
                        ),
                        function()
                            if comms_source.jump_upgrade then
                                ...
                                comms_source.goods.nanites = comms_source.goods.nanites
                                    - 1
                                comms_source.cargo = comms_source.cargo + 1
                                if
                                ...
                                setCommsMessage(
                                    _(
                                        "upgrade-comms",
                                        "Your jump drive has been upgraded"
                                    )
                                )
                ...
                if
                    comms_source.goods.robotic ~= nil
                    and comms_source.goods.robotic > 0
                then
                    addCommsReply(
                        _(
                            "upgrade-comms",
                            "Provide robotic for jump drive upgrade"
                        ),
                        function()
                            if comms_source.jump_upgrade then
                                setCommsMessage(
                                    _(
                                        "upgrade-comms",
                                        "You already have the jump drive upgrade"
                                    )
                                )
                            else
                                comms_source.goods.robotic = comms_source.goods.robotic
                                    - 1
                                comms_source.cargo = comms_source.cargo + 1
                                if
                                    comms_source:getTypeName()
            ...
            then
                addCommsReply(
                    string.format(
                        _(
                            "upgrade-comms",
                            "Provide dilithium for %f percent impulse engine speed upgrade"
                        ),
                        impulseBump
                    ),
                    function()
                        if comms_source.impulse_upgrade then
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "You already have the impulse drive upgrade"
                                )
                            )
                        else
                            comms_source.goods.dilithium = comms_source.goods.dilithium
                                - 1
                            comms_source.cargo = comms_source.cargo + 1
                            ...
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "Your impulse engine speed has been upgraded"
                                )
                            )
            ...
            then
                addCommsReply(
                    string.format(
                        _(
                            "upgrade-comms",
                            "Provide tritanium for %f percent impulse engine speed upgrade"
                        ),
                        impulseBump
                    ),
                    function()
                        if comms_source.impulse_upgrade then
                            ...
                            comms_source.goods.tritanium = comms_source.goods.tritanium
                                - 1
                            comms_source.cargo = comms_source.cargo + 1
                            comms_source:setImpulseMaxSpeed(
                                comms_source:getImpulseMaxSpeed()
            ...
            if comms_source.goods[spinComponent] > 0 then
                addCommsReply(
                    string.format(
                        _(
                            "upgrade-comms",
                            "Provide %s for %.2f percent maneuver speed upgrade"
                        ),
                        spinComponent,
                        spinBump
                    ),
                    function()
                        if comms_source.spin_upgrade then
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "You already have the maneuver speed upgrade"
                                )
                            )
                        else
                            comms_source.goods[spinComponent] = comms_source.goods[spinComponent]
                                - 1
                            comms_source.cargo = comms_source.cargo + 1
                            comms_source:setRotationMaxSpeed(
                                comms_source:getRotationMaxSpeed()
                                    * (1 + spinBump / 100)
                            )
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "Your spin speed has been upgraded"
                                )
                            )
            ...
            if comms_source.goods[beamComponent] > 0 then
                addCommsReply(
                    string.format(
                        _(
                            "upgrade-comms",
                            "Provide %s for %.2f percent beam range upgrade"
                        ),
                        beamComponent,
                        beamRangeBump
                    ),
                    function()
                        if comms_source.beam_range_upgrade then
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "You already have the beam range upgrade"
                                )
                            )
                        else
                            if comms_source:getBeamWeaponRange(0) < 1 then
                                setCommsMessage(
                                    _(
                                        "upgrade-comms",
                                        "Your ship does not support a beam weapon upgrade"
                                    )
                                )
                            else
                                comms_source.goods[beamComponent] = comms_source.goods[beamComponent]
                                    - 1
                                comms_source.cargo = comms_source.cargo + 1
                                local bi = 0
                                repeat
                                ...
                                setCommsMessage(
                                    _(
                                        "upgrade-comms",
                                        "Your beam range has been upgraded"
                                    )
                                )
            ...
            if comms_source.goods[tubeComponent] > 0 then
                addCommsReply(
                    string.format(
                        _(
                            "upgrade-comms",
                            "Provide %s for additional homing missile tube"
                        ),
                        tubeComponent
                    ),
                    function()
                        if comms_source.tube_upgrade then
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "You already have the missile tube upgrade"
                                )
                            )
                        else
                            comms_source.goods[tubeComponent] = comms_source.goods[tubeComponent]
                                - 1
                            comms_source.cargo = comms_source.cargo + 1
                            local original_tubes =
                            ...
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "You now have an additional homing missile tube"
                                )
                            )
            ...
            if comms_source.goods[energyComponent] > 0 then
                addCommsReply(
                    string.format(
                        _(
                            "upgrade-comms",
                            "Provide %s for 25 percent energy capacity upgrade"
                        ),
                        energyComponent
                    ),
                    function()
                        if comms_source.energy_upgrade then
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "You already have the energy capacity upgrade"
                                )
                            )
                        else
                            comms_source.goods[energyComponent] = comms_source.goods[energyComponent]
                                - 1
                            comms_source.cargo = comms_source.cargo + 1
                            ...
                            setCommsMessage(
                                _(
                                    "upgrade-comms",
                                    "You now have upgraded energy capacity"
                                )
                            )
                    ...
                    if comms_source:getShieldCount() == 1 then
                        addCommsReply(
                            string.format(
                                _(
                                    "upgrade-comms",
                                    "Provide %s for %.2f percent shield upgrade"
                                ),
                                shieldComponent,
                                shieldBump
                            ),
                            function()
                                if comms_source.front_shield_upgrade then
                                    setCommsMessage(
                                        _(
                                            "upgrade-comms",
                                            "You already have the shield upgrade"
                                        )
                                    )
                                else
                                    comms_source.goods[shieldComponent] = comms_source.goods[shieldComponent]
                                        - 1
                                    comms_source.cargo = comms_source.cargo + 1
                                    ...
                                    setCommsMessage(
                                        _(
                                            "upgrade-comms",
                                            "You now have upgraded shield capacity"
                                        )
                                    )
                    ...
                    elseif comms_source:getShieldCount() > 1 then
                        addCommsReply(
                            string.format(
                                _(
                                    "upgrade-comms",
                                    "Provide %s for %.2f percent front shield upgrade"
                                ),
                                shieldComponent,
                                shieldBump
                            ),
                            function()
                                if comms_source.front_shield_upgrade then
                                    setCommsMessage(
                                        _(
                                            "upgrade-comms",
                                            "You already have the front shield upgrade"
                                        )
                                    )
                                else
                                    comms_source.goods[shieldComponent] = comms_source.goods[shieldComponent]
                                    ...
                                    setCommsMessage(
                                        _(
                                            "upgrade-comms",
                                            "You now have upgraded front shield capacity"
                                        )
                                    )
                                    comms_source.front_shield_upgrade = true
                                end
                        ...
                        addCommsReply(
                            string.format(
                                _(
                                    "upgrade-comms",
                                    "Provide %s for %.2f percent rear shield upgrade"
                                ),
                                shieldComponent,
                                shieldBump
                            ),
                            function()
                                if comms_source.rear_shield_upgrade then
                                    setCommsMessage(
                                        _(
                                            "upgrade-comms",
                                            "You already have the rear shield upgrade"
                                        )
                                    )
                                else
                                    comms_source.goods[shieldComponent] = comms_source.goods[shieldComponent]
                                    ...
                                    setCommsMessage(
                                        _(
                                            "upgrade-comms",
                                            "You now have upgraded rear shield capacity"
                                        )
                                    )
...
function handleUndockedState()
    ...
    addCommsReply(_("station-comms", "I need information"), function()
        ...
        addCommsReply(
            _("upgrade-comms", "Do you upgrade spaceships?"),
            function()
                for i, station_info in ipairs(station_upgrade_list) do
                    if station_info.station == comms_target then
                        setCommsMessage(station_info.desc)
        ...
        if comms_target == stationTimer then
            addCommsReply(
                _("station-comms", "How far am I from the race start point?"),
                function()
                    local p_x, p_y = comms_source:getPosition()
                    local current_distance =
                        distance(p_x, p_y, racePoint1x, racePoint1y)
                    local qualify = _(
                        "station-comms",
                        "not close enough to qualify for the start of the race"
                    )
                    if current_distance < 5000 then
                        qualify = _(
                            "station-comms",
                            "close enough to qualify for the start of the race"
                        )
                    end
                    ...
                    if current_distance <= 1 then
                        setCommsMessage(
                            string.format(
                                _(
                                    "station-comms",
                                    "%s, my messaging terminal says you're %.1f unit away, %s."
                                ),
                                comms_source:getCallSign(),
                                current_distance,
                                qualify
                        ...
                        setCommsMessage(
                            string.format(
                                _(
                                    "station-comms",
                                    "%s, my messaging terminal says you're %.1f units away, %s."
                                ),
                                comms_source:getCallSign(),
                                current_distance,
                                qualify
        ...
        addCommsReply(
            _("station-comms", "Station docking service status"),
            function()
                local out =
                    _("station-comms", "Station docking services report:")
                out = string.format(
                    _("station-comms", "%s\nRestock scan probes: %s"),
                    out,
                    comms_target:getRestocksScanProbes()
                )
                out = string.format(
                    _("station-comms", "%s\nRecharge energy: %s"),
                    out,
                    comms_target:getSharesEnergyWithDocked()
                )
                out = string.format(
                    _("station-comms", "%s\nRepair hull: %s"),
                    out,
                    comms_target:getRepairDocked()
                )
                setCommsMessage(out)
                addCommsReply(_("Back"), commsStation)
            end
    ...
    if diagnostic then
        addCommsReply("Diagnostic data", function()
            if raceStartDelay > 0 then
                dMsg =
                    string.format("Seconds to race start: %f", raceStartDelay)
            else
                dMsg = string.format(
                    "Race has been running for %f seconds",
                    raceTimer
                )
            end
            dMsg = dMsg
            ...
            if raceStartDelay <= 0 then
                addCommsReply("Show unfinished racers", function()
                    dMsg =
                        string.format("Unfinished racers: %i", unfinishedRacers)
                    setCommsMessage(dMsg)
                    addCommsReply(_("Back"), commsStation)
                end)
            end
...
function commsShip()
    ...
    if comms_target.comms_data.goods == nil then
        ...
        comms_target.comms_data.goods[selected_good] =
            { cost = math.random(20, 80), quantity = 1 }
    end
    if comms_source:isFriendly(comms_target) then
...
function enemyComms()
    if comms_target.owner == nil then
        ...
        setCommsMessage(
            string.format(
                _("shipEnemy-comms", "I belong to %s"),
                comms_target.owner
            )
        )
        addCommsReply(_("Back"), commsShip)
...
function spawnTargetDrone(originx, originy, targetDroneID, area, sequenceNumber)
    ...
    td = CpuShip()
        :setTemplate(enemyTemplate)
        :setPosition(originx + tdx, originy + tdy)
        :setFaction("Kraylor")
...
function namePlayerShip(p, template_type)
    if
        playerShipNamesFor[template_type] ~= nil
        and #playerShipNamesFor[template_type] > 0
    then
...
function soloComplete()
    ...
    gMsg = string.format(
        _("msgMainscreen", "Race completed in %.2f seconds. Time rank: %s"),
        playerList[1].raceTime,
        playerList[1].timeRank
    )
    eliminatedDrones = countEliminatedDrones(playerList[1])
    gMsg = gMsg
        .. string.format(
            _("msgMainscreen", "\nTarget drones eliminated: %i"),
            eliminatedDrones
        )
end
function soloExpire()
    ...
    gMsg = _(
        "msgMainscreen",
        "Race administrators got tired of waiting. Race stopped after 2000 seconds. Time Rank: Cadet"
    )
    eliminatedDrones = countEliminatedDrones(playerList[1])
    gMsg = gMsg
        .. string.format(
...
function competeExpire()
    ...
    gMsg = string.format(
        _(
            "msgMainscreen",
            "Race administrators got tired of waiting. Race stopped after %.2f seconds."
        ),
        raceTimer
    )
    competeResults()
    ...
    if pi ~= nil then
        ...
        gMsg = gMsg
            .. string.format(
                _(
                    "msgMainscreen",
                    "\n%s time: %.2f seconds. Time rank: %s. Placement points: %i"
                ),
                playerList[pi]:getCallSign(),
                playerList[pi].raceTime,
                playerList[pi].timeRank,
                playerList[pi].timePoints
            )
    else
...
function unorderedFinalTally()
    for pl = 1, #playerList do
        gMsg = gMsg
            .. string.format(
                _("msgMainscreen", "%s Drones shot: %i, Total score: %i. "),
                playerList[pl]:getCallSign(),
                playerList[pl].dronePoints,
                playerList[pl].score
            )
    end
end
function finalTally()
    ...
    for plo = 1, outerIndex do
        ...
        gMsg = gMsg
            .. string.format(
                _("msgMainscreen", "%s Drones shot: %i, Total score: %i. "),
                bestPlayer:getCallSign(),
                bestPlayer.dronePoints,
                bestPlayer.score
            )
...
function gatherStats(final_score)
    ...
    for pidx = 1, 32 do
        ...
        if p ~= nil then
            ...
            if p:isValid() then
                ...
                score_list[p:getCallSign()].rank = laps * 10000
                    + rank_goal * 1000
                if p.raceTime ~= nil then
                    score_list[p:getCallSign()].rank = score_list[p:getCallSign()].rank
                        + (original_patience_time_limit - p.raceTime)
                ...
                if p.raceTime ~= nil then
                    score_list[p.name].rank = score_list[p:getCallSign()].rank
                        + (original_patience_time_limit - p.raceTime)
                    score_list[p.name].time = p.raceTime
                end
                stat_list.ship[p.name] = {
                    is_alive = false,
                    participant = participant,
                    lap_count = laps,
                    waypoint_goal = goal,
            ...
            if stat_list.ship[name] == nil then
                if p ~= nil then
                    ...
                    if score_list[name] ~= nil then
                        ...
                        if p.raceTime ~= nil then
                            score_list[name].rank = score_list[p:getCallSign()].rank
                                + (original_patience_time_limit - p.raceTime)
    ...
    for name, details in pairs(score_list) do
        table.insert(sorted_score_list, {
            name = name,
            drone_points = details.drone_points,
            rank = details.rank,
            time = details.time,
        })
    end
    ...
    if
        sorted_score_list ~= nil
        and #sorted_score_list > 0
        and player_count > 0
    then
    ...
    if final_score then
        print(
            "in gather stats function when the final score is being calculated"
        )
        local sorted_stat_list = {}
        for name, details in pairs(stat_list.ship) do
            table.insert(sorted_stat_list, {
                name = name,
                lap_count = details.lap_count,
                waypoint_goal = details.waypoint_goal,
                score = details.score,
                rank_points = details.rank_points,
                drone_points = details.drone_points,
                time = details.time,
            })
        end
        table.sort(sorted_stat_list, function(a, b)
            return a.score > b.score
                or (a.score == b.score and a.lap_count > b.lap_count)
                or (
                    a.score == b.score
                    and a.lap_count == b.lap_count
                    and a.time < b.time
                )
        end)
...
function update(delta)
            ...
            if p ~= nil and p:isValid() then
                if p.nameAssigned == nil then
                    ...
                    if use_fixed then
                        ...
                        p.control_code =
                            predefined_player_ships[pps_index].control_code
                        p:setControlCode(
                            predefined_player_ships[pps_index].control_code
                        )
                    else
                        namePlayerShip(p, tempPlayerType)
                        local control_code_index =
                            math.random(1, #control_code_stem)
                    ...
                    print(
                        "Control Code for " .. p:getCallSign(),
                        p.control_code
                    )
    ...
    if raceInstructionMessage ~= "sent" then
        ...
        primaryOrders = string.format(
            _(
                "raceOrders-comms",
                "Start race on time at waypoint 1\nRace Length: %f units"
            ),
            raceLength
        )
        for p1idx = 1, 32 do
                ...
                if delay_minutes > 1 then
                    p1:addToShipLog(
                        string.format(
                            _(
                                "raceOrders-shipLog",
                                "Race starts in %i minutes. Be at waypoint 1 on time or forfeit"
                            ),
                            delay_minutes
                        ),
                        "Magenta"
                    ...
                    p1:addToShipLog(
                        string.format(
                            _(
                                "raceOrders-shipLog",
                                "Race starts in %i minute. Be at waypoint 1 on time or forfeit"
                            ),
                            delay_minutes
                        ),
                        "Magenta"
    ...
    if raceStartDelay > 0 then
        ...
        stationTimer:setCallSign(
            string.format(
                _("race-", "Race Start Countdown: %.2f"),
                raceStartDelay
            )
        )
        if stationsBuilt == "done" then
            for p2idx = 1, 32 do
                ...
                if p2 ~= nil and p2:isValid() then
                    ...
                    if
                        ...
                        and raceStartDelay > 1
                        and raceStartDelay < 2
                    then
            ...
            if
                reset_1 == nil
                and raceStartDelay > 0
                and raceStartDelay < 0.5
            then
                start_zone:setColor(0, 64, 0)
                reset_1 = "reset"
            end
        ...
        if startLineCheck ~= "done" then
            ...
            raceTimer = 0
            primaryOrders =
                _("raceOrders-comms", "Complete race. Win if possible.")
            player_start_list = {}
            ...
            for p4idx = 1, 32 do --make some target drones
                ...
                if
                    p4 ~= nil
                    and p4:isValid()
                    and p4.participant == "participant"
                then
            ...
            stationTimer:setCallSign(
                string.format(_("race-", "Race Run Time %.2f"), raceTimer)
            )
            ...
            if follow_up_message == nil then
                ...
                if player_count > 0 then
                    ...
                    globalMessage(
                        _(
                            "race-msgMainscreen",
                            "Race aborted. Nobody made it to the starting line"
                        )
                    )
                    victory("Exuari")
                end
            ...
            for p5idx = 1, 32 do
                ...
                if
                    p5 ~= nil
                    and p5:isValid()
                    and p5.participant == "participant"
                    and p5.laps < 3
                then
                    ...
                    local name_tag_text = string.format(
                        _("race-tabHelms&Tactical&Singlepilot", "%s in %s"),
                        p5:getCallSign(),
                        p5:getSectorName()
                    )
                        ...
                        if distance(p5, racePoint2x, racePoint2y) < 1000 then
                            ...
                            p5:addToShipLog(
                                string.format(
                                    _(
                                        "race-shipLog",
                                        "Waypoint 2 met. Go to waypoint 3. Leg took %f seconds. You have completed %i %s."
                                    ),
                                    p5.legtimer,
                                    p5.laps,
                                    lapString
                        ...
                        if distance(p5, racePoint3x, racePoint3y) < 1000 then
                            ...
                            p5:addToShipLog(
                                string.format(
                                    _(
                                        "race-shipLog",
                                        "Waypoint 3 met. Go to waypoint 4. Leg took %f seconds. You have completed %i %s."
                                    ),
                                    p5.legtimer,
                                    p5.laps,
                                    lapString
                        ...
                        if distance(p5, racePoint4x, racePoint4y) < 1000 then
                            ...
                            p5:addToShipLog(
                                string.format(
                                    _(
                                        "race-shipLog",
                                        "Waypoint 4 met. Go to waypoint 1. Leg took %f seconds. You have completed %i %s."
                                    ),
                                    p5.legtimer,
                                    p5.laps,
                                    lapString
                            ...
                            if p5.laps >= 3 then
                                ...
                                p5:addToShipLog(
                                    string.format(
                                        _(
                                            "race-shipLog",
                                            "Completed race. Race time in seconds: %f"
                                        ),
                                        p5.raceTime
                                    ),
                                    "Magenta"
                                ...
                                p5:addToShipLog(
                                    string.format(
                                        _(
                                            "race-shipLog",
                                            "Waypoint 1 met. Go to waypoint 2. Leg took %f seconds. You have completed %i %s. Lap took %f seconds."
                                        ),
                                        p5.legtimer,
                                        p5.laps,
                                        lapString,
            ...
            if player_count > 0 then
                if
                    finished_racers < #reward_grid[player_count]
                    and reward_grid[player_count][finished_racers + 1] == 0
                then
