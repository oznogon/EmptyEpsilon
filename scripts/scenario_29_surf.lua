...
function init()
    ...
    print(
        string.format(
            "    ----    Scenario: Surf's Up!    ----    Version %s    ----    Tested with EE version %s    ----",
            scenario_version,
            ee_version
        )
    )
...
function setVariations()
    enemy_config = {
        ["Easy"] = { number = 0.5, desc = _("msgMainscreen", "Easy") },
...
function constructEnvironment()
    ...
    local station_x, station_y =
        vectorFromAngle(station_angle, random(2000, 5000))
    local name_category = tableRemoveRandom(name_categories)
    local station =
        placeStation(station_x, station_y, name_category, "Human Navy")
    setReinforcements(station)
    table.insert(friendly_stations, station)
    table.insert(station_list, station)
    local dx, dy =
        vectorFromAngle(station_angle + random(-60, 60), random(2000, 5000))
    ...
    for i, station in ipairs(friendly_stations) do
        if not station:getRestocksScanProbes() then
            if station.probe_fail_reason == nil then
                ...
                mission_goods["restock_probes"] =
                    mission_reasons["restock_probes"][station.probe_fail_reason]
            end
        end
        if not station:getRepairDocked() then
            if station.repair_fail_reason == nil then
                ...
                mission_goods["hull"] =
                    mission_reasons["hull"][station.repair_fail_reason]
            end
        end
        if not station:getSharesEnergyWithDocked() then
            if station.energy_fail_reason == nil then
                ...
                mission_goods["energy"] =
                    mission_reasons["energy"][station.energy_fail_reason]
            end
        end
    end
    -- Random nebulae
    ...
    for n = 1, 5 do
        local d_neb_x, d_neb_y =
            vectorFromAngle(random(0, 360), random(2500, 10000))
        table.insert(
            nebulas,
            Nebula():setPosition(neb_x + d_neb_x, neb_y + d_neb_y)
        )
    end
    -- Random asteroids
    for cnt = 1, random(2, 7) do
        ...
        for a_cnt = 1, 25 do
            ...
            Asteroid()
                :setPosition(base_x + dx1 + dx2, base_y + dy1 + dy2)
                :setSize(random(4, 300) + random(4, 300) + random(4, 300))
        end
        for a_cnt = 1, 50 do
...
function setImprovementMissions()
    ...
    for i, neutral_station in ipairs(neutral_stations) do
        if
            neutral_station.comms_data ~= nil
            and neutral_station.comms_data.goods ~= nil
        then
    ...
    for mission, stations_goods in pairs(missions_stations_goods) do
        ...
        if #station_pool > 0 then
            ...
            local good = stations_goods[selected_station][math.random(
                1,
                #stations_goods[selected_station]
            )]
            if #already_selected_good > 0 then
                ...
                if not good_selected then
                    mission_good[mission] =
                        { good = good, station = selected_station }
        ...
        if details.station == nil then
            for i, station in ipairs(neutral_stations) do
                if station.selected_mission_good == nil then
                    ...
                    station.comms_data.goods[details.good] = {
...
function setReinforcements(station)
    ...
    station.comms_data.service_cost.phobos_reinforcements =
        math.random(175, 225)
    station.comms_data.service_cost.stalker_reinforcements =
        math.random(275, 325)
    station.comms_data.service_available = {}
    station.comms_data.service_available.amk3_reinforcements = random(1, 100)
        < 72
    station.comms_data.service_available.hornet_reinforcements = random(1, 100)
        < 72
    station.comms_data.service_available.reinforcements = true
    station.comms_data.service_available.amk8_reinforcements = random(1, 100)
        < 72
    station.comms_data.service_available.phobos_reinforcements = random(1, 100)
        < 72
    station.comms_data.service_available.stalker_reinforcements = random(1, 100)
        < 72
end
--------------------------------
...
function getDuration()
    ...
    if duration > 60 then
        ...
        if minutes > 1 then
            ...
            duration_string = string.format(
                _("msgMainscreen", "%s minute and %s seconds"),
                minutes,
                seconds
            )
        end
        ...
        duration_string =
            string.format(_("msgMainscreen", "%s seconds"), duration_string)
    end
...
function earlyEnd()
    if trigger_end_status == nil then
        ...
        addGMFunction(_("buttonGM", "End with Odin"), function()
            ...
            addGMMessage(
                _("msgGM", "30 second countdown to Odin spawn has begun.")
            )
            clearGMFunctions()
        end)
    elseif trigger_end_status == "started" then
        if getScenarioTime() > odin_spawn_time then
            if friendly_stations ~= nil then
                if endOdin == nil then
                    ...
                    if #station_pool > 1 then
                        ...
                        local oy = (fy + sy) / 2
                        endOdin =
                            CpuShip():setTemplate("Odin"):setFaction("Ghosts")
                    ...
                    endOdin
                        :setImpulseMaxSpeed(30)
                        :setRotationMaxSpeed(5)
                        :setAcceleration(5)
                    local forward, reverse = endOdin:getImpulseMaxSpeed()
                    endOdin:setImpulseMaxSpeed(
                        forward * speed_factor,
                        reverse * speed_factor
                    )
                    endOdin:setRotationMaxSpeed(
                        endOdin:getRotationMaxSpeed() * speed_factor
                    )
                    endOdin:setAcceleration(
                        endOdin:getAcceleration() * speed_factor
                    )
            ...
            local banner =
                string.format(_("timer-tab", "Anomalous Time:%s"), out_time)
            for i, p in ipairs(getActivePlayerShips()) do
...
function spawnWave()
    ...
    if not asteroid_storm then
        if
            random(1, 100) - (wave_number + wave_advance) < 27
            and #storm_asteroids == 0
        then
            asteroid_storm = true
        end
    end
    ...
    enemy_strength = math.pow((wave_number + wave_advance), 1.35)
            * 10
            * enemy_power
        + player_power
    ...
    local wave_styles = {
        {
            ...
            chance = 10,
            msg = string.format(
                _("shipLog", "Wave %i. Hunt down enemy base."),
                wave_number
            ),
            func = spawnDefense,
        },
        ...
        },
        {
            style = "formation",
            chance = 50,
            msg = string.format(_("shipLog", "Wave %i."), wave_number),
            func = spawnFormation,
        },
        {
            style = "simple",
    ...
    if asteroid_storm then
        wave_message = string.format(
            _("shipLog", "%s Asteroid storm reported."),
            wave_style.msg
        )
    end
    for i, p in ipairs(getActivePlayerShips()) do
        ...
        local wave_number_out =
            string.format(_("tabRelay&Ops", "Wave %i"), wave_number)
...
function spawnSimple()
    ...
    while enemy_strength > 0 do
        ...
        if
            selected_ship.warp_jammer ~= nil
            and selected_ship.warp_jammer ~= "none"
        then
        ...
        if not split then
            if current_enemy_strength >= enemy_strength then
                ...
                local dist =
                    random(30000, 35000 + 1000 * (wave_number + wave_advance))
...
function spawnBase()
    for i, p in ipairs(getActivePlayerShips()) do
        ...
        if
            distance(px, py, base_spawn_x, base_spawn_y)
            < p:getLongRangeRadarRange()
        then
            base_spawn_x, base_spawn_y = vectorFromAngle(
                spawn_angle,
                spawn_range + p:getLongRangeRadarRange()
            )
            break
        end
    end
    enemy_station =
        placeStation(base_spawn_x, base_spawn_y, "Sinister", "Ghosts")
...
function baseDestroyed(self, instigator)
    ...
    for i, enemy in ipairs(enemy_list) do
        if
            enemy ~= nil
            and enemy:isValid()
            and enemy:getTypeName() == "Defense platform"
        then
    ...
    if destruct_warning then
        for i, p in ipairs(getActivePlayerShips()) do
            p:addToShipLog(
                _("shipLog", "Defense platform self destruct activated."),
                "Red"
            )
        end
    end
...
function subWave()
    ...
    elseif getScenarioTime() > sub_wave_time then
        sub_wave_time = nil
        enemy_strength = math.pow((wave_number + wave_advance), 1.3)
            * 10
            * enemy_power
        if enemy_station ~= nil and enemy_station:isValid() then
            ...
            while enemy_strength > 0 do
                ...
                repeat
                    selected_ship =
                        ship_templates[math.random(1, #ship_templates)]
                    attempts = attempts + 1
                ...
                if spawn_point_leader == nil then
                    ...
                    local defend_x, defend_y =
                        vectorFromAngle(random(0, 360), 5000)
                    ship:setPosition(
                        base_spawn_x + defend_x,
                        base_spawn_y + defend_y
                    )
                    ship:orderDefendTarget(spawn_point_leader)
                end
                enemy_strength = enemy_strength - selected_ship.strength
                ...
                if enemy ~= nil and enemy:isValid() then
                    if enemy:getTypeName() == "Defense platform" then
                        ...
                        for j, p in ipairs(getActivePlayerShips()) do
                            if
                                distance(enemy, p) < p:getLongRangeRadarRange()
                            then
                                explode_it = true
                                break
                            end
...
function spawnFormation()
    ...
    if #form_pool > 0 then
        ...
        print(
            "Formation pool was not empty. Selected formation:",
            selected_form.form,
            selected_form.template,
            "strength:",
            selected_form.strength
        )
    else
        print(
            "Formation pool was empty. Weakest formation selected:",
            selected_form.form,
            selected_form.template,
            "strength:",
            selected_form.strength
    ...
    local fly_angle =
        angleFromVectorNorth(target_x, target_y, base_spawn_x, base_spawn_y)
...
function asteroidStorm()
    ...
    if clean_list then
        if asteroid_storm then
            if #storm_asteroids < 150 then
                ...
                if getScenarioTime() > storm_asteroid_time then
                    ...
                    local edge_start_x, edge_start_y =
                        vectorFromAngleNorth(asteroid_angle, 80000)
                    local launch_x, launch_y = vectorFromAngleNorth(
                        asteroid_angle + 90,
                        random(-80000, 80000)
                    )
                    local asteroid_kind = {}
                    ...
                    if
                        asteroid_kind[math.random(1, #asteroid_kind)] == "real"
                    then
                        storm_asteroid = Asteroid():setSize(
                            random(4, 300) + random(4, 300) + random(4, 300)
                        )
                    else
                        storm_asteroid = VisualAsteroid():setSize(
                            random(4, 300) + random(4, 300) + random(4, 300)
                        )
                    end
                    storm_asteroid:setPosition(
                        edge_start_x + launch_x,
                        edge_start_y + launch_y
                    )
                    storm_asteroid.trajectory = asteroid_angle + 180
                    storm_asteroid.speed = random(10, 100)
                    storm_asteroid.dist = distance(
                        0,
                        0,
                        edge_start_x + launch_x,
                        edge_start_y + launch_y
        ...
        for i, asteroid in ipairs(storm_asteroids) do
            local dx, dy =
                vectorFromAngleNorth(asteroid.trajectory, asteroid.speed)
            local ax, ay = asteroid:getPosition()
        ...
        for i, transport in ipairs(transport_list) do
            if transport:isDocked(transport.target) then
                ...
                if getScenarioTime() > transport.undock_time then
                    ...
                    transport.target =
                        station_list[math.random(1, #station_list)]
            ...
            for j, p in ipairs(getActivePlayerShips()) do
                ...
                if distance(p, enemy) < 5000 then
                    ...
                    if enemy.jammer_drop_time == nil then
                        enemy.jammer_drop_time = getScenarioTime()
                            + random(5, 10)
                    end
                    if getScenarioTime() > enemy.jammer_drop_time then
                        enemy.jammer_drop_time = nil
                        local ex, ey = enemy:getPosition()
                        enemy.active_warp_jammer = WarpJammer()
                            :setPosition(ex, ey)
                            :setRange(20000)
                            :setFaction("Ghosts")
        ...
        if zone ~= nil and zone:isValid() then
            if zone:isInside(p) then
                if zone.detriment == "beam" then
                    p:setSystemHealth(
                        "beamweapons",
                        p:getSystemHealth("beamweapons") * 0.999
                    )
                    if zone.player_warn == nil then
                        ...
                        if zone.player_warn[p] == nil then
                            zone.player_warn[p] =
                                { time = getScenarioTime() + 5, sent = false }
                        else
                            if not zone.player_warn[p].sent then
                                if
                                    getScenarioTime() > zone.player_warn[p].time
                                then
                                    ...
                                    p:addCustomMessage(
                                        ...
                                        _(
                                            "msgScience",
                                            "In addition to jamming our FTL drive, there's a warp jammer that is damaging our beam weapons"
                                        )
                                    )
                ...
                elseif zone.detriment == "missile" then
                    p:setSystemHealth(
                        "missilesystem",
                        p:getSystemHealth("missilesystem") * 0.999
                    )
                    if zone.player_warn == nil then
                        zone.player_warn = {}
                    else
                            ...
                            if not zone.player_warn[p].sent then
                                ...
                                then
                                    ...
                                    p:addCustomMessage(
                                        ...
                                        _(
                                            "msgScience",
                                            "In addition to jamming our FTL drive, there's a warp jammer that is damaging our missile systems"
                                        )
                                    )
                                    p.zone_warn_msg_ops = "zone_warn_msg_ops"
                                    p:addCustomMessage(
...
function update(delta)
    ...
    for i, enemy in ipairs(enemy_list) do
        if enemy ~= nil and enemy:isValid() then
            ...
            if
                isObjectType(enemy, "SpaceStation")
                or enemy:getTypeName() == "Defense platform"
            then
                enemy_base_count = enemy_base_count + 1
            else
                if
                    enemy:getOrder() == "Idle"
                    or enemy:getOrder() == "Defend Location"
                then
                    enemy:orderRoaming()
                end
            end
    ...
    if enemy_count == 0 then
        ...
        if wave_number == 0 then
            ...
            for i, p in ipairs(getActivePlayerShips()) do
                p:addToShipLog(
                    string.format(
                        _("msgMainscreen", "Wave %i cleared."),
                        wave_number
                    ),
                    "Green"
                )
    ...
    if friendly_count == 0 then
        ...
        if player_spawn_count > 1 then
            msg = string.format(
                _("msgMainscreen", "%s\n%i player ships deployed."),
                msg,
                player_spawn_count
            )
        else
            msg = string.format(
                _("msgMainscreen", "%s\nOne player ship deployed."),
                msg
            )
        end
        if completed_waves > 1 then
            msg = string.format(
                _("msgMainscreen", "%s\n%i waves completed at the %s setting."),
                msg,
                completed_waves,
                enemy_config[getScenarioSetting("Enemies")].desc
            )
        elseif completed_waves > 0 then
            msg = string.format(
                _("msgMainscreen", "%s\nOne wave completed at the %s setting."),
                msg,
                completed_waves,
                enemy_config[getScenarioSetting("Enemies")].desc
            )
            ...
            msg = string.format(
                _("msgMainscreen", "%s\nNo waves completed at the %s setting."),
                msg,
                enemy_config[getScenarioSetting("Enemies")].desc
            )
        end
