...
function init()
    ...
    research_station =
        SpaceStation():setTemplate("Small Station"):setFaction("Human Navy")
    research_station:setPosition(23500, 16100):setCallSign("Research-1")
    main_station =
        SpaceStation():setTemplate("Medium Station"):setFaction("Human Navy")
    main_station:setPosition(-25200, 32200):setCallSign("Orion-5")
    enemy_station =
        SpaceStation():setTemplate("Large Station"):setFaction("Exuari")
    ...
    CpuShip()
        ...
        :setFaction("Exuari")
        :setPosition(-44000, -14000)
        :orderDefendTarget(enemy_station)
    CpuShip()
        :setTemplate("Phobos T3")
        :setFaction("Exuari")
        :setPosition(-47000, -14000)
        :orderDefendTarget(enemy_station)
    enemy_dreadnought =
        CpuShip():setTemplate("Atlantis X23"):setFaction("Exuari")
    enemy_dreadnought
        :setPosition(-46000, -18000)
        :orderDefendTarget(enemy_station)
    CpuShip()
        :setTemplate("MT52 Hornet")
        :setFaction("Exuari")
        :setPosition(-46000, -18100)
        :orderDefendTarget(enemy_dreadnought)
    CpuShip()
        :setTemplate("MT52 Hornet")
        :setFaction("Exuari")
        :setPosition(-46000, -18200)
        :orderDefendTarget(enemy_dreadnought)
    CpuShip()
        :setTemplate("MT52 Hornet")
        :setFaction("Exuari")
        :setPosition(-46000, -18300)
        :orderDefendTarget(enemy_dreadnought)

    transport_RT4 = CpuShip()
        :setTemplate("Flavia")
        :setFaction("Human Navy")
        :setPosition(3750, 31250)
    ...
    exuari_RT4_guard1 = CpuShip()
        :setTemplate("Adder MK5")
        :setFaction("Exuari")
        :setPosition(3550, 31250)
        :setRotation(0)
    exuari_RT4_guard2 = CpuShip()
        :setTemplate("Adder MK5")
        :setFaction("Exuari")
        :setPosition(3950, 31250)
        :setRotation(180)
...
Our last contact with RT-4 was before it entered the nebula at sector ]]
                .. transport_RT4:getSectorName()
                .. [[. The nebula is blocking our long-range scans, so we're asking you to investigate and recover RT-4 if possible.]]
        )
    )
...
function missionRT4UnderAttack(delta)
    if not transport_RT4:isValid() then
        ...
        transport_RT4_drop = SupplyDrop()
            :setFaction("Human Navy")
            :setPosition(3750, 31250)
            :setDescriptions(
                _("scienceDescription-lifepod", "Life Pod"),
                _("scienceDescription-lifepod", "J. J. Johnson in Life Pod")
            )
    ...
    if not transport_RT4_drop:isValid() then
        ...
        if transport_RT4_drop_time > 60 * 5 then
            ...
            research_station:sendCommsMessage(
                ...
                _(
                    "incCall",
                    [[J.J. Johnson seems to have suffocated. This is a great loss for our cause of peace.
...
Please deliver his body back to Research-1. We will arrange for you to take over his mission.]]
            ...
            research_station:sendCommsMessage(
                ...
                _(
                    "incCall",
                    [[Just received message that Sir Johnson is safely aboard your ship! Great job!

Please deliver the diplomat to Orion-5 in sector ]]
                        .. main_station:getSectorName()
                        .. [[. Do this by docking with the station.]]
                )
            )
        end
    end
...
The officers at Orion-5 will gladly make this trade, and they ask that you retrieve the criminals for them at Refugee-X in sector ]]
                            .. neutral_station:getSectorName()
                            .. [[.
...
function missionRT4Died(delta)
    if player:isDocked(research_station) then
        ...
        research_station:sendCommsMessage(
            ...
            _(
                "incCall",
                [[J.J. Johnson transmitted his mission details to Orion-5 before he passed away. Head to Orion-5 in sector ]]
                    .. main_station:getSectorName()
                    .. [[ for details.]]
            )
        )
...
function missionRetrieveCriminals(delta)
    if player:isDocked(neutral_station) then
        neutral_station:sendCommsMessage(
            ...
            _(
                "incCall",
                [[Two tough-looking criminals board your ship. They are already cuffed and do not look too happy about the situation.
...
function missionWaitForAmbush(delta)
    if distance(player, main_station) < 50000 then
        ...
        WarpJammer()
            :setFaction("Exuari")
            :setPosition(x - 2008, y + 2711)
            :setDescriptions(
                _("scienceDescription-artifact", "High powered field generator"),
                _(
                    "scienceDescription-artifact",
                    "Warp/Jump Jammer. Radius visible if jammer visible"
                )
            )
            :setScanningParameters(1, 1)
        ambush_main = CpuShip()
            ...
            :setTemplate("Starhammer II")
            :setScanned(true)
            :setPosition(x - 1667, y + 2611)
            :setRotation(-80)
            :orderAttack(player)
        ambush_side1 = CpuShip()
            :setFaction("Exuari")
            :setTemplate("Nirvana R5")
            :setScanned(true)
            :setPosition(x - 736, y + 2875)
            :setRotation(-80)
            :orderAttack(player)
        ambush_side2 = CpuShip()
            :setFaction("Exuari")
            :setTemplate("Nirvana R5")
            :setScanned(true)
            :setPosition(x - 2542, y + 2208)
            :setRotation(-80)
            :orderAttack(player)
...
function missionAmbushed(delta)
    if player:isDocked(main_station) then
        ...
        if
            not ambush_main:isValid()
            and not ambush_side1:isValid()
            and not ambush_side2:isValid()
        then
            message = _(
                "incCall",
...
We extracted some vital info from the Exuari. In the next transport convoy toward Research-1, an Exuari death squad is hiding in one of the ships. The transport detail is heading in from sector ]]
                    .. getSectorName(40000, -40000)
                    .. [[. Seek them out and scan the ships to find the Exuari transport.]]
            )
            if refilled then
                message = message
                    .. _("incCall", [[We have refitted your nukes and EMPs.]])
                    .. _(
                        "incCall",
            ...
            CpuShip()
                ...
                :orderDefendTarget(neutral_station)
                :setCommsScript("")
            CpuShip()
                :setTemplate("Nirvana R5")
                :setFaction("Human Navy")
            ...
            for n = 1, 5 do
                table.insert(
                    ...
                    CpuShip()
                        ...
                        :setPosition(
                            50000 + random(-10000, 10000),
                            -30000 + random(-10000, 10000)
                        )
                )
            end

            transport_target = CpuShip()
                :setTemplate("Personnel Freighter 2")
                :setFaction("Exuari")
                :setPosition(
                    50000 + random(-10000, 10000),
        ...
        if refilled then
            main_station:sendCommsMessage(
                player,
                _("incCall", [[We have refitted your nukes and EMPs.]])
                    .. _("incCall", [[Now to get those Exuari!]])
            )
        end
    end
end
...
function missionGotoTransport(delta)
    if distance(player, transport_target) < 30000 then
        main_station:sendCommsMessage(
            ...
            _(
                "incCall",
                [[Scan the transports to identify the Exuari one. When you have identified it, do NOT destroy it.
...
function missionIdentifyTransport(delta)
    if not transport_target:isValid() then
        ...
        main_station:sendCommsMessage(
            ...
            _(
                "incCall",
                [[What the hell? I told you NOT to destroy the transport.]]
            )
        )
        victory("Exuari")
...
function missionStopTransport(delta)
    ...
    elseif transport_target:getSystemHealth("impulse") <= 0.0 then
        main_station:sendCommsMessage(
            ...
            _(
                "incCall",
                [[The transport is disabled. We're dispatching a recovery team and need you to defend the transport from the Exuari, who will likely destroy it rather then let it fall into our hands.]]
            )
        )
        transport_target
            :setFaction("Independent")
            :orderIdle()
            :setCallSign(transport_target:getCallSign() .. "-CAP")
        ...
        transport_recovery_team = CpuShip()
            :setTemplate("Flavia")
            :setFaction("Human Navy")
            :setPosition(x - random(8000, 10000), y + random(8000, 10000))
        transport_recovery_team:setCallSign("RTRV"):setScanned(true)
        transport_recovery_team
            :orderFlyTowardsBlind(transport_target:getPosition())
            :setCommsScript("")
    end
end
...
function missionTransportDone(delta)
    if player:isDocked(main_station) then
        main_station:sendCommsMessage(
            ...
            _(
                "incCall",
                [[Thanks to the captured Exuari death squad, we now know the location of the Exuari base in the area.

Lead the assault on the Exuari base in sector ]]
                    .. enemy_station:getSectorName()
        ...
        CpuShip()
            ...
            :setFaction("Exuari")
            :setPosition(-44500, -15000)
            :orderDefendTarget(enemy_station)
        CpuShip()
            :setTemplate("Ranus U")
            :setFaction("Exuari")
            :setPosition(-43000, -9000)
            :orderAttack(player)
        mission_state = nil
    end
end
...
function update(delta)
    ...
    if
        not player:isValid()
        or not research_station:isValid()
        or not main_station:isValid()
    then
        victory("Exuari")
        return
    end
