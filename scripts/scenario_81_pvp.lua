...
function init()
    ...
    -- Stations
    shangri_la = SpaceStation()
        :setPosition(10000, 10000)
        :setTemplate("Large Station")
        :setFaction("Independent")
        :setRotation(random(0, 360))
        :setCallSign("Shangri-La")
        :setCommsFunction(shangrilaComms)

    do
        local faction = "Human Navy"
        shipyard_human = SpaceStation()
            :setPosition(-7500, 15000)
            :setTemplate("Small Station")
            :setFaction(faction)
            :setRotation(random(0, 360))
            :setCallSign("Mobile Shipyard")
            :setCommsFunction(stationComms)
        CpuShip()
            :setTemplate("Phobos T3")
            :setFaction(faction)
            :setPosition(-8000, 16500)
            :orderDefendTarget(shipyard_human)
            :setScannedByFaction(faction, true)
        CpuShip()
            :setTemplate("Phobos T3")
            :setFaction(faction)
            :setPosition(-6000, 13500)
            :orderDefendTarget(shipyard_human)
            :setScannedByFaction(faction, true)
        CpuShip()
            :setTemplate("Phobos T3")
            :setFaction(faction)
            :setPosition(-7000, 14000)
            :orderDefendTarget(shipyard_human)
            :setScannedByFaction(faction, true)
        CpuShip()
            :setTemplate("Nirvana R5")
            :setFaction(faction)
            :setPosition(-8000, 14000)
            :orderDefendTarget(shipyard_human)
            :setScannedByFaction(faction, true)
    end
    ...
    do
        ...
        shipyard_kraylor = SpaceStation()
            :setPosition(27500, 5000)
            :setTemplate("Small Station")
            :setFaction(faction)
            :setRotation(random(0, 360))
            :setCallSign("Forward Command")
            :setCommsFunction(stationComms)
        CpuShip()
            :setTemplate("Phobos T3")
            :setFaction(faction)
            :setPosition(29000, 5000)
            :orderDefendTarget(shipyard_kraylor)
            :setScannedByFaction(faction, true)
        CpuShip()
            :setTemplate("Phobos T3")
            :setFaction(faction)
        ...
        CpuShip()
            :setTemplate("Phobos T3")
            :setFaction(faction)
            :setPosition(27500, 6000)
            :orderDefendTarget(shipyard_kraylor)
            :setScannedByFaction(faction, true)
        CpuShip()
            :setTemplate("Nirvana R5")
            :setFaction(faction)
            :setPosition(27000, 5000)
            :orderDefendTarget(shipyard_kraylor)
            :setScannedByFaction(faction, true)
    end

    -- Spawn players
    gallipoli = PlayerSpaceship()
        :setFaction("Human Navy")
        :setTemplate("Atlantis")
        :setPosition(-8500, 15000)
        :setCallSign("HNS Gallipoli")
        :setScannedByFaction("Kraylor", false)
    crusader = PlayerSpaceship()
        :setFaction("Kraylor")
        :setTemplate("Atlantis")
        :setPosition(26500, 5000)
        :setCallSign("Crusader Naa'Tvek")
        :setScannedByFaction("Human Navy", false)
    ...
    shipyard_human:sendCommsMessage(
        ...
        string.format(
            _(
                "goal-incCall",
                "Captain, it seems that the Kraylor are moving to take the Shangri-La station in sector %s!\n\nProvide cover while our troop transports board the station to reclaim it.\n\nGood luck, and stay safe."
            ),
            shangri_la:getSectorName()
        )
    )
    ...
    do
        local faction = "Human Navy"
        local transport = spawnTransport()
            :setFaction(faction)
            :setPosition(-7000, 15000)
            :orderDock(shangri_la)
            :setScannedByFaction(faction, true)
        table.insert(troops_human, transport)
        CpuShip()
            :setTemplate("MT52 Hornet")
            :setFaction(faction)
            :setPosition(-7000, 15500)
            :orderDefendTarget(transport)
            :setScannedByFaction(faction, true)
        CpuShip()
            :setTemplate("MT52 Hornet")
            :setFaction(faction)
            :setPosition(-7000, 14500)
            :orderDefendTarget(transport)
            :setScannedByFaction(faction, true)
    end
    ...
    do
        ...
        CpuShip()
            :setTemplate("MT52 Hornet")
            :setFaction(faction)
            :setPosition(26500, 5500)
            :orderDefendTarget(transport)
            :setScannedByFaction(faction, true)
        CpuShip()
            :setTemplate("MT52 Hornet")
            :setFaction(faction)
            :setPosition(26500, 4500)
            :orderDefendTarget(transport)
            :setScannedByFaction(faction, true)
    end
end
...
function getStatusReport()
    return table.concat({
        ...
        string.format(
            _("situationReport-comms", "Human dominance: %d"),
            points_human
        ),
        string.format(
            _("situationReport-comms", "Kraylor dominance: %d"),
            points_kraylor
        ),
...
Greetings, Captain. What can we do for you?]]
        ...
        addCommsReply(
            ...
            function()
                ...
                setCommsMessage(
                    _(
                        "reinforcements-comms",
                        "Aye, Captain. We've deployed a squad with fighter escort to support the assault on Shangri-La."
                    )
                )
                if comms_target:getFaction() == "Human Navy" then
                    ...
                    CpuShip()
                        :setTemplate("MT52 Hornet")
                        :setFaction(comms_target:getFaction())
                        :setPosition(comms_target:getPosition())
                        :orderDefendTarget(transport)
                        :setScannedByFaction(comms_source:getFaction(), true)
        ...
        addCommsReply(
            ...
            function()
                ...
                setCommsMessage(
                    _(
                        "reinforcements-comms",
                        "Confirmed. We've dispatched a strike wing to support space superiority around Shangri-La."
                    )
                )
                local strike_leader = CpuShip()
                    :setTemplate("Phobos T3")
                    :setFaction(comms_target:getFaction())
                    :setPosition(comms_target:getPosition())
                    :orderDefendTarget(shangri_la)
                    :setScannedByFaction(comms_source:getFaction(), true)
                CpuShip()
                    :setTemplate("MU52 Hornet")
                    :setFaction(comms_target:getFaction())
                    :setPosition(comms_target:getPosition())
                    :orderFlyFormation(strike_leader, -1000, 0)
                    :setScannedByFaction(comms_source:getFaction(), true)
                CpuShip()
                    :setTemplate("MU52 Hornet")
                    :setFaction(comms_target:getFaction())
...
function addCommsReplySupply(args)
    ...
    addCommsReply(
        string.format(
            args.request .. " " .. _("ammo-comms", "(%d rep each)"),
            price
        ),
        function()
            if not comms_source:isDocked(comms_target) then
                ...
                not comms_source:takeReputationPoints(
                    price
                        * (
                            ...
                            - comms_source:getWeaponStorage(missile_type)
                        )
                )
            then
...
function supplyDialogue()
    ...
    addCommsReplySupply({
        ...
        reply_full = _(
            "ammo-comms",
            "Sorry, Captain, but you are fully stocked with homing missiles."
        ),
        reply_filled = _(
            "ammo-comms",
            "We've replenished your homing missile supply."
        ),
    })
    ...
    addCommsReplySupply({
        ...
        reply_full = _(
            "ammo-comms",
            "Captain, you already have all the mines you can fit in that ship."
        ),
    ...
    addCommsReplySupply({
        ...
        reply_full = _(
            "ammo-comms",
            "Your nukes are already charged and primed for destruction."
        ),
        reply_filled = _(
            "ammo-comms",
    ...
    addCommsReplySupply({
        ...
        reply_full = _(
            "ammo-comms",
            "All storage for EMP missiles is already full, Captain."
        ),
        reply_filled = _(
            "ammo-comms",
            "We've recalibrated the electronics and fitted you with all the EMP missiles you can carry."
        ),
    })
    ...
    addCommsReplySupply({
        ...
        reply_full = _(
            "ammo-comms",
            "Sorry, Captain, but you are fully stocked with HVLIs."
        ),
...
function update(delta)
    ...
    if not crusader:isValid() then
        if respawn_kraylor > 20 then
            crusader = PlayerSpaceship()
                :setFaction("Kraylor")
                :setTemplate("Atlantis")
                :setPosition(19000, -14500)
                :setCallSign("Crusader Elak'raan")
                :setScannedByFaction("Human Navy", false)
        else
    ...
    if
        ((not gallipoli:isValid()) and (not shipyard_human:isValid()))
        or points_kraylor > 50
    then
        victory("Kraylor")
    end

    if
        ((not crusader:isValid()) and (not shipyard_kraylor:isValid()))
        or points_human > 50
    then
        victory("Human Navy")
    end
    ...
    if wave_timer > 150 and (shipyard_human:isValid()) then
        do
            ...
            CpuShip()
                :setTemplate("MT52 Hornet")
                :setFaction(faction)
                :setPosition(-7000, 5500 + line)
                :orderDefendTarget(transport)
                :setScannedByFaction(faction, true)
            CpuShip()
                :setTemplate("MT52 Hornet")
                :setFaction(faction)
                :setPosition(-7000, 4500 + line)
                :orderDefendTarget(transport)
                :setScannedByFaction(faction, true)
        end
        ...
        do
            ...
            CpuShip()
                :setTemplate("MT52 Hornet")
                :setFaction(faction)
                :setPosition(27000, -5500 + line)
                :orderDefendTarget(transport)
                :setScannedByFaction(faction, true)
            CpuShip()
                :setTemplate("MT52 Hornet")
                :setFaction(faction)
                :setPosition(27000, -4500 + line)
                :orderDefendTarget(transport)
                :setScannedByFaction(faction, true)
        end
