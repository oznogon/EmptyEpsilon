...
function manageControlCodes()
    ...
    if #active_player_ships == 0 then
        addGMMessage(
            _(
                "msgGM",
                "There are no active player ships for you to view or set the control codes for."
            )
        )
        mainGMButtons()
    else
...
function viewControlCodes()
    ...
    for i, p in ipairs(active_player_ships) do
        if p.control_code ~= nil then
            table.insert(name_code_faction, {
                name = p:getCallSign(),
            ...
            table.insert(
                name_code_faction,
                { name = p:getCallSign(), faction = p:getFaction() }
            )
        end
    end
    if code_count > 0 then
        ...
        addGMMessage(
            _(
                "msgGM",
                "None of the active player ships have a control code set for you to view."
            )
        )
    end
end
function setControlCodes()
        ...
        if p ~= nil then
            addGMFunction(_("buttonGM", "+Set Control Code"), function()
                ...
                if dcp ~= nil then
                    ...
                    addGMMessage(
                        string.format(
                            _("msgGM", "%s now has a control code of %s"),
                            dcp:getCallSign(),
                            dcp.control_code
                        )
                    )
                    ...
                    addGMMessage(
                        _("msgGM", "Player ship not selected. No action taken")
                    )
                end
                setControlCodes()
            end)
    ...
    if default_player_ship_control_code ~= nil then
        addGMFunction(
            _("buttonGM", "+Default Control Codes"),
...
function changeCodeObject()
    ...
    if object_list ~= nil then
        if #object_list == 1 then
            ...
            addGMMessage(
                string.format(
                    _(
                        "msgGM",
                        "Object in %s selected to set control code.\nPlace control code in unscanned description field via tweak button"
                    ),
                    code_object:getSectorName()
                )
            )
            setControlCodes()
        else
            addGMMessage(
                _(
                    "msgGM",
                    "Select only one object to use to set control code via its unscanned description field. No action taken"
                )
            )
            setControlCodes()
        end
        ...
        addGMMessage(
            _(
                "msgGM",
                "Select an object to use to set control code via its unscanned description field. No action taken"
            )
        )
        setControlCodes()
    end
...
function setDefaultControlCodes()
    ...
    addGMFunction(_("buttonGM", "Set Defaults"), function()
        ...
        if #codes_set > 0 then
            out = _(
                "msgGM",
                "The following player ships have their control codes set:"
            )
...
function viewPlayersMatchesDefaults()
    ...
    local matches_out =
        _("msgGM", "No matches between player ships and defaults.")
    table.sort(matches)
    if #matches > 0 then
        matches_out =
            _("msgGM", "Matches between active player ships and defaults:")
    ...
    for i, name in ipairs(player_ship_names) do
        player_ships_out =
            string.format(_("msgGM", "%s\n%s"), player_ships_out, name)
    end
    local ship_codes = {}
    for name, code in pairs(default_player_ship_control_code) do
        table.insert(
            ship_codes,
            string.format(_("msgGM", "%s: %s"), name, code)
        )
    end
    ...
    if #matches == #active_player_ships then
        out = _(
            "msgGM",
            "All active player ships have matches in the list of defaults.\n"
        )
